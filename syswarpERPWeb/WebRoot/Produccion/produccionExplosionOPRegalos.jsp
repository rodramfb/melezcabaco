...
String color_fondo ="";
String titulo = "ORDENES DE PRODUCCION PENDIENTES PARA REGALOS EMPRESARIOS  ";
...
%>
<html>
<jsp:useBean id="BPMPA"  class="ar.com.syswarp.web.ejb.BeanProduccionExplosionOPRegalos"   scope="page"/>
...
<form action="produccionExplosionOPRegalos.jsp" method="POST" name="frm" >
...
<table width="100%" border="0" cellspacing="1" cellpadding="1" name="rsTable" id=rsTable cols=<%=totCol+2-4%>  >
  <tr class="fila-encabezado">
     <td width="2%" >&nbsp;</td>
		 ...
		 <td width="2%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);">&nbsp;</td>
		 <td width="5%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);">Realizado</td>
		 <td width="3%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><div align="right"><%=tituCol[3]%></div></td>
     <td width="3%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><div align="right"><%=tituCol[4]%></div></td>
		 
     <td width="5%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><div align="right"><%=tituCol[0]%></div></td>
     <td width="5%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><div align="right"><%=tituCol[12]%></div></td>
     <td width="3%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><div align="right"><%=tituCol[1]%></div></td>
     <td width="9%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><%=tituCol[8]%></td>
     <td width="43%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><%=tituCol[9]%></td>
   ...
   <tr onMouseOver="mOvr(this,this.className='fila-det-rojo');" onMouseOut="mOut(this,this.className='<%=color_fondo%>');" class="<%=color_fondo%>" scope="col" > 
      ...
      <td bgcolor="#E1EEF4" class="fila-det-border" ><img src="../imagenes/default/gnome_tango/devices/stock_printers.png" width="18" height="18" onClick="abrirVentana('produccionCalculoNecesidadFrame.jsp?idop=<%=sCampos[0]%>&tipoPedido=R');" style="cursor:pointer"></td>
      <td class="fila-det-border" ><img src="../imagenes/default/gnome_tango/apps/pdf.jpg" title="Reimprimir " width="18" height="18" onClick="abrirVentana('../reportes/jasper/generaPDF.jsp?plantillaImpresionJRXML=notas_pedido_regalos&idpedido_cabe=<%=sCampos[12]%>','pedido',750, 400)" style="cursor:pointer"></td>
      <td bgcolor="#E1EEF4" class="fila-det-border" ><input name="cantidadparcial" type="text" disabled class="style1 style2" id="cantidadparcial" value="<%=gb.getNumeroFormateado(Float.parseFloat(sCampos[4] ) - Float.parseFloat(sCampos[3]) , 10, 2)%>" size="5" maxlength="10">			</td>
      <td bgcolor="#E1EEF4" class="fila-det-border" ><div align="right"><%=gb.getNumeroFormateado(Float.parseFloat(sCampos[3]) , 10, 2)%>
          <input name="realizado" type="hidden" value="<%=gb.getNumeroFormateado(Float.parseFloat(sCampos[3] ), 10, 2)%>"id="realizado" disabled>			
        </div></td>
      <td bgcolor="#E1EEF4" class="fila-det-border" ><div align="right"><%=gb.getNumeroFormateado(Float.parseFloat(sCampos[4]), 10, 2 )%>
      ...
      <td class="fila-det-border" ><div align="center"><img src="../imagenes/default/gnome_tango/status/info.png" width="22" height="22" title="Observaciones:<%=sCampos[7]%>"></div></td>
    </tr>
<%
   }
	 %>
