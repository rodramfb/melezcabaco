...
<title><%=titulo%></title>
 ...
 <script >
function bajarDatos(idlocalidad, localidad, idprovincia, provincia, cpostal){
	for(var i = 0; i<10;i++)
		if(opener.document.forms[i].idlocalidad)
			break;
	opener.document.forms[i].idlocalidad.value = idlocalidad ;
	opener.document.forms[i].localidad.value = localidad + ' - CP: ' + cpostal; 
	window.close();
}
 </script>
</head>
...
<table width="100%" border="0" cellspacing="1" cellpadding="1" name="rsTable" id=rsTable cols=<%=totCol+2-4%>  >
  <tr class="fila-encabezado">
   ...
   <tr onMouseOver="mOvr(this,this.className='fila-det-rojo');" onMouseOut="mOut(this,this.className='<%=color_fondo%>');" class="<%=color_fondo%>" scope="col" > 
      <td class="fila-det-border" ><img src="../imagenes/default/gnome_tango/actions/down.png" width="18" height="18" onClick="bajarDatos('<%=sCampos[0]%>', '<%=sCampos[1]%>', '<%=sCampos[2]%>', '<%=sCampos[3]%>', '<%=Common.setNotNull(sCampos[4]).trim()%>' )"></td>
