...
function bajarDatos(idcliente, cliente){
...
<form action="lov_clientesPedidos.jsp" method="POST" name="frm">
...
<table width="100%" border="0" cellspacing="1" cellpadding="1" name="rsTable" id=rsTable cols=<%=totCol+2-4%>  >
  <tr class="fila-encabezado">
     ...
     <td colspan="3 " onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><div align="center"><%=tituCol[1]%></div></td>
     <td width="27%" rowspan="2" valign="bottom" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><%=tituCol[30]%></td>
     <td width="9%" rowspan="2" valign="bottom" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><div align="center">Estado</div></td>
  </tr>
  <tr class="fila-encabezado">
    <td width="4%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);">Baco</td>
    <td width="4%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);">Kosher</td>
    <td width="4%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);">Razón Soc.</td>
    <td width="4%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);">Nro. Documento</td>
    <td width="4%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);">Correo electrónico</td>
  </tr>
  <tr class="fila-encabezado">
    <td >&nbsp;</td>
    <td >&nbsp;</td>
    ...
    <td onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><input name="filtroIdclienteKosher" type="text" value="<%=BGLA.getFiltroIdclienteKosher()%>" id="filtroIdclienteKosher" size="5" maxlength="10" style="text-align:right"  onKeyPress="if(!validaNumericosFF(event)) return false;"></td>
    <td onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><input name="filtroCliente" type="text" value="<%=BGLA.getFiltroCliente()%>" id="filtroCliente" size="25" maxlength="30"></td>
    <td onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><input name="filtroNrodocumento" type="text" value="<%=BGLA.getFiltroNrodocumento()%>" id="filtroNrodocumento" size="9" maxlength="11" style="text-align:right"></td>
    <td onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><input type="text" name="filtroEmail" id="filtroEmail" value="<%=BGLA.getFiltroEmail() %>"/> </td>
   ...
   <tr onMouseOver="mOvr(this,this.className='fila-det-rojo');" onMouseOut="mOut(this,this.className='<%=color_fondo%>');" class="<%=color_fondo%>" scope="col" > 
      ...
      <td class="fila-det-border" ><img src="<%=sCampos[34]%>" title="<%=sCampos[33]%>" ></td>
      <td class="fila-det-border" >
        <div align="center">
        <%if(Common.setNotNull(sCampos[31]).equalsIgnoreCase("S")){ %>
        <img src="../imagenes/default/gnome_tango/actions/mail_replyall.png" width="18" height="18" onClick="abrirVentana('lovClientesRelaciones.jsp?idcliente=<%=sCampos[0]%>&razon=<%=sCampos[1]%>', 'relacioncliente', 750, 350)" title="Seleccionar Otra Razón Social Asociada">
        <%}else out.write("&nbsp;");  %>
        </div>      </td>
      <td class="fila-det-border" ><%=sCampos[0]%>&nbsp;</td>
      <td class="fila-det-border" ><%=Common.setNotNull(sCampos[35])%>&nbsp;</td>
      <td class="fila-det-border"><%=sCampos[1]%>

      </td>
      <td class="fila-det-border"><%=sCampos[4]%>&nbsp;
      </td>
      <td class="fila-det-border"><%=!Common.setNotNull(sCampos[38]).equalsIgnoreCase("") ? sCampos[38] : "No tiene mail"%>&nbsp;</td>     
