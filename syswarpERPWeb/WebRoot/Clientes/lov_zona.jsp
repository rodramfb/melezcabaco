...
%>
<html>
<jsp:useBean id="BFFF"  class="ar.com.syswarp.web.ejb.BeanClientesZonaLov"   scope="page"/>
...
<title><%=titulo%></title>
 ...
 <script >
function bajarDatos(idzona, zona, idexpreso, expreso){
	for(var i = 0; i<10;i++)
		if(opener.document.forms[i].idzona)
			break;
	opener.document.forms[i].idzona.value = idzona;
	opener.document.forms[i].zona.value = zona; 
	if(opener.document.forms[i].idexpreso){
...
<form action="lov_zona.jsp" method="POST" name="frm">
...
<table width="100%" border="0" cellspacing="1" cellpadding="1" name="rsTable" id=rsTable cols=<%=totCol+2-4%>  >
  <tr class="fila-encabezado">
   ...
   <tr onMouseOver="mOvr(this,this.className='fila-det-rojo');" onMouseOut="mOut(this,this.className='<%=color_fondo%>');" class="<%=color_fondo%>" scope="col" > 
	  ...
	  <%if(!sCampos[2].equalsIgnoreCase("0")){  %>
	    <img src="../imagenes/default/gnome_tango/actions/down.png" width="20" height="20" onClick="bajarDatos('<%=sCampos[0]%>', '<%=sCampos[1]%>', '<%=sCampos[2]%>', '<%=sCampos[3]%>' )">
	  <%} else out.write("&nbsp;");  %>
	  </td>
