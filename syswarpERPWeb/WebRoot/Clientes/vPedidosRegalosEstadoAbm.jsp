...
String color_fondo ="";
String titulo = "CONSULTA DE PEDIDOS PARA REGALOS POR ESTADO";
...
%>
<html>
<jsp:useBean id="BVPEA"  class="ar.com.syswarp.web.ejb.BeanVPedidosRegalosEstadoAbm"   scope="page"/>
...
<form action="vPedidosRegalosEstadoAbm.jsp" method="POST" name="frm">
...
<table width="100%" border="0" cellspacing="1" cellpadding="1" name="rsTable" id=rsTable cols=<%=totCol+2-4%>  >
  <tr class="fila-encabezado">
    ...
    <td width="2%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);">&nbsp;</td>
     ...
     <td width="2%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><div align="right"><%=tituCol[12]%></div></td>
     <td width="2%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><div align="center"><%=tituCol[13]%></div></td>
   ...
   <tr onMouseOver="mOvr(this,this.className='fila-det-rojo');" onMouseOut="mOut(this,this.className='<%=color_fondo%>');" class="<%=color_fondo%>" scope="col" >
     <td class="fila-det-border" ><% if(sCampos[1].equals("1") ){ %><img src="../imagenes/default/gnome_tango/apps/text-editor.png" width="20" height="20"  title="Modificar Orden Para Regalos" onClick="modificarOrden(<%=sCampos[0]%>);">  <% }else{ %><img src="../imagenes/default/gnome_tango/actions/gnome-lockscreen.png" width="20" height="20"  title="Orden Para Regalos Ya Procesada, no es posible modificarla.">
      <% } %></td>
     <td class="fila-det-border" ><img src="../imagenes/default/gnome_tango/actions/gtk-refresh.png" width="20" height="20" style="cursor:pointer" title="Actualizar Estado del Pedido Número: <%=sCampos[0]%>" onClick="cambiarEstado(<%=sCampos[0]%>);"></td> 
      <td class="fila-det-border" ><div align="center"><img src="../imagenes/default/gnome_tango/mimetypes/x-office-spreadsheet.png" width="20" height="20" style="cursor:pointer" onClick="callDetalle('<%=sCampos[0]%>', '<%=sCampos[3]%>', '<%=sCampos[4]%>', '<%=sCampos[5]%>')" title="Consultar Detalle del Pedido"></div></td>
      <td class="fila-det-border" ><img src="../imagenes/default/gnome_tango/apps/config-users.png" width="20" height="20" onClick="getAudit(<%=sCampos[0]%>, <%=sCampos[1]%>);" title="Auditoría Cambio de Estado."></td>
      <td class="fila-det-border" ><div align="center"><img src="../imagenes/default/gnome_tango/apps/kwin.png" width="20" height="20"  onClick="abrirVentana('pedidosRemitosPedido.jsp?idpedido=<%=sCampos[0]%>&tipopedido=R', 'remitos', 500, 250 );" title="Ver remitos asociados"></div></td>
      <td class="fila-det-border" ><div align="right" onClick="abrirVentana('../reportes/jasper/generaPDF.jsp?plantillaImpresionJRXML=<%= plantillaImpresionJRXML %>&idpedido_cabe=<%=sCampos[0]%>','pedido',750, 400); "><a href="#"><%=Common.setNotNull(sCampos[0])%></a></div></td>
      <td class="fila-det-border" ><% if(!Common.setNotNull(sCampos[12]).equals("")) {%><div align="right" onClick="abrirVentana('../reportes/jasper/generaPDF.jsp?plantillaImpresionJRXML=<%= plantillaImpresionJRXML %>&idpedido_cabe=<%=sCampos[12]%>','pedido',750, 400); "><a href="#"><%=Common.setNotNull(sCampos[12])%></a></div><% }else { %>&nbsp;<% } %></td>
      <td class="fila-det-border" ><div align="center"><%=Common.setNotNull(sCampos[13]).equalsIgnoreCase("S") ? "SI" : "NO"%></div></td>
...
<%
   ...
   <input name="idpedido_regalos_cabe" value="" type="hidden">
