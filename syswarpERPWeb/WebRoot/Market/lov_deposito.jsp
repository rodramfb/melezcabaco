...
<title><%=titulo%></title>
 ...
 <script >
function bajarDatos(codigo_dt, d_codigo_dt){
	for(var i = 0; i<10;i++)
	  // || opener.document.forms[i].codigo_dt 
		// Agregado EJV 20071214 llamada desde otra pagina
		if(opener.document.forms[i].codigo_dt || opener.document.forms[i].codigo_dt)
			break;
	if(opener.document.forms[i].codigo_dt)	{	
	  opener.document.forms[i].codigo_dt.value = codigo_dt;
	  opener.document.forms[i].d_codigo_dt.value = d_codigo_dt; 
  // Agregado EJV 20071214 llamada desde otra pagina
	}else if(opener.document.forms[i].codigo_dt)	{	
	  opener.document.forms[i].codigo_dt.value = codigo_dt;
	  opener.document.forms[i].descrip_dt.value = d_codigo_dt; 
	}
	window.close();
}
 </script>
...
<form action="lov_deposito.jsp" method="POST" name="frm">
