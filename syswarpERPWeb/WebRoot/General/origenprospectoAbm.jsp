...
/* 
   Grilla para la entidad: origenprospecto
...
String color_fondo ="";
String titulo = "Origen Prospecto";
// variables de entorno
String pathskin = session.getAttribute("pathskin").toString();
String pathscript = session.getAttribute("pathscript").toString();
// variables de paginacion
int i = 0;
Iterator iterOrigenprospecto   = null;
...
%>
<html>
<jsp:useBean id="BOA"  class="ar.com.syswarp.web.ejb.BeanOrigenprospectoAbm"   scope="page"/>
<%-- EJECUTAR SETEO DE PROPIEDADES --%>
<jsp:setProperty name="BOA" property="*" />
<%
 BOA.setResponse(response);
 BOA.setRequest(request);
 BOA.setIdempresa(new BigDecimal(session.getAttribute("empresa").toString()));
 BOA.ejecutarValidacion();
%>
<head>
<title><%=titulo%></title>
...
tituCol[0] = "Codigo";
tituCol[1] = "Origen";
java.util.List Origenprospecto = new java.util.ArrayList();
Origenprospecto= BOA.getOrigenprospectoList();
iterOrigenprospecto = Origenprospecto.iterator();
%>
<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" >
<div id="overDiv" style="position:absolute; visibility:hidden; z-index:1000;"></div>
<form action="origenprospectoAbm.jsp" method="POST" name="frm">
                   ...
                   <td width="89%">
                      <table width="100%" border="0">
                         <tr>
                           ...
                           <td width="22%">
                              <input name="ocurrencia" type="text" value="<%=BOA.getOcurrencia()%>" id="ocurrencia" size="30" maxlength="30">
                           </td>
                           <td width="72%">
                                   ...
                                   <td>
                                     <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                        <tr class="text-globales">
                                          ...
                                          <td width="23%">&nbsp;Total de registros:&nbsp;<%=BOA.getTotalRegistros()%></td>
                                          ...
                                          <td width="11%">
                                             <select name="limit" >
                                                <%for(i=15; i<= 150 ; i+=15){%>
                                                    ...
                                                    <%}
                                                      if( i >= BOA.getTotalRegistros() ) break;
                                                    %>
                                                <%}%>
                                                <option value="<%= BOA.getTotalRegistros()%>">Todos</option>
                                             </select>
                                          </td>
                                          <td width="7%">&nbsp;P&aacute;gina:</td>
                                          <td width="12%">
                                             <select name="paginaSeleccion" >
                                                <%for(i=1; i<= BOA.getTotalPaginas(); i++){%>
                                                    <%if ( i==BOA.getPaginaSeleccion() ){%>
                                                       <option value="<%=i%>" selected><%=i%></option> 
                                                    <%}else{%>
                                                       <option value="<%=i%>"><%=i%></option>
                                                    <%}%>
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  <tr >
   <td class="fila-det-bold-rojo"><jsp:getProperty name="BOA" property="mensaje"/></td>
  </tr>
</table>
<table width="100%" border="0" cellspacing="1" cellpadding="1" name="rsTable" id=rsTable cols=<%=totCol+2-4%>  >
  <tr class="fila-encabezado">
   ...
   <%int r = 0;
   while(iterOrigenprospecto.hasNext()){
      ++r;
      String[] sCampos = (String[]) iterOrigenprospecto.next(); 
   ...
   <tr onMouseOver="mOvr(this,this.className='fila-det-bold-rojo');" onMouseOut="mOut(this,this.className='<%=color_fondo%>');" class="<%=color_fondo%>" scope="col" > 
      <td class="fila-det-border" ><input type="radio" name="idorigen" value="<%= sCampos[0]%>"></td>
