...
function validarCampos() {
	...
	if (document.forms[0].cContable2.value == "") {
		alert("Debe ingresarse una cuenta contable a buscar");	
		return false;
	}
	if (document.forms[0].cContable2.value != "") {
	}
...
function passBack(cContable, d_cContable) {  
  ...
  if(opcion == "byId"){
    ...
    opener.document.getElementById(id).value = d_cContable ;		
		...
		var i = 0;
		 for(i=0;i<10;i++)
			 if(opener.document.forms[i].cuenve2_st)
				 break;    
		 opener.document.forms[i].cuenve2_st.value = cContable; 
	}
   close();
}
...
<hr>
<form action="lov_cuentascontables_ventas2.jsp">
  <table width="59%" border="0" cellspacing="0" cellpadding="0" height="23">
    <tr> 
      ...
      <td width="54%" height="39"> 
        <input name="cContable2" type="text" class="campo" value="<%=cContable2%>" size="25">
      </td>
...
if (!cContable2.equals("*")) {
   ...
   try{
   	...
   	cuentasContables =  repo.getCuentasOcu(ejercicioActivo, cContable2, new BigDecimal( session.getAttribute("empresa").toString() ));
