...
%>
<html>
<jsp:useBean id="BCIA"  class="ar.com.syswarp.web.ejb.BeanLovCajaChequesPendientes"   scope="page"/>
<%-- EJECUTAR SETEO DE PROPIEDADES --%>
<jsp:setProperty name="BCIA" property="*" />
<%
 String titulo = "TIPO DE CHEQUES: " + BCIA.getTipodeposito();
...
<form action="lov_cajaChequesPendientes.jsp" method="POST" name="frm">
...
<table width="100%" border="0" cellspacing="1" cellpadding="1" name="rsTable" id=rsTable cols=<%=totCol+2-4%>  >
  <tr class="fila-encabezado">
     ...
     <td width="51%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><%=tituCol[3]%></td>
...
&nbsp;
     <% 
  Hashtable htChqDepositar = (Hashtable) session.getAttribute("htChqDepositar");
	if(htChqDepositar != null && !htChqDepositar.isEmpty()){
	   %>
   ...
   <table width="95%"  border="0" align="center" cellpadding="0" cellspacing="1">
     ...
     <%   
	   			Enumeration en = htChqDepositar.keys();
					int element = 0;
					while (en.hasMoreElements()) {
						String key = (String) en.nextElement();
						String [] identificador = (String []) htChqDepositar.get(key); 
     ...
     <tr class="fila-det">
       ...
       <td class="fila-det-border">&nbsp;
				 <input name="chequenro" type="text" value="<%= identificador[30] %>" class="campo" id="chequenro" size="10" maxlength="10" readonly<%//= str.esNulo( identificador[6] ).equalsIgnoreCase("C") ? "": "readonly"%>></td>

       <td class="fila-det-border">&nbsp;
				 <input name="detalle" type="text" value="<%= identificador[29] %>" class="campo" id="detalle" size="30" maxlength="50" readonly <%//= str.esNulo( identificador[6] ).equalsIgnoreCase("C") || str.esNulo( identificador[6] ).equalsIgnoreCase("C") ? "": "readonly"%>></td>
   ...
   <p>
     <input name="accion" value="" type="hidden">
      <input name="cartera_mt" value="<%= BCIA.getCartera_mt()%>" type="hidden">
	    ...
	    <input name="tipodeposito" value="<%= BCIA.getTipodeposito()%>" type="hidden">		
	    <input name="banco" value="<%= BCIA.getBanco()%>" type="hidden">	
   </p>
   <%  
			if(BCIA.getAccion().equalsIgnoreCase("confirmar")  
			   && BCIA.getMensaje().equals("") ){%>
			<script>
			  opener.document.frm.importeDeposito.value = '<%= BCIA.getTotalDeposito() %>' ; 
