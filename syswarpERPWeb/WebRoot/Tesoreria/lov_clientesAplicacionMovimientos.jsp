...
%>
<html>
<jsp:useBean id="BCMCA"  class="ar.com.syswarp.web.ejb.BeanLovClientesMovCliAplicacionesAbm"   scope="page"/>
...
<title><%=titulo%></title>
 ...
 <script>
	...
	function generarAdelanto(comprobante){
		if(confirm("Confirma ingreso de Adelanto ?")){
				document.frm.accion.value = "adelanto";
				agregarComprobante(comprobante,"adelanto");
				//document.frm.submit();
		}
	}

  function agregarComprobante(comprobante,accion){
...
<form action="lov_clientesAplicacionMovimientos.jsp" method="POST" name="frm">
...
<table width="100%" border="0" cellspacing="1" cellpadding="1" name="rsTable" id=rsTable cols=<%=totCol+2-4%>  >
  <tr class="fila-encabezado">
     ...
     <td width="9%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"></td>
    </tr>
   <%int r = 0;
   ...
   <tr onMouseOver="mOvr(this,this.className='fila-det-rojo');" onMouseOut="mOut(this,this.className='<%=color_fondo%>');" class="<%=color_fondo%>" scope="col" > 
      <td class="fila-det-border" ><div align="center">
        <img src="../imagenes/default/nuevo.gif" width="16" height="16" onClick="agregarComprobante('<%= sCampos[2] + "-" + sCampos[3] + "-" + sCampos[6]  %>','agregar');">
      </div></td>
      ...
      <td class="fila-det-border" ><input name="adelanto" type="button" class="boton" id="adelanto" value="Adelanto >>" onclick="generarAdelanto('<%= sCampos[2] + "-" + sCampos[3] + "-" + sCampos[6]  %>');" > </td>
    </tr>
<%
   }%>
  </table>
  ...
  Hashtable htComprobantes = (Hashtable) session.getAttribute("htComprobantes");
	if(htComprobantes != null && !htComprobantes.isEmpty()){
	   %> 		 
   <table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0">
     <tr class="text-globales">
       <td width="3%">Sel</td>
       <td width="21%">Nro. Interno </td>
       <td width="29%">Comprobante</td>
       <td width="23%"><div align="center">Importe Original </div></td>
       <td width="24%"><div align="center">A Cobrar </div></td>
     </tr>
		<%
	   			Enumeration en = htComprobantes.keys();
					while (en.hasMoreElements()) {
						String key = (String) en.nextElement();
						String [] comprobantes = (String []) htComprobantes.get(key);
     ...
     <tr class="fila-det">
       ...
       <td class="fila-det-border"> &nbsp;<%= comprobantes[0] %>
       <input  name="keyHashDatosComprobante" type="hidden" value="<%= key %>"></td>
       <td class="fila-det-border">&nbsp;<%= key.equalsIgnoreCase("ADELANTO") ? key :comp[2] + "-" + Common.strZero( comp[0], 4) + "-" + Common.strZero( comp[1], 8)  %></td> 
       <td class="fila-det-border">
         <div align="right"><%= comprobantes[2] %>&nbsp;</div></td>
       <td class="fila-det-border"><div align="center">
         <input name="totalcobrar" type="text" value="<%= comprobantes[1] %>" class="campo" size="10" maxlength="15" style="text-align:right">
       </div></td>
     </tr>
		<%
	        }%>		 
