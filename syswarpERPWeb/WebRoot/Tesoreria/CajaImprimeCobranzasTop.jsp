...
String color_fondo ="";
String titulo = "Comprobante de Cobranzas ";
...
%>
<html>
<jsp:useBean id="BVCA"  class="ar.com.syswarp.web.ejb.BeanCajaCobranzasImpresion"   scope="page"/>
...
<title><%=titulo%></title>
 ...
 function seleccionarCliente(){
   abrirVentana('vlov_ClientesAbm.jsp', 'clientes', 700, 400); 
 }
...
<table width="90%"  border="0" align="center" cellpadding="0" cellspacing="0">
	...
	if(htComprobantesOK != null && !htComprobantesOK.isEmpty()){
		...
		while (en.hasMoreElements()) {
			...
			String [] comp = key.split("-");
						  %>
      <tr class="fila-det" >
        <td width="44%" > <%= key.equalsIgnoreCase("ADELANTO") ? key :comp[2] + "-" + Common.strZero( comp[0], 4) + "-" + Common.strZero( comp[1], 8)  %>&nbsp;</td>
        <td width="32%" ><%= comprobantes[3].equals("") ? "" : Common.setObjectToStrOrTime( Timestamp.valueOf(comprobantes[3]), "JSTsToStr") %>&nbsp;</td>
        <td width="24%" >
	...
	if(htIdentificadoresOK != null && !htIdentificadoresOK.isEmpty()){
		...
		while (en.hasMoreElements()) {
			...
			String [] identificadores = (String []) htIdentificadoresOK.get(key);
						  %>
      <tr class="fila-det" >
        <td width="44%"><%= identificadores[0] %> - <%= identificadores[1] %>&nbsp;</td>
        <td width="32%">&nbsp;</td>
        <td width="24%"><div align="right">-<%= identificadores[8] %> </div> </td>
      </tr>
      <% 
  ...
  <tr class="subtitulo">
    <td height="18" colspan="2">Liquidado de la Siguiente Forma 
		</td>
    <td height="18" colspan="2"></td>
  </tr>
  <tr class="fila-det">
	...
	if(htIdentificadoresIngresosOK != null && !htIdentificadoresIngresosOK.isEmpty()){
		...
		while (en.hasMoreElements()) {
			...
			String [] identificadores = (String []) htIdentificadoresIngresosOK.get(key);
						  %>
      <tr class="fila-det" >
        <td width="22%" > <%= BVCA.getDescripcionIdentificador(key)%> &nbsp;</td>
        <td width="14%" ><%= identificadores[30] %>&nbsp;</td>
        <td width="15%" ><%= identificadores[31] %>&nbsp;</td>
        <td width="23%" >&nbsp;</td>
