...
<html>
	<%-- INSTANCIAR BEAN --%>
	<jsp:useBean id="BPF"
		class="ar.com.syswarp.web.ejb.BeanProveedoresInformeComprasProveedores" scope="page" />
	<head>
	...
	<BODY>
		...
		<% 
 String titulo = "Consulta de Informe de Proveedores.";
 ...
 %>
	<form action="proveedoConsultaInformeProveedores.jsp" method="post" name="frm">
...
<%if (BPF.getAccion().equalsIgnoreCase("consulta")){%>		
   ...
   while( iterConsulta1.hasNext() ) {  
	  ...
	  if ( !curProveedor.equalsIgnoreCase(sCampos[0]) ){ // solamente aca imprimo todos los titulos de cabecera
	     if (!esPrimero ){ // subtotalizo
         %>
          <tr class="fila-det">
	      <td class="fila-det-border"  div align="right"><div align="left">Total Importe &nbsp;<%=total%></div></td> 
          </tr>
         <%		 
   ...
   <tr onMouseOver="mOvr(this,this.className='fila-det-rojo');" onMouseOut="mOut(this,this.className='<%=color_fondo%>');" class="<%=color_fondo%>"        scope="col" >
    ...
    <%
	 total = total.add(new BigDecimal(sCampos[6]));  
	 totalGeneral = totalGeneral.add(new BigDecimal(sCampos[6])); 	 
     esPrimero=false;
	}  
...
<tr class="fila-det">
	      <td class="fila-det-border"  div align="right"><div align="left">Total General &nbsp;<%=totalGeneral%></div></td> 
          </tr>
