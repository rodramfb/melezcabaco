...
public class BeanLovArticulosListaPreciosPedidos implements SessionBean,
		Serializable {
	static Logger log = Logger
			.getLogger(BeanLovArticulosListaPreciosPedidos.class);
	...
	public BeanLovArticulosListaPreciosPedidos() {
		super();
	}
	...
	public boolean ejecutarValidacion() {
		...
		this.conExistencia = this.isPrimeraCarga() ? "S" : this.conExistencia;
		...
		String filtro = " WHERE idlista = "
				+ idlista.toString()
				+ "                AND codigo_dt NOT IN (SELECT codigo_dt FROM stockdepositoslockregalos WHERE current_date BETWEEN fechadesde AND fechahasta  AND fechabaja IS NULL  AND idempresa = "
				+ idempresa
				+ ") "
		...
		try {
			...
			if (buscar && accion.equals("")) {
				...
				this.totalOcurrenciaEnEsquema = clientes.getTotalEntidadFiltro(
						entidadEsquemas, " WHERE " + filtroAuxiliar,
						this.idempresa);
				...
				this.proveedoArticulosList = clientes
						.getArtListaPreciosPedidosOcu(this.limit, this.offset,
								this.ocurrencia, this.idlista, this.idzona,
				...
				this.proveedoArticulosList = clientes
						.getArtListaPreciosPedidosAll(this.limit, this.offset,
								this.idlista, this.idzona, this.conExistencia,
