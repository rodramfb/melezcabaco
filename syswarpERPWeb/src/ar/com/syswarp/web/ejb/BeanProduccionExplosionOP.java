...
public class BeanProduccionExplosionOP implements SessionBean, Serializable {
	static Logger log = Logger.getLogger(BeanProduccionExplosionOP.class);
	...
	public BeanProduccionExplosionOP() {
		super();
	}
	...
	public boolean ejecutarValidacion() {
		...
		String filtro = " " +
				...
				+ "  AND idop NOT IN (SELECT idop FROM interfacesopregalos WHERE idempresa = "
				+ idempresa + ")  AND idpedido_regalos_cabe IS NULL ";
		int i = 0;
		try {
			...
			if (!this.explosion.equalsIgnoreCase("")) {
				...
				if (mensaje.equals("")) {
					this.mensaje = produccionMovProdu
							.produccionGenerarExplosionOP(this.idop,
			...
			if (buscar) {
				...
				this.produccionMovProduList = produccionMovProdu
						.getProduccionMovProduPendientesOcu(this.limit,
				...
				this.produccionMovProduList = produccionMovProdu
						.getProduccionMovProduPendientesAll(this.limit,
