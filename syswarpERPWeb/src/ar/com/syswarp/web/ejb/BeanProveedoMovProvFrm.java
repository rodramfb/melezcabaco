...
public class BeanProveedoMovProvFrm implements SessionBean, Serializable {
	private SessionContext context;

	private BigDecimal idempresa = new BigDecimal(-1);

	static Logger log = Logger.getLogger(BeanProveedoMovProvFrm.class);
	...
	private List listaCondiciones = new ArrayList();

	private BigDecimal idCondPago = new BigDecimal(-1);

	// <------------------------------------------

	public BeanProveedoMovProvFrm() {
		super();
	}
	...
	public void ejecutarSentenciaDML() {
		...
		try {
			...
			if (this.accion.equalsIgnoreCase("alta")) {
				...
				dispatcher = request
						.getRequestDispatcher("proveedoMovProvAsientoFrm.jsp");
	...
	public void getDatosProveedoMovProv() {
		try {
			...
			if (iterProveedoMovProv.hasNext()) {
				...
				if (idCondPago.longValue() > 0) {
					this.idcondicionpago = idCondPago;
				}
				// <-------------------------------------------
	...
	public boolean ejecutarValidacion() {
		...
		try {
			...
			// 20110811 - CAMI - Condiciones de pago ---------->
			Proveedores prov = Common.getProveedores();
			listaCondiciones = prov.getProveedoCondicioAll(250, 0,
					this.idempresa);
			// <-----------------------------------------------
			...
			if (!this.validar.equalsIgnoreCase("")) {
				if (!this.accion.equalsIgnoreCase("baja")) {
					...
					if (idcondicionpago.longValue() < 0) {
					...
					if (this.observacionesContables.trim().length() == 0) {
						this.mensaje = "No se puede dejar vacia la observacion contable.  ";
						return false;
					}
					...
					if (!Common.esNumerico(this.provPorcEspecialIvaPercep))
					{
						this.mensaje = "La percepcion de I.V.A. debe ser numerica";
						return false;
					}
					if (new BigDecimal(this.provPorcEspecialIvaPercep).longValue()<0)
					{
						this.mensaje = "La percepcion de I.V.A. no puede ser menor a cero.";
						return false;
					}
	...
	// 20110811 - CAMI - Condicion de pago ------>
	public List getListaCondiciones() {
		return listaCondiciones;
	}

	public void setListaCondiciones(List listaCondiciones) {
