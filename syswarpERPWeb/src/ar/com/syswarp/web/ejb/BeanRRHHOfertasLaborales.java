...
public class BeanRRHHOfertasLaborales implements SessionBean, Serializable {
	static Logger log = Logger.getLogger(BeanRRHHOfertasLaborales.class);
	...
	public BeanRRHHOfertasLaborales() {
		super();
	}
	...
	public boolean ejecutarValidacion() {
		...
		try {
			...
			if (this.accion.equalsIgnoreCase("postular")) {
				dispatcher = request
			...
			String filtro = " WHERE current_date BETWEEN fechabusquedadesde AND fechabusquedahasta ";

			if (buscar) {

				String filtroOcu = " AND UPPER(referencia) LIKE '"
						+ ocurrencia.toUpperCase() + "%' OR  UPPER(descripciontarea) LIKE '"
						+ ocurrencia.toUpperCase() + "%'  " ;
				filtro += filtroOcu;

				this.totalRegistros = RRHHbusquedasLaborales
						.getTotalEntidadFiltro("RRHHbusquedasLaborales",
				...
				this.RRHHbusquedasLaboralesList = RRHHbusquedasLaborales
						.getRRHHbusquedasLaboralesActivasOcu(filtroOcu,
								this.idempresa);
			} else {
				this.totalRegistros = RRHHbusquedasLaborales
						.getTotalEntidadFiltro("RRHHbusquedasLaborales",
				...
				this.RRHHbusquedasLaboralesList = RRHHbusquedasLaborales
						.getRRHHbusquedasLaboralesActivas(this.idempresa);
			}
