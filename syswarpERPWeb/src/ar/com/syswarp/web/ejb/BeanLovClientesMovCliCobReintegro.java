...
public class BeanLovClientesMovCliCobReintegro implements SessionBean,
		Serializable {
	static Logger log = Logger
			.getLogger(BeanLovClientesMovCliCobReintegro.class);
	...
	public BeanLovClientesMovCliCobReintegro() {
		super();
	}
	...
	public boolean ejecutarValidacion() {
		...
		Hashtable htComprobantesCobCli = (Hashtable) session
				.getAttribute("htComprobantesCobCli") != null ? (Hashtable) session
				.getAttribute("htComprobantesCobCli")
				: (Hashtable) session.getAttribute("htComprobantesCobCliOK");

		try {

			String filtro = "WHERE cliente =  "
					+ this.cliente
					+ " AND tipomovs = 'COB' AND (anulada = 'N' OR anulada IS NULL) ";
			...
			if (buscar) {
				...
				this.clientesMovCliList = clientesMovCli
						.getLovClientesMovCliCobReintegroAll(this.limit,
								this.offset, this.cliente, this.idempresa);
			}
			...
			if (this.accion.equalsIgnoreCase("eliminar")) {
				if (this.delKey == null) {
					...
					for (int i = 0; i < this.delKey.length; i++) {
						if (htComprobantesCobCli.containsKey(this.delKey[i])) {
							htComprobantesCobCli.remove(this.delKey[i]);
						}
					}
			...
			if (cliente == null || cliente.longValue() < 0) {
				...
				if (this.accion.equalsIgnoreCase("agregar")) {
					...
					if (!this.comprobante.equals("")) {
						...
						List lista = clientesMovCli
								.getLovClientesMovCliCobReintegroOne(cliente,
										new BigDecimal(datosComp[0]),
										new BigDecimal(datosComp[1]),
										datosComp[2], this.idempresa);
						...
						if (lista.size() > 0) {
							...
							htComprobantesCobCli.put("COBRANZA", datos);
				...
				if (this.accion.equalsIgnoreCase("adelanto")) {
					...
					htComprobantesCobCli.put("ADELANTO", datos);
				}
			...
			if (htComprobantesCobCli == null)
				log.info("htComprobantesCobCli.size: "
						+ htComprobantesCobCli.size());

			this.asignarValidar(htComprobantesCobCli);
	...
	private boolean asignarValidar(Hashtable ht) {
		...
		try {
			...
			if ((this.accion.equalsIgnoreCase("confirmar") || (ht.isEmpty() && this.accion
				...
				log.info("CONFIRMAR: " + ht);

				Hashtable htConfirmar = new Hashtable();
				Common.htRellenar(ht, htConfirmar);

				session.setAttribute("htComprobantesCobCliOK", htConfirmar);
				session.removeAttribute("htComprobantesCobCli");

			} else {
				log.info("session.setAttribute(htComprobantesCobCli, ht)");
				if (!mensaje.equals(""))
					this.mensaje = mensaje;
				// he aqui
				session.setAttribute("htComprobantesCobCli", ht);
			}
