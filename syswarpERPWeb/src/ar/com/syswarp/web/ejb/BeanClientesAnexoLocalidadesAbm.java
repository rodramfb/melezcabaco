...
public class BeanClientesAnexoLocalidadesAbm implements SessionBean,
		Serializable {
	static Logger log = Logger.getLogger(BeanClientesAnexoLocalidadesAbm.class);
	...
	public BeanClientesAnexoLocalidadesAbm() {
		super();
	}
		...
		try {
			...
			if (buscar) {
				String entidad = ""
						...
						+ "       al.idempresa,al.usuarioalt,al.usuarioact,al.fechaalt,al.fechaact,l.cpostal"
				...
				this.clientesAnexoLocalidadesList = clientesAnexoLocalidades
						.getClientesAnexoLocalidadesOcu(this.limit,
								this.offset, this.ocurrencia, this.idempresa);
			} else {
				this.totalRegistros = clientesAnexoLocalidades.getTotalEntidad(
						"clientesAnexoLocalidades", this.idempresa);
				this.totalPaginas = (this.totalRegistros / this.limit) + 1;
				if (this.totalPaginas < this.paginaSeleccion)
					this.paginaSeleccion = this.totalPaginas;
				this.offset = (this.paginaSeleccion - 1) * this.limit;
				...
				this.clientesAnexoLocalidadesList = clientesAnexoLocalidades
						.getClientesAnexoLocalidadesAll(this.limit,
