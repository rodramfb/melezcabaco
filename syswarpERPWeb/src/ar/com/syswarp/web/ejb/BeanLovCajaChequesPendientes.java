...
public class BeanLovCajaChequesPendientes implements SessionBean, Serializable {
	static Logger log = Logger.getLogger(BeanLovCajaChequesPendientes.class);
	...
	private BigDecimal totalDeposito = new BigDecimal(0);

	private String banco = "";

	public BeanLovCajaChequesPendientes() {
		super();
	}
	...
	public boolean ejecutarValidacion() {
		...
		Hashtable htChqDepositar = (Hashtable) session
				.getAttribute("htChqDepositar");

		if (htChqDepositar == null) {
			htChqDepositar = new Hashtable();
			Common.htRellenar((Hashtable) session
					.getAttribute("htChqDepositarOK"), htChqDepositar);
		}
		...
		try {
			...
			if (this.tipodeposito.equalsIgnoreCase("CARTERA")) {

				operador = "<=";
				filtroExtra = " AND usado_mt IS NULL ";
				filtro = " WHERE usado_mt IS NULL  AND cartera_mt =  '"
						+ this.cartera_mt + "'   AND tipcart_mt = '"
						+ this.tipcart_mt + "'     AND fecha_mt::DATE " + operador
						+ "  '" + this.fecha_mt + "'::DATE ";

			} else if (this.tipodeposito.equalsIgnoreCase("CAUCIONADOS")) {
				filtroExtra = " AND usado_mt = 'U' AND movsal_mt =  '"
						+ this.banco + "' ";

				operador = "<=";
				filtro = " WHERE usado_mt = 'U' AND movsal_mt =  '"
						+ this.banco + "'   AND cartera_mt = '"
						+ this.cartera_mt + "'   AND tipcart_mt = '"
						+ this.tipcart_mt + "'     AND fecha_mt::DATE " + operador
						+ "  '" + this.fecha_mt + "'::DATE ";

			} else if (this.tipodeposito.equalsIgnoreCase("CAUCIONAR")) {
			...
			if (buscar) {
				...
				this.totalRegistros = cajaIdentificadores
						.getTotalEntidadFiltro("cajamovteso", filtro  + filtroExtra,
								this.idempresa);
				...
				this.cajaIdentificadoresList = cajaIdentificadores
						.getLovChequesPendientesOcu(this.limit, this.offset,
								this.ocurrencia, this.cartera_mt,
								this.tipcart_mt, this.fecha_mt, filtroExtra,
								operador, this.idempresa);
			} else {
				...
				this.cajaIdentificadoresList = cajaIdentificadores
						.getLovChequesPendientesAll(this.limit, this.offset,
								this.cartera_mt, this.tipcart_mt,
								this.fecha_mt, filtroExtra, operador,
								this.idempresa);
			}
				...
				if (!this.idmovteso.equals("0")) {
					...
					if (lista.size() > 0) {
						...
						if (datos[6].equalsIgnoreCase("T")
							...
							if (htChqDepositar.containsKey(clave))
								this.reasigna = true;
							htChqDepositar.put(clave, datos);

						} else {

							htChqDepositar
									.put(this.idmovteso.toString(), datos);
						}
			...
			if (this.accion.equalsIgnoreCase("eliminar")) {
				if (this.delKey == null) {
					...
					for (int i = 0; i < this.delKey.length; i++) {
						if (htChqDepositar.containsKey(this.delKey[i])) {
							htChqDepositar.remove(this.delKey[i]);
						}
					}
				}
			}

			this.asignarValidar(htChqDepositar);
	...
	private boolean asignarValidar(Hashtable ht) {
		...
		try {
				...
				for (int i = 0; i < this.keyHashDatosIdentificador.length; i++) {
					if (ht.containsKey(this.keyHashDatosIdentificador[i])) {
						...
						if (fOk) {
							...
							totalDeposito = totalDeposito.add(new BigDecimal(
									datos[36]));
							...
							datos[32] = this.clearing[i];
							// datos[33] = this.cuotas[i];
							this.totalDeposito = new BigDecimal(0);
			...
			if ((this.accion.equalsIgnoreCase("confirmar") || (ht.isEmpty() && this.accion
				...
				session.setAttribute("htChqDepositarOK", ht);
				session.removeAttribute("htChqDepositar");

			} else {
				if (!mensaje.equals(""))
					this.mensaje = mensaje;
				session.setAttribute("htChqDepositar", ht);
			}
