...
public class BeanPedidosRegalosDetalleEntrega implements SessionBean,
		Serializable {
	static Logger log = Logger
			.getLogger(BeanPedidosRegalosDetalleEntrega.class);
	...
	public BeanPedidosRegalosDetalleEntrega() {
		super();
	}
	...
	public boolean ejecutarValidacion() {
		...
		String filtro = " WHERE idpedido_regalos_entrega_cabe = "
				+ this.idpedido_cabe.toString();

		String entidad = "pedidos_regalos_entregas_cabe";
		...
		try {
			...
			this.pedidos_CabeList = pedidos_Deta
					.getPedidosRegalosDetalleEntregaPK(this.idpedido_cabe,
			...
			if (this.pedidos_CabeList != null
				...
				/*
				 * pc.idpedido_regalos_cabe, es.estado, pc.idcliente,
				 * pc.idsucursal, cd.calle, cd.nro , cd.piso, cd.depto, cd.cpa,
				 * cd.postal, cd.contacto, gl.localidad, gp.provincia,
				 * pc.fechapedido, ex.expreso, pc.obsarmado, pc.obsentrega,
				 * pr.prioridad, z.zona, pc.idempresa, pc.usuarioalt,
				 * pc.usuarioact, pc.fechaalt, pc.fechaact FROM
				 * pedidos_regalos_entregas_cabe pc
				 */

				this.idpedido_regalos_cabe = new BigDecimal(datosCabecera[0]);
				...
				this.expreso = datosCabecera[14];
				// this.comision = datosCabecera[17];
				// this.ordencompra = datosCabecera[18];
				this.obsarmado = datosCabecera[15];
				this.obsentrega = datosCabecera[16];
				...
				this.prioridad = datosCabecera[17];
				this.zona = datosCabecera[18];
			...
			this.pedidos_DetaList = pedidos_Deta
					.getPedidosRegalosDetalleEntrega(this.idpedido_cabe,
							this.idempresa);
