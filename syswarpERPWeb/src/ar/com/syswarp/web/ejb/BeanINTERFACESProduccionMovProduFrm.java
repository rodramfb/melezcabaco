...
public class BeanINTERFACESProduccionMovProduFrm implements SessionBean, Serializable {
	private SessionContext context;

	private BigDecimal idempresa = new BigDecimal(-1);

	static Logger log = Logger.getLogger(BeanINTERFACESProduccionMovProduFrm.class);
	...
	public BeanINTERFACESProduccionMovProduFrm() {
		super();
	}
	...
	public void ejecutarSentenciaDML() {
		try {
			...
			if (this.accion.equalsIgnoreCase("alta")) {
				...
				this.observaciones = "[INTERFACES]" + this.observaciones; 
				
				this.mensaje = bc.intfOrdenProdCreate(
				...
				this.mensaje = bc.intfOrdenProdUpdate(
	...
	public void getDatosProduccionMovProdu() {
		try {
			BC bc = Common.getBc();
			List listProduccionMovProdu = bc
					.getINTFMovProduPK(this.idop, this.idempresa);
			...
			listaDetalleOrden = bc
					.getINTFMovProdu_DetaPK(this.idop, this.idempresa);
	...
	public boolean ejecutarValidacion() {
		try {
			if (!this.volver.equalsIgnoreCase("")) {
				response.sendRedirect("ordenesProdABM.jsp");
				return true;
			}
