/* 
   javabean para la entidad: clientesIndicadoresClientes
   Copyrigth(r) sysWarp S.R.L. 
   Fecha de creacion: Fri Apr 30 15:18:45 GMT-03:00 2010 
   
   Para manejar la pagina: clientesIndicadoresClientesAbm.jsp
...
public class BeanClientesIndicadoresClientesAbm implements SessionBean,
		Serializable {
	static Logger log = Logger
			.getLogger(BeanClientesIndicadoresClientesAbm.class);
	...
	public BeanClientesIndicadoresClientesAbm() {
		super();
	}
		...
		try {
			...
			if (this.accion.equalsIgnoreCase("modificacion")) {
				if (iddato.longValue() < 0) {
					...
					dispatcher = request
							.getRequestDispatcher("clientesIndicadoresClientesFrm.jsp");
			...
			if (buscar) {
				...
				this.totalRegistros = clientes.getTotalEntidadOcu(
						"clientesIndicadoresClientes", campos, this.ocurrencia,
						this.idempresa);
				...
				this.clientesIndicadoresClientesList = clientes
						.getClientesIndicadoresClientesOcu(this.limit,
								this.offset, this.ocurrencia, this.idempresa);
			} else {
				this.totalRegistros = clientes.getTotalEntidad(
						"clientesIndicadoresClientes", this.idempresa);
				this.totalPaginas = (this.totalRegistros / this.limit) + 1;
				if (this.totalPaginas < this.paginaSeleccion)
					this.paginaSeleccion = this.totalPaginas;
				this.offset = (this.paginaSeleccion - 1) * this.limit;
				...
				this.clientesIndicadoresClientesList = clientes
						.getClientesIndicadoresClientesAll(this.limit,
