...
public class BeanClientesRemitosAsignarHojaArmadoXZona implements SessionBean,
		Serializable {
	static Logger log = Logger
			.getLogger(BeanClientesRemitosAsignarHojaArmadoXZona.class);
	...
	private String[] auxRemito = new String[] {};
	...
	private Hashtable htSelecccion = new Hashtable();

	public BeanClientesRemitosAsignarHojaArmadoXZona() {
		super();
	}
	...
	public boolean ejecutarValidacion() {
		...
		try {
			...
			if (this.zonaUsuario.longValue() < 0) {
				...
				this.mensaje = "Usuario no tiene zona asignada. Por favor comuniquese con Sistemas.";
				return false;

			} else if (this.zonaUsuario.longValue() > 0) {// -->

				this.listExpresosZonas = remitoscliente
						.getClientesExpresosZonasXZona(500, 0,
								this.zonaUsuario, this.idempresa);

				filtro = " WHERE tipopedido =  '" + this.tipopedido
						+ "'  AND idzona = " + this.zonaUsuario;

				this.depositosList = Common.getStock().getStockdepositosAll(
						500, 0, this.idempresa);

				if (this.filtroExpresoZona.equalsIgnoreCase("")) {

					this.mensaje = "Es necesario seleccionar Expreso para iniciar selección";
					return false;

				} else if (this.filtroDepOrigen.equalsIgnoreCase("")) {

					this.mensaje = "Es necesario seleccionar Depósito Origen para iniciar selección";
					return false;

				} else if (!this.validarPeriodo()) {

					return false;

				} else {

					this.llenarHTSeleccion();
					this.generaMostrarTotales();

					if (this.accion.equalsIgnoreCase("asignarhojaarmado")) {
						String[] resultado = new String[] { "", "", "" };
						Map parameters = new HashMap();
						// TODO: 20090312 / EJV - Pendiente de asignar el valor
						// para
						// plantillaImpresionJRXML desde la entidad
						// clientesexpresos.
						// Para poder llevar a cabo dicha asignacion es
						// necesario
						// que al
						// momento de generar la hoja de armado se este
						// filtrando
						// para
						// un solo EXPRESO.
						...
						if (this.filtroDepOrigen.equals("")
							...
							resultado[0] = "Es necesario filtrar por Expreso/Zona y Depósito Origen.";
					...
					if (!auxFiltro.equals("")) {
						...
						this.totalRegistros = remitoscliente
						...
						this.pedidos_cabeList = remitoscliente
								.getClientesRemitosPendientesHAXZonaAll(
										this.limit, this.offset,
										this.zonaUsuario, this.idestado,
										this.tipopedido, this.sort,
										this.idempresa);
					}
	...
	public String generaMostrarTotales() {

		String salida = "";
		String selectIN = "";
		List listTotales = new ArrayList();
		Iterator it;
		...
		try {
			...
			if (this.idremitocliente != null) {
				...
				for (int j = 0; j < this.idremitocliente.length; j++) {

					selectIN += this.idremitocliente[j] + ", ";

				}

				if (!selectIN.equals("")) {

					selectIN = selectIN.substring(0, selectIN.lastIndexOf(','));
					listTotales = Common.getClientes()
							.getClientesRemitosHATotalesFamila(selectIN,
									this.idempresa);

					it = listTotales.iterator();
					salida = "<table border='1' cellpadding='0' cellspacing='0' width='100%'>";
					while (it.hasNext()) {

						String[] datos = (String[]) it.next();
						salida += "<tr>";
						salida += salida = "<td ><font size='1'>" + datos[0] + " </font></td><td > <font size='1'>"
								+ datos[1] + "</font> </td><td > <font size='1'>" + datos[2] + "</font></td>";
						salida += "</tr>";
			...
			log.error("generaMostrarTotales(): " + e);
		}

		return salida;
	}

	private void llenarHTSeleccion() {

		try {

			if (this.idremitocliente != null) {
				for (int j = 0; j < this.idremitocliente.length; j++) {
					this.htSelecccion.put(this.idremitocliente[j],
							this.idremitocliente[j]);
				}
			}

		} catch (Exception e) {
			log.error("llenarHTSeleccion(): " + e);
		}
	...
	private boolean validarPeriodo() {
		...
		long anoActual = Calendar.getInstance().get(Calendar.YEAR);
		long mesActual = Calendar.getInstance().get(Calendar.MONTH);

		if (!Common.esEntero(this.filtroMes)) {
			// mes no numerico
			this.mensaje = "Es necesario seleccionar período valido: Mes incorrecto, no numerico";
			retorno = false;
		} else if (Long.parseLong(this.filtroMes) < 1
				|| Long.parseLong(this.filtroMes) > 12) {
			// mes fuera de rango
			this.mensaje = "Es necesario seleccionar período valido: Año incorrecto, fuera de rango 01 - 12.";
			retorno = false;
		} else if (!Common.esEntero(this.filtroAno)) {
			// ano no numerico
			this.mensaje = "Es necesario seleccionar período valido: Año incorrecto, no numerico";
			retorno = false;
		} else if (Long.parseLong(this.filtroAno) < (anoActual - 1)
				|| Long.parseLong(this.filtroAno) > (anoActual + 1)) {
			// ano fuera de rango
			this.mensaje = "Es necesario seleccionar período valido: Año incorrecto, fuera de rango "
					+ (anoActual - 1) + " - " + (anoActual + 1);
			retorno = false;
		}
	...
	public String[] getAuxRemito() {
		return auxRemito;
	}

	public void setAuxRemito(String[] auxRemito) {
	...
	public Hashtable getHtSelecccion() {
		return htSelecccion;
	}

	public void setHtSelecccion(Hashtable htSelecccion) {
		this.htSelecccion = htSelecccion;
	}
