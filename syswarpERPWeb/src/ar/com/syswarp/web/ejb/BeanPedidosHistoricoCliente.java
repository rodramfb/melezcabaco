...
public class BeanPedidosHistoricoCliente implements SessionBean, Serializable {
	static Logger log = Logger.getLogger(BeanPedidosHistoricoCliente.class);
	...
	public BeanPedidosHistoricoCliente() {
		super();
	}
	...
	public boolean ejecutarValidacion() {
		...
		try {
			...
			String entidad = ""
					+ "(SELECT pc.idpedido_cabe, pc.idcliente, es.estado, pc.idempresa "
					+ "  FROM pedidos_cabe pc "
					+ "       INNER JOIN pedidosestados es ON pc.idestado = es.idestado AND pc.idempresa = es.idempresa "
					+ " UNION ALL "
					+ "SELECT pc.idpedido_regalos_cabe, pc.idcliente, es.estado, pc.idempresa "
					+ "  FROM pedidos_regalos_cabe pc "
					+ "       INNER JOIN pedidosestados es ON pc.idestado = es.idestado AND pc.idempresa = es.idempresa ) pedidos";
			...
			if (buscar) {
				...
				this.pedidos_CabeList = pedidos_Cabe
						.getPedidosHistoricoClienteOcu(this.limit, this.offset,
								this.ocurrencia, this.idcliente, this.idempresa);
			} else {
				this.totalRegistros = pedidos_Cabe.getTotalEntidadFiltro(
						entidad, filtro, this.idempresa);
				this.totalPaginas = (this.totalRegistros / this.limit) + 1;
				...
				this.pedidos_CabeList = pedidos_Cabe
						.getPedidosHistoricoClienteAll(this.limit, this.offset,
