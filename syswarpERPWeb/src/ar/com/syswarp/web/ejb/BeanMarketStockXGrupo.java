...
public class BeanMarketStockXGrupo implements SessionBean, Serializable {
	...
	public boolean ejecutarValidacion() {
		...
		try {
			...
			String entidad = "("
					...
					+ "   AND l.idlista =  " + this.marketIdlista + " where st.grupo_st = "+ codigo_gr.toString()+" AND st.idempresa = " + idempresa.toString()+ "  order by 3) entidad ";
			...
			if (buscar) {
				this.filtro = " WHERE (UPPER(codigo_st)   LIKE '%"
						+ cmpBuscar.replaceAll("'", "''").toUpperCase() + "%' ";
				this.filtro += " OR UPPER(descrip_st) LIKE '%"
						+ cmpBuscar.replaceAll("'", "''").toUpperCase()
						+ "%') ";
				...
				this.StockstockList = Stockstock
						.getStockStockMarketOcu(this.limit, this.offset,
								this.cmpBuscar,this.codigo_gr, this.idempresa);
			} else {
				this.filtro = " WHERE grupo_st = " + codigo_gr.toString();
				this.totalRegistros = Stockstock.getTotalEntidadFiltro(
				...
				this.StockstockList = Stockstock
						.getStockStockXGrupo(this.limit, this.offset,
								this.codigo_gr, this.idempresa);
			}
