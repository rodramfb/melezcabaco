...
public class BeanLovClientesMovCliAplicacionesAbm implements SessionBean,
		Serializable {
	static Logger log = Logger
			.getLogger(BeanLovClientesMovCliAplicacionesAbm.class);
	...
	public BeanLovClientesMovCliAplicacionesAbm() {
		super();
	}
	...
	public boolean ejecutarValidacion() {
		...
		try {
			...
			if (buscar) {
				...
				this.clientesMovCliList = clientesMovCli
						.getLovClientesAplicacionMovimientosAll(cliente,
								this.idempresa);
			}
			...
			if (cliente == null || cliente.longValue() < 0) {
				...
				if (this.accion.equalsIgnoreCase("agregar")) {
					...
					if (!this.comprobante.equals("")) {
						...
						List lista = clientesMovCli
								.getLovClientesAplicacionMovimientosOne(
				...
				if (this.accion.equalsIgnoreCase("adelanto")) {
					String[] datosComp = this.comprobante.split("-");
					List lista = clientesMovCli
							.getLovClientesAplicacionMovimientosOne(cliente,
		...
		try {
			if (this.keyHashDatosComprobante != null) {
				for (int i = 0; i < this.keyHashDatosComprobante.length; i++) {
					log.info("totalcobrar[" + i + "]: " + totalcobrar[i]);
					if (ht.containsKey(this.keyHashDatosComprobante[i])) {
						...
						// Double cobrar = Double.parseDouble ();
						...
						if (fOk) {
							...
							datos[1] = Common.getGeneral().getNumeroFormateado(
									Float.parseFloat(this.totalcobrar[i]), 10,
									4);
							datos[1] = new BigDecimal(datos[1]).toString();
							// HEAD OF BALDE
