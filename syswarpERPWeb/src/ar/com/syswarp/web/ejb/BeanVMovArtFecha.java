...
public class BeanVMovArtFecha implements SessionBean, Serializable {
	static Logger log = Logger.getLogger(BeanVMovArtFecha.class);
	...
	public boolean ejecutarValidacion() {
		...
		try {
			...
			if (this.accion.equalsIgnoreCase("alta")) {
				dispatcher = request
						.getRequestDispatcher("vMovArtFechaFrm.jsp");
			...
			if (buscar) {
				...
				this.totalRegistros = vMovArtFecha
						.getTotalStockAgrupadoFechaOcu((java.sql.Date) Common
								.setObjectToStrOrTime(this.fecha_ms,
										"strToJSDate"), this.ocurrencia, this.idempresa);
				...
				this.vMovArtFechaList = vMovArtFecha.getVMovArtFechaOcu(
						this.limit, this.offset, (java.sql.Date) (Common
								.setObjectToStrOrTime(this.fecha_ms,
										"strToJSDate")), this.ocurrencia, this.idempresa);
			} else {
				this.totalRegistros = vMovArtFecha
						.getTotalStockAgrupadoFecha((java.sql.Date) Common
								.setObjectToStrOrTime(this.fecha_ms,
										"strToJSDate"), this.idempresa);
				this.totalPaginas = (this.totalRegistros / this.limit) + 1;
				if (this.totalPaginas < this.paginaSeleccion)
					this.paginaSeleccion = this.totalPaginas;
				this.offset = (this.paginaSeleccion - 1) * this.limit;
				...
				this.vMovArtFechaList = vMovArtFecha.getVMovArtFechaAll(
						(java.sql.Date) (Common.setObjectToStrOrTime(
								this.fecha_ms, "strToJSDate")), this.limit,
