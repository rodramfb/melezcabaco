/* 
   javabean para la entidad: sasmotivoscontactos
   Copyrigth(r) sysWarp S.R.L. 
   Fecha de creacion: Fri May 27 10:58:35 ART 2011 
   
   Para manejar la pagina: sasmotivoscontactosAbm.jsp
...
public class BeanSasmotivoscontactosAbm implements SessionBean, Serializable {
	static Logger log = Logger.getLogger(BeanSasmotivoscontactosAbm.class);
	...
	private List sasmotivoscontactosList = new ArrayList();
	...
	public BeanSasmotivoscontactosAbm() {
		super();
	}
		...
		try {
			if (this.accion.equalsIgnoreCase("baja")) {
				if (idmotivocontacto == null
					...
					this.mensaje = clientes.sasmotivoscontactosDelete(
							idmotivocontacto, this.idempresa);
				}
			}
			if (this.accion.equalsIgnoreCase("modificacion")) {
				if (idmotivocontacto == null
					...
					dispatcher = request
							.getRequestDispatcher("sasmotivoscontactosFrm.jsp");
			...
			if (buscar) {
				...
				this.sasmotivoscontactosList = clientes
						.getSasmotivoscontactosOcu(this.limit, this.offset,
								this.ocurrencia, this.idempresa);
			} else {
				this.totalRegistros = clientes.getTotalEntidad(
						"sasmotivoscontactos", this.idempresa);
				this.totalPaginas = (this.totalRegistros / this.limit) + 1;
				if (this.totalPaginas < this.paginaSeleccion)
					this.paginaSeleccion = this.totalPaginas;
				this.offset = (this.paginaSeleccion - 1) * this.limit;
				...
				this.sasmotivoscontactosList = clientes
						.getSasmotivoscontactosAll(this.limit, this.offset,
								this.idempresa);
			}
	...
	public List getSasmotivoscontactosList() {
		return sasmotivoscontactosList;
	}

	public void setSasmotivoscontactosList(List sasmotivoscontactosList) {
		this.sasmotivoscontactosList = sasmotivoscontactosList;
	}
