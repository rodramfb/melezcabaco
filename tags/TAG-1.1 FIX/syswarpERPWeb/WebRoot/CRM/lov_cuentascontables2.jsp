...
<% 
 ...
 //   String ctactrapartida   =""; 
	String ctactrapartida   = request.getParameter("ctactrapartida");	
	String opcion   = str.esNulo(request.getParameter("opcion"));		
	String id   = str.esNulo(request.getParameter("id"));		
	if (ctactrapartida == null) ctactrapartida = "";
...
function validarCampos() {
	...
	if (document.forms[0].ctactrapartida.value == "") {
...
function passBack(ctactrapartida, d_ctactrapartida) {  
  var opcion = "<%= opcion %>";
  var id     =  "<%= id %>";
  if(opcion == "byId"){
    opener.document.getElementById(id).value = ctactrapartida ;
		id = (parseInt(id)+1) + "" ;
    opener.document.getElementById(id).value = d_ctactrapartida ;		
  }
	else{
		var i = 0;
		 for(i=0;i<10;i++)
			 if(opener.document.forms[i].ctactrapartida)
				 break;    
		 opener.document.forms[i].ctactrapartida.value = ctactrapartida; 
	}
   close();
}
...
<hr>
<form action="lov_cuentascontables2.jsp">
  <table width="59%" border="0" cellspacing="0" cellpadding="0" height="23">
    <tr> 
      ...
      <td width="54%" height="39"> 
        <input name="ctactrapartida" type="text" class="campo" id="ctactrapartida" value="<%=ctactrapartida%>" size="25">
      </td>
...
if (!ctactrapartida.equals("*")) {
  ...
  <table width="100%"  cellPadding=0 cellSpacing=0  border="0">
	...
	<input type="hidden" name="ctactrapartida" value="<%=ctactrapartida%>">
</form>
