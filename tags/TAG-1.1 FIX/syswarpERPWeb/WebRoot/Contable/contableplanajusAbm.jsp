...
/* 
   Grilla para la entidad: contableplanajus
...
String color_fondo ="";
String titulo = "Plan Ajuste";
// variables de entorno
String pathskin = session.getAttribute("pathskin").toString();
String pathscript = session.getAttribute("pathscript").toString();
// variables de paginacion
int i = 0;
Iterator iterContableplanajus   = null;
...
%>
<html>
<jsp:useBean id="BCA"  class="ar.com.syswarp.web.ejb.BeanContableplanajusAbm"   scope="page"/>
<%-- EJECUTAR SETEO DE PROPIEDADES --%>
<jsp:setProperty name="BCA" property="*" />
<%
 BCA.setResponse(response);
 BCA.setRequest(request);
 BCA.setIdejercicio( new BigDecimal( session.getAttribute("ejercicioActivo").toString() ));
...
tituCol[0] = "cuenta_pl";
tituCol[1] = "des_cuenta_pl";
tituCol[2] = "indice_pl";
tituCol[3] = "des_indice_pl";
java.util.List Contableplanajus = new java.util.ArrayList();
Contableplanajus= BCA.getContableplanajusList();
iterContableplanajus = Contableplanajus.iterator();
%>
<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" >
<div id="overDiv" style="position:absolute; visibility:hidden; z-index:1000;"></div>
<form action="contableplanajusAbm.jsp" method="POST" name="frm">
...
<table width="100%" border="0" cellspacing="1" cellpadding="1" name="rsTable" id=rsTable cols=<%=totCol+2-4%>  >
  <tr class="fila-encabezado">
	 ...
	 <td width="30%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><%=tituCol[2]%></td>
	 <td width="32%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><%=tituCol[3]%></td>
</tr>
   <%int r = 0;
   while(iterContableplanajus.hasNext()){
      ++r;
      String[] sCampos = (String[]) iterContableplanajus.next(); 
      // estos campos hay que setearlos segun la grilla 
      if (color_fondo.equals("fila-det-verde")) color_fondo = "fila-det";
      else color_fondo = "fila-det-verde";%>
   <tr onMouseOver="mOvr(this,this.className='fila-det-rojo');" onMouseOut="mOut(this,this.className='<%=color_fondo%>');" class="<%=color_fondo%>" scope="col" > 
      <td class="fila-det-border" ><input type="radio" name="cuenta_pl" value="<%= sCampos[0]%>"></td>
