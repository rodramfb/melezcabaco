...
/* 
   Grilla para la entidad: tickets
...
String color_fondo ="";
String titulo = "TICKETS";
// variables de entorno
String pathskin = session.getAttribute("pathskin").toString();
String pathscript = session.getAttribute("pathscript").toString();
// variables de paginacion
int i = 0;
Iterator iterTickets   = null;
...
%>
<html>
<jsp:useBean id="BTA"  class="ar.com.syswarp.web.ejb.BeanTicketsAbm"   scope="page"/>
...
tituCol[5] = "Descrip.";
tituCol[6] = "Estado";
java.util.List Tickets = new java.util.ArrayList();
Tickets= BTA.getTicketsList();
iterTickets = Tickets.iterator();
%>
<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" >
<div id="overDiv" style="position:absolute; visibility:hidden; z-index:1000;"></div>
<form action="ticketsAbm.jsp" method="POST" name="frm">
...
<table width="100%" border="0" cellspacing="1" cellpadding="1" name="rsTable" id=rsTable cols=<%=totCol+2-4%>  >
  <tr class="fila-encabezado">
     <td width="1%" >&nbsp;</td>
     <td width="1%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><div align="right"><%=tituCol[0]%></div></td>
     ...
     <td width="30%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><%=tituCol[5]%></td>
     <td width="5%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><%=tituCol[6]%></td>
   </tr>
   <%int r = 0;
   while(iterTickets.hasNext()){
      ++r;
      String[] sCampos = (String[]) iterTickets.next(); 
      // estos campos hay que setearlos segun la grilla 
      if (color_fondo.equals("fila-det-verde")) color_fondo = "fila-det";
      else color_fondo = "fila-det-verde";%>
   <tr onMouseOver="mOvr(this,this.className='fila-det-rojo');" onMouseOut="mOut(this,this.className='<%=color_fondo%>');" class="<%=color_fondo%>" scope="col"> 
      <td class="fila-det-border" ><input type="radio" name="idticket" value="<%= sCampos[0]%>"></td>
      <td class="fila-det-border" ><div align="right">&nbsp;<%=sCampos[0]%></div></td>
      <td class="fila-det-border" >&nbsp;<%=!Common.setNotNull(sCampos[1]).equalsIgnoreCase("") ? sCampos[1] : "No tiene asignado grupo" %></td>
      <td class="fila-det-border" >&nbsp;<%=!Common.setNotNull(sCampos[2]).equalsIgnoreCase("") ? sCampos[2] : "No tiene asignado usuario" %></td>
      <td class="fila-det-border" >&nbsp;<%=sCampos[3]%></td>
      <td class="fila-det-border" >&nbsp;<%=sCampos[4]%></td>
      <td class="fila-det-border" >&nbsp;<%=sCampos[5]%></td>
      <td class="fila-det-border" bgcolor="<%=sCampos[8]%>">&nbsp;<%=sCampos[6]%></td>
   </tr>
<%
   }%>
   </table>
