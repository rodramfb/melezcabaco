...
String color_fondo ="";
String titulo = "Articulo Hasta";
// variables de entorno
String pathskin = session.getAttribute("pathskin").toString();
String pathscript = session.getAttribute("pathscript").toString();
// variables de paginacion
int i = 0;
Iterator iterClientesarticulo   = null;
...
%>
<html>
<jsp:useBean id="BFFF"  class="ar.com.syswarp.web.ejb.BeanClientesArticuloLov"   scope="page"/>
<%-- EJECUTAR SETEO DE PROPIEDADES --%>
<jsp:setProperty name="BFFF" property="*" />
<%
 String idempresainforme = session.getAttribute("empresa").toString(); //request.getParameter("idempresainforme");
 BFFF.setResponse(response);
 BFFF.setRequest(request);
 BFFF.setIdempresa( new BigDecimal( idempresainforme ) );  
...
<title><%=titulo%></title>
 ...
 <script >
function bajarDatos(codigo_hasta_st, descrip_hasta_st){
	for(var i = 0; i<10;i++)
		if(opener.document.forms[i].codigo_hasta_st)
			break;
	opener.document.forms[i].codigo_hasta_st.value = codigo_hasta_st;
	opener.document.forms[i].descrip_hasta_st.value = descrip_hasta_st; 
	window.close();
}
 </script>
</head>
...
tituCol[0] = "Codigo";
tituCol[1] = "Articulo";
tituCol[2] = "Es Serializable";
tituCol[3] = "Numero de Serie";

java.util.List clientesdescrip_hasta_st = new java.util.ArrayList();
clientesdescrip_hasta_st= BFFF.getclientesarticuloList();
iterClientesarticulo = clientesdescrip_hasta_st.iterator();
%>
<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" >
<div id="overDiv" style="position:absolute; visibility:hidden; z-index:1000;"></div>
<form action="lov_articulo_hasta_informe.jsp" method="POST" name="frm">
...
<table width="100%" border="0" cellspacing="1" cellpadding="1" name="rsTable" id=rsTable cols=<%=totCol+2-4%>  >
  <tr class="fila-encabezado">
   ...
   <%int r = 0;
   while(iterClientesarticulo.hasNext()){
      ++r;
      String[] sCampos = (String[]) iterClientesarticulo.next(); 
