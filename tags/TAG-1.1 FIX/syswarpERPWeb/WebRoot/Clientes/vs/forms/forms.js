...
function validaNumericosFF(evento){
  var codigo = getCodigo(evento);
  if( (codigo< 48 || codigo >57)  &&  (codigo != 8  && codigo != 46) ) return false;
  else return true;
}

function validaNumericosFFRemito(evento){
  var codigo = getCodigo(evento);
 
  if( (codigo< 48 || codigo >57)  &&  (codigo != 8 && codigo != 45 && codigo != 46) ) return false;
  else return true;
}
...
DATE: 20120516
AUTOR: EJV
OBJETIVO: Reemplazar el atributo TITLE/ALT en las etiquetas HTML.
*/
function mostrarMasInfo(masInfo){
	overlib( masInfo , STICKY, CAPTION, '[INFO]',TIMEOUT,25000, HAUTO, VAUTO, BGCOLOR, '#000033');  
}
