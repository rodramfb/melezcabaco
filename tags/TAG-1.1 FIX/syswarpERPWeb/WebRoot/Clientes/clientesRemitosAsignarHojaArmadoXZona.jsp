...
%>
<html>
<jsp:useBean id="BPA"  class="ar.com.syswarp.web.ejb.BeanClientesRemitosAsignarHojaArmadoXZona"   scope="page"/>
...
function verTotales(mensaje){
    if(mensaje)
	  overlib( mensaje , STICKY, CAPTION, '[INFO:Totales Por familia]',TIMEOUT,25000,HAUTO,VAUTO,BGCOLOR,'#000000');
	else 
	  overlib( 'No existen totales.' , STICKY, CAPTION, '[INFO:Totales Por familia]',TIMEOUT,25000,HAUTO,VAUTO,BGCOLOR,'#000000');
}
...
function sendValidacion(){
  ...
  if(cmpInput.value.length == 19  ){ 
    var suc = cmpInput.value.substring(7, 11 );
	var comprob = cmpInput.value.substring(11);
    cmpInput.value = '';	
	if(checkRemito(suc + comprob)){
      sucursal.value = suc;
      comprobante.value = comprob;
	  
	  document.frm.accion.value = 'seleccionremito';
      document.frm.submit();
	}
  }
...
function checkRemito(remito){

  var obj = eval("document.getElementById('auxRemito_" +  remito + "')");
  if(obj){
    if(document.frm.idremitocliente.length){
	  //alert(document.frm.idremitocliente[obj.value].checked) ;
	  document.frm.idremitocliente[obj.value].checked = true;
	}
	return true;
  }else {
   // marca
   // style="background:#99cc66;border:hidden;color:#FF0000;font:bold"
   var objTextTipo = document.getElementById("salida");
   objTextTipo.style.color = '#FF0000';
   objTextTipo.style.font = 'bold';
   objTextTipo.firstChild.nodeValue = 'Remito ' + remito + ' no existe en la seleccion actual.';
   return false;
  }
}


function sugiereRecalculo(){ 
  overlib('No olvide recalcular totales!!', STICKY, CAPTION, 'ATENCION',TIMEOUT,5000,HAUTO,VAUTO, WIDTH,200,BGCOLOR, '#CCCCCC', CAPCOLOR, '#FF0000');
}
...
<form action="clientesRemitosAsignarHojaArmadoXZona.jsp" method="POST" name="frm">
                                    ...
                                    <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                       ...
                                       <tr class="text-globales">
                                         <td height="44"><div align="center">
                                           <input name="recalculartotales" type="submit" class="boton" id="recalculartotales" value="Recalcular Totales" >
                                         </div></td>
                                       ...
                                       <tr class="text-globales">
                                         <td height="44"><div align="center">
                                           <input name="vertotales" type="button" class="boton" id="vertotales" value="Ver Totales por Familia" onClick="verTotales(<%=BPA.generaMostrarTotales() %>);">
                                         </div></td>
                                         <td height="44"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                                             <tr class="text-globales">
                                               <td width="33%">Lectura Remito: </td>
                                               <td width="67%"><input name="inputremito" type="text" class="campo" id="inputremito" style="text-align:right" onKeyUp="sendValidacion(this)" size="19" maxlength="100" >
                                                 <input name="nrosucursal" type="hidden" value="<%//=Common.strZero(BBOEA.getNrosucursal(), 4)%>" id="nrosucursal" size="5" maxlength="4" class="campo"  style="text-align:right" >
                                                 <input name="nroremitocliente" type="hidden" value="<%//=Common.strZero(BBOEA.getNroremitocliente(), 8)%>" id="nroremitocliente" size="10" maxlength="8" class="campo" style="text-align:right" ></td>
                                             </tr>
                                           </table></td>
                                         <td height="44" colspan="4" id="salida">&nbsp;</td>
                                       </tr>
                                   </table>                                 </td>
                              </tr>
                          </table>                        </td>
...
<table width="100%" border="0" cellspacing="1" cellpadding="1" name="rsTable" id=rsTable cols=<%=totCol+2-4%>  >
  ...
  <tr class="text-globales">
    <td ><input name="filtroGrupoArmado" type="text" value="<%=BPA.getFiltroGrupoArmado()%>" id="filtroGrupoArmado" size="7" maxlength="250" style="text-align:right"></td> 
    <td >&nbsp;</td>
    <td ><div align="center">
    ...
    <td ><div align="center">
      ...
      Iterator iter = BPA.getListExpresosZonas().iterator();
			...
			String [] datos = (String[])iter.next();%>
				<option value="<%= datos[0] %>" <%= datos[0].equals( BPA.getFiltroExpresoZona().toString()) ? "selected" : "" %>><%= datos[2] %></option>
				<%  
			}%>
			</select> 
    ...
    <td ><div align="center">
			<select name="filtroDepOrigen" id="filtroDepOrigen" class="campo" style="width:80%"  onChange="document.frm.submit();">
				<option value="">Seleccionar</option>
				<%
   ...
   while(iterPedidos_cabe.hasNext()){
      ...
      if(!grupoArmado.equals(sCampos[22])){
        ...
        grupoArmado = sCampos[22]; 
		 ...
		 %>
	<tr height="3px" >
     <td colspan="9" bgcolor="<%= colorGrupo %>"  class="fila-det-border" height="3px" ></td>
    </tr>
		 <% 
   ...
   <tr onMouseOver="mOvr(this,this.className='fila-det-rojo');" onMouseOut="mOut(this,this.className='<%=color_fondo%>');" class="<%=color_fondo%>" scope="col" >
     <td rowspan="2" bgcolor="<%= colorGrupo %>"  class="fila-det-border" ><div align="center" onMouseOver="mostrarMensaje('<%= grupoArmado %>')">&nbsp;</div></td> 
      ...
      <td rowspan="2" class="fila-det-border" ><div align="center">
          <input name="idremitocliente" type="checkbox" id="idremitocliente" value="<%=sCampos[0]%>" <%= BPA.getHtSelecccion() .containsKey(sCampos[0]) ? "checked" : "" %> onClick="sugiereRecalculo()">
		  <input name="auxRemito" type="hidden" id="auxRemito_<%=sCampos[2] + sCampos[3]%>" value="<%=r%>">
      </div></td>
   </tr>
   <tr onMouseOver="mOvr(this,this.className='fila-det-rojo');" onMouseOut="mOut(this,this.className='<%=color_fondo%>');" class="<%=color_fondo%>" scope="col" >
