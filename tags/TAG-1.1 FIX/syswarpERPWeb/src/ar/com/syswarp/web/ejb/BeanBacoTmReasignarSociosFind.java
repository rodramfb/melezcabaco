...
public class BeanBacoTmReasignarSociosFind implements SessionBean, Serializable {

	static Logger log = Logger.getLogger(BeanBacoTmReasignarSociosFind.class);
	...
	private BigDecimal idtelemarkdestino = new BigDecimal(-1);
	...
	public BeanBacoTmReasignarSociosFind() {
		super();
	}
		...
		try {
			...
			if (!this.accion.equals("")) {
				...
				if (this.idtelemark.longValue() < 0) {
					this.mensaje = "Es necesario seleccionar Telemarketer Destino.";
					return false;
				}
				...
				if (!this.idclientedesde.trim().equals("")) {
					if (Common.esEntero(this.idclientedesde)) {
						...
						this.mensaje = "Ingrese valores v�lidos para socio desde.";
						return false;
					}
				}
				...
				if (this.filtros.equals("")) {
					...
					if (this.idtelemark.longValue() == this.idtelemarkdestino.longValue() ) {
						this.mensaje = "El Telemarketer Origen no puede ser igual a Telemarketer Destino.";
						return false;	
					}
					
					if (this.idtelemark.longValue() < 0) {
						this.mensaje = "Es necesario seleccionar telemarketer Origen.";
						return false;
					}
					if (this.idtelemarkdestino.longValue() < 0) {
						this.mensaje = "Es necesario seleccionar telemarketer Destino.";
						return false;
					}
                    
					
					if (this.idclientedesde.equals("") && !this.idclientehasta.equals("")){
						this.mensaje = "Ingrese Cliente Hasta!";
						return false;
					}
					
					if (!this.idclientedesde.equals("") && this.idclientehasta.equals("")){
						this.mensaje = "Ingrese Cliente Desde!";
						return false;
					}
					...
					if (!this.limiteasignacion.equals("")) {
					    ...
					    this.mensaje = Common.getClientes().bacoTmReasignacionSocioUpdate(this.idcampacabe,
					    this.idtelemark,this.idtelemarkdestino,filtros,stmtLimit,
					    this.idempresa,this.usuarioalt,this.idclientedesde,this.idclientehasta); 
					
                       if (Common.esEntero(this.mensaje)) {
						  this.mensaje = " Se Reasignaron un total de " + this.mensaje + " registros.";
                       }else{
   						this.mensaje = "Error por favor Verifique!";
					...
					this.totalRegistros = Common.getClientes()
							.getBacoTmReasignacionTotalSocios(this.idcampacabe,
									filtros, this.idempresa,this.idtelemark).longValue();
					...
					if (this.totalRegistros < 0)
						...
						this.mensaje = "Se contabiliz� un total de "
								+ this.totalRegistros
	...
	public BigDecimal getIdtelemarkdestino() {
		return idtelemarkdestino;
	}

	public void setIdtelemarkdestino(BigDecimal idtelemarkdestino) {
		this.idtelemarkdestino = idtelemarkdestino;
	}
	
	public long getMensaje2() {
		return mensaje2;
	}

	public void setMensaje2(long mensaje2) {
