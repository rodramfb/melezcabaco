...
public class BeanLovCajaMovimientosCancelarPagosEntrada implements SessionBean,
		Serializable {
	static Logger log = Logger
			.getLogger(BeanLovCajaMovimientosCancelarPagosEntrada.class);
	...
	public BeanLovCajaMovimientosCancelarPagosEntrada() {
		super();
	}
	...
	public boolean ejecutarValidacion() {
		...
		Hashtable htMovimientosEntradaCancelar = (Hashtable) session
				.getAttribute("htMovimientosEntradaCancelar");

		if (htMovimientosEntradaCancelar == null) {
			htMovimientosEntradaCancelar = new Hashtable();
			Common.htRellenar((Hashtable) session
					.getAttribute("htMovimientosEntradaCancelarOK"),
					htMovimientosEntradaCancelar);
		}

		BigDecimal totalCartera = new BigDecimal(-1);
		String[] tipoPropietario = null;
		// (Hashtable) session.getAttribute("htMovimientosEntradaCancelarOK");
		...
		try {
			...
			if (tipoPropietario[0].equalsIgnoreCase("T")
					&& tipoPropietario[1].equalsIgnoreCase("S")) {
			...
			String filtro = " WHERE usado_mt IS NULL  AND cartera_mt =  '"
					...
					+ this.tipcart_mt + "'     AND fecha_mt <=  '"
					+ this.fecha_mt + "'::timestamp ";
			if (buscar) {
			...
			if (this.accion.equalsIgnoreCase("agregar")) {
				...
				if (!this.idmovteso.equals("0")) {
					...
					log.info("this.idmovteso:" + this.idmovteso);
					...
					if (lista.size() > 0) {
						...
						if (datos[6].equalsIgnoreCase("T")
								&& datos[7].equalsIgnoreCase("S")) {

							datos[36] = totalCartera.toString();

							
							if (htMovimientosEntradaCancelar.containsKey(clave))
								this.reasigna = true;
							htMovimientosEntradaCancelar.put(clave, datos);

						} else {

							htMovimientosEntradaCancelar.put(this.idmovteso
									.toString(), datos);
						}
					} else {
			...
			if (this.accion.equalsIgnoreCase("eliminar")) {
				if (this.delKey == null) {
					...
					for (int i = 0; i < this.delKey.length; i++) {
						if (htMovimientosEntradaCancelar
								.containsKey(this.delKey[i])) {
							htMovimientosEntradaCancelar.remove(this.delKey[i]);
						}
					}
				}
			}

			this.asignarValidar(htMovimientosEntradaCancelar);
	...
	private boolean asignarValidar(Hashtable ht) {
		...
		try {
			...
			if ((this.accion.equalsIgnoreCase("confirmar") || (ht.isEmpty() && this.accion
					...
					&& mensaje.equals("")) {
				session.setAttribute("htMovimientosEntradaCancelarOK", ht);
				session.removeAttribute("htMovimientosEntradaCancelar");

			} else {
				if (!mensaje.equals(""))
					this.mensaje = mensaje;
				session.setAttribute("htMovimientosEntradaCancelar", ht);
			}
