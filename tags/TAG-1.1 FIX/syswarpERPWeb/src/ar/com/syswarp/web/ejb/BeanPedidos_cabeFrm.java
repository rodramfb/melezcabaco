...
public class BeanPedidos_cabeFrm implements SessionBean, Serializable {
	private SessionContext context;

	static Logger log = Logger.getLogger(BeanPedidos_cabeFrm.class);
	...
	private java.sql.Timestamp fpedido = null;
	// <--
	...
	// 20100210 - EJV -->
	// Mantis 383
	private BigDecimal idtipoivaloca = new BigDecimal(-1);

	private BigDecimal idtipoivacliente = new BigDecimal(-1);

	private boolean calculoOk = true;

	// <--

	// 20100630 - EJV / Referidos -->
	private BigDecimal idlistaReferidos = new BigDecimal(-1);
	...
	// idpromocion,promocion,idtipoclie,fechadesde,fechahasta,convenio,porc_desc_ci,porc_liq

	private BigDecimal idpromocion = new BigDecimal(-1);

	private String promocion = "";

	private java.sql.Date fDesdePromo = null;

	private java.sql.Date fHastaPromo = null;

	private BigDecimal porc_desc_ci = new BigDecimal(-1);

	// <--

	// Mantis 666 - EJV - 20110127
	// -->

	private List listVentasEspeciales = new ArrayList();

	private BigDecimal idventaespecial = new BigDecimal(-1);

	private String fechafactura = "";

	private java.sql.Date ffactura = null;

	// <--

	// --> 20110207 - EJV - Mantis - 671

	private java.sql.Date hoy = null;

	// <--

	public BeanPedidos_cabeFrm() {
		super();
	}
	...
	public void ejecutarSentenciaDML() {
		try {
			...
			resultado = pedidos_cabe.pedidosNormalesCreate(GeneralBean.setNull(
					this.idcampacabe, 1), this.idestado, this.idcliente,
					this.idsucursal, this.iddomicilio, this.fpedido,
					this.idcondicion, this.idvendedor, this.idexpreso,
					...
					this.idlistaReferidos, this.idanexolocalidad, GeneralBean
							.setNull(this.idpromocion, 1), GeneralBean.setNull(
							this.idventaespecial, 1), this.ffactura,
					this.usuarioalt, this.idempresa, null);

			if (Common.esNumerico(resultado[0]) && resultado[1].equals("")) {
				this.mensaje = "Se genero el pedido nro.: " + resultado[0]
						+ ".";
				this.idpedido_cabe = new BigDecimal(resultado[0]);
	...
	public boolean ejecutarValidacion() {
		try {
			...
			this.hoy = new java.sql.Date(cal.getTimeInMillis());

			Clientes pedidos_cabe = Common.getClientes();

			this.listPrioridades = pedidos_cabe.getPedidosPrioridadesAll(50, 0);

			this.llenarListas(pedidos_cabe);
			...
			if (isPrimeraCarga()) {
				...
				if (this.accion.equalsIgnoreCase("limpiardetalle")) {
					...
					// 20100210 - EJV -->
					// Mantis 383
					if (this.idtipoivaloca.longValue() < 1) {
						this.idtipoiva = this.idtipoivacliente;
					}
				...
				if (this.accion.equalsIgnoreCase("recarga")) {
					// reiniciaIds();
					this.getDatosCliente(pedidos_cabe);
			...
			this.getDatosPromocion();

			// <--

			// log.info("this.idcondicion: " + this.idcondicion);
			...
			if (this.idcondicion.longValue() == 5) {
				...
				if (this.listaTarjetasCredito != null
					...
					// --> 20110207 - EJV - Mantis - 671
					// this.hoy = new java.sql.Date(Calendar.getInstance()
					// .getTimeInMillis());
					// <--
					...
					while (it.hasNext()) {
						...
						if (Common.setNotNull(datos[12]).equalsIgnoreCase("S")
								&& java.sql.Date.valueOf(datos[9]).after(
										this.hoy)
								// --> 20110725 - EJV - Mantis - 745
								&& this.idtarjeta.longValue() == -1
						// <--
						) {
			...
			if (!this.validar.equalsIgnoreCase("")) {
				...
				if (!Common.isFormatoFecha(this.fechapedido)
					...
					this.mensaje = "Ingrese fecha de pedido valida. ";
					return false;
				}

				this.fpedido = (java.sql.Timestamp) Common
						.setObjectToStrOrTime(this.fechapedido, "StrToJSTs");

				if ((new java.sql.Date(this.fpedido.getTime()))
						.before(this.hoy)) {
					this.mensaje = "Fecha de pedido debe ser igual o mayor a fecha actual: "
							+ Common.initObjectTimeStr();
					return false;
				}
				...
				if (this.idanexolocalidad == null
					...
					this.mensaje = "Anexo-Localidad no asociado para el domicilio seleccionado.";
					return false;
				}
				...
				if (!Common.isFormatoFecha(fechafactura)
						&& !Common.isFechaValida(fechafactura)) {
					this.mensaje = "Ingrese fecha de factura. ";
					return false;
				}

				this.ffactura = (java.sql.Date) Common.setObjectToStrOrTime(
						this.fechafactura, "StrToJSDate");
				// <--
	...
	private void llenarListas(Clientes clientes) {
		...
		try {
			...
			this.getDomiciliosCliente(clientes);
			...
			this.listVentasEspeciales = clientes.getPedidosVentasEspecialesAll(
					cuantos, desde, idempresa);
	...
	private void getDatosCliente(Clientes clientes) {
		try {
			 ...
			 * 29-pf.preferencia
			 * ,....................................................
			 * 30-cl.usuarioalt,cl.usuarioact,cl.fechaalt,cl.fechaact
			 */
			...
			if (!listaCliente.isEmpty() && listaCliente.size() == 1) {
				...
				// log.info("datos[" + M + "]: " + datos[M]);

				// ----------------------------------------->
				// 20100209 - EJV
				// this.idtipoiva = new BigDecimal(datos[6]);
				// Mantis 383
				if (this.idtipoivaloca.longValue() > 0)
					this.idtipoiva = this.idtipoivaloca;
				else
					this.idtipoiva = new BigDecimal(datos[6]);

				this.idtipoivacliente = new BigDecimal(datos[6]);
				// <----------------------------------------

				this.idcondicion = new BigDecimal(datos[8]);
				this.idmoneda = new BigDecimal(datos[14]);
				this.idlista = new BigDecimal(datos[16]);
				// this.obsentrega = datos[20];
				this.idvendedor = new BigDecimal(datos[27]);
				...
				this.adidesc = new BigDecimal(datos[33]);
				// 20110124 - EJV - Mantis 597 -->
	...
	private void getDomiciliosCliente(Clientes clientes) {
		try {
			this.listaDomicilios = clientes.getClientesDomiciliosCliente(5000,
			...
			if (!this.listaDomicilios.isEmpty()
				...
				while (iter.hasNext()) {
					...
					this.datosJSDomi += "\tvar datosDomicilio" + datos[0]
							...
							+ Common.setNotNull(datos[41]) + "', '"
							// 20100804 - EJV - Reasignacion
							+ Common.setNotNull(datos[39]) + "' );\n";

					if (this.accion.equalsIgnoreCase("recarga")) {

						if (datos[4] == null || datos[21] == null
								|| datos[19] == null || datos[39] == null) {
							...
							// 20100804 - EJV - Reasignacion
							this.idanexolocalidad = new BigDecimal(
									datos[39] != null ? datos[39] : "-1");
							...
							this.idtipoivaloca = new BigDecimal(datos[41]);
							// 20100804 - EJV - Reasignacion
							this.idanexolocalidad = new BigDecimal(datos[39]);
	...
	private BigDecimal getTotalesAsiento(Hashtable ht, int indice) {
		...
		try {
			...
			if (ht != null) {
				// 20100210 - EJV -->
				// Mantis 383
				if (this.idtipoiva == null || this.idtipoiva.longValue() < 1) {
				...
				this.listaTipoIva = Common.getClientes().getClientestablaivaPK(
						this.idtipoiva, this.idempresa);

				if (this.idcliente.compareTo(new BigDecimal(0)) < 1) {

					this.mensaje = "Es necesario seleccionar un cliente para calcular importes.";
					this.calculoOk = false;

				} else if (this.idtipoiva == null
						|| this.idtipoiva.longValue() < 1) {
					mensaje = "Es necesario seleccionar tipo de iva.";
					this.calculoOk = false;
			...
			this.mensaje = "Se produjo un error al calcular importes.";
			this.calculoOk = false;
			...
			if (this.keyHashArticulo != null) {
				for (int i = 0; i < this.keyHashArticulo.length; i++) {
					if (ht.containsKey(this.keyHashArticulo[i])) {
						...
						if (this.idlista.longValue() == this.idlistaReferidos
								.longValue()) {

							if (Float.parseFloat(this.porcdesc_apli[i]) != 100) {
								mensajeAV = "El porcentaje de descuento para canje de puntos debe ser 100 %  para "
										+ datos[0];
								fOk = false;
							}
	...
	private void getDatosPromocion() {

		try {

			Iterator it = Common.getClientes().getPromocionesActivasXTipoClie(
					this.idtipoclie, this.idempresa).iterator();
			...
			if (it != null) {
				...
				if (it.hasNext()) {
					...
					// for (int i = 0; i < datos.length; i++) {
					// log.info("datos[" + i + "]: " + datos[i]);
					// }

					this.idpromocion = new BigDecimal(datos[0]);
					this.promocion = datos[1];
					// 20120817 - EJV - Error detectado y corregido, indice
					// this.fDesdePromo = java.sql.Date.valueOf(datos[3]);
					// this.fHastaPromo =
					// !Common.setNotNull(datos[4]).equals("") ? java.sql.Date
					// .valueOf(datos[4])
					// : null;
					// this.porc_desc_ci = new BigDecimal(datos[6]);
					this.fDesdePromo = java.sql.Date.valueOf(datos[4]);
					this.fHastaPromo = !Common.setNotNull(datos[5]).equals("") ? java.sql.Date
							.valueOf(datos[5])
							: null;

					this.porc_desc_ci = new BigDecimal(datos[7]);
					// <--
				}
	...
	// 20100210 - EJV -->
	// Mantis 383
	public BigDecimal getIdtipoivaloca() {
		return idtipoivaloca;
	}

	public void setIdtipoivaloca(BigDecimal idtipoivaloca) {
		this.idtipoivaloca = idtipoivaloca;
	}

	public BigDecimal getIdtipoivacliente() {
		return idtipoivacliente;
	}

	public void setIdtipoivacliente(BigDecimal idtipoivacliente) {
		this.idtipoivacliente = idtipoivacliente;
	}

	// <--

	public BigDecimal getIdlistaReferidos() {
		return idlistaReferidos;
	}

	public void setIdlistaReferidos(BigDecimal idlistaReferidos) {
		this.idlistaReferidos = idlistaReferidos;
	}
	...
	public java.sql.Date getFDesdePromo() {
		return fDesdePromo;
	}

	public void setFDesdePromo(java.sql.Date desdePromo) {
		fDesdePromo = desdePromo;
	}

	public java.sql.Date getFHastaPromo() {
		return fHastaPromo;
	}

	public void setFHastaPromo(java.sql.Date hastaPromo) {
		fHastaPromo = hastaPromo;
	}

	public BigDecimal getPorc_desc_ci() {
		return porc_desc_ci;
	}

	public void setPorc_desc_ci(BigDecimal porc_desc_ci) {
	...
	public List getListVentasEspeciales() {
		return listVentasEspeciales;
	}
