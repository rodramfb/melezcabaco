...
public class BeanLovCajaIdentificadoresEntradaOtrosMov implements SessionBean,
		Serializable {
	static Logger log = Logger
			.getLogger(BeanLovCajaIdentificadoresEntradaOtrosMov.class);
	...
	public BeanLovCajaIdentificadoresEntradaOtrosMov() {
		super();
	}
	...
	public boolean ejecutarValidacion() {
		...
		Hashtable htIdentificadoresIngresos = (Hashtable) session
				.getAttribute("htIdentificadoresIngresos") != null ? (Hashtable) session
				.getAttribute("htIdentificadoresIngresos")
				: (Hashtable) session
						.getAttribute("htIdentificadoresIngresosOK");

		Hashtable htOMDocTercerosCancelarINOK = (Hashtable) session
				.getAttribute("htOMDocTercerosCancelarINOK") != null ? (Hashtable) session
		...
		try {
			...
			String entidad = "videntificadorestipo";
			String filtro = " WHERE idtipoidentificador NOT IN( (SELECT idtipoidentificador FROM cajatipoidentificadores WHERE tipomov = 'T' OR (tipomov = 'D' AND propio = 'S') )) ";
			...
			if (buscar) {
				...
				this.totalRegistros = caja.getTotalEntidadFiltro(entidad,
						filtro, idempresa);
				...
				this.cajaIdentificadoresList = caja
						.getLovCajaIdentOtrosMovINOcu(this.limit, this.offset,
				...
				this.cajaIdentificadoresList = caja
						.getLovCajaIdentOtrosMovINAll(this.limit, this.offset,
								this.idempresa);
			...
			if (this.accion.equalsIgnoreCase("agregar")) {
				...
				// session.setAttribute("htIdentificadoresIngresos",
				// htIdentificadoresIngresos);
			}
			...
			if (this.accion.equalsIgnoreCase("eliminar")) {
				if (this.delKey == null) {
					...
					for (int i = 0; i < this.delKey.length; i++) {
						if (htIdentificadoresIngresos
								.containsKey(this.delKey[i])) {
							htIdentificadoresIngresos.remove(this.delKey[i]);
						}
						if (htOMDocTercerosCancelarINOK
								.containsKey(this.delKey[i])) {
							htOMDocTercerosCancelarINOK.remove(this.delKey[i]);
						}
					}

					session.setAttribute("htOMDocTercerosCancelarINOK", htOMDocTercerosCancelarINOK);
					
					if (htOMDocTercerosCancelarINOK == null
							|| htOMDocTercerosCancelarINOK.isEmpty()){
			...
			this.asignarValidar(htIdentificadoresIngresos);
	...
	private boolean asignarValidar(Hashtable ht) {
		...
		try {
				...
				for (int i = 0; i < this.keyHashDatosIdentificador.length; i++) {
					if (ht.containsKey(this.keyHashDatosIdentificador[i])) {
						...
						if (fOk) {
							...
							datos[34] = this.descripcion_cuenta[i];
							datos[24] = this.cc1[i];
							datos[25] = this.cc2[i];
			...
			if ((this.accion.equalsIgnoreCase("confirmar") || (ht.isEmpty() && this.accion
				...
				session.setAttribute("htIdentificadoresIngresosOK", ht);
				session.removeAttribute("htIdentificadoresIngresos");

			} else {
				if (!mensaje.equals(""))
					this.mensaje = mensaje;
				session.setAttribute("htIdentificadoresIngresos", ht);
			}
