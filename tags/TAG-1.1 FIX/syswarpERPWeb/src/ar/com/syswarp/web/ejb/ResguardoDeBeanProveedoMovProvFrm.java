...
public class ResguardoDeBeanProveedoMovProvFrm implements SessionBean, Serializable {
	private SessionContext context;

	private BigDecimal idempresa = new BigDecimal(-1);

	static Logger log = Logger.getLogger(ResguardoDeBeanProveedoMovProvFrm.class);
	...
	public ResguardoDeBeanProveedoMovProvFrm() {
		super();
	}
				...
				if (Common.esNumerico(porcentaje)) {
					if (Common.esPorcentaje(porcentaje)) {
						session.setAttribute(nombre, Common
								.getNumeroFormateado(Double.parseDouble(porcentaje), 3, 2));
					}
				}
			}
		} catch (Exception e) {
				...
				if (!Common.esPorcentaje(this.porcentajeBonificacion)) {
					...
					if (this.session.getAttribute("htArticulosConfirmados") != null) {
						...
						this.bonificacion = Common
								.getNumeroFormateado(
										(this.subTotalGravado * (Double.parseDouble(this.porcentajeBonificacion) / 100)),
										10, 2);
						// falta formatear
						this.netoGravado = (this.subTotalGravado - Double.parseDouble(this.bonificacion))
								+ "";
					}
								...
								+ Common
										.getNumeroFormateado(
												Double.parseDouble(this.netoGravado)
														* (Double.parseDouble(this.provPorcNormalIva) / 100),
												10, 2);
					else
						this.iva = "0.00";
					...
					if (this.readonlyRetIva.equals(""))
						...
						this.percepcionIva = ""
								+ Common
										.getNumeroFormateado(
												Double.parseDouble(this.netoGravado)
														* (Double.parseDouble(this.provPorcEspecialIvaPercep) / 100),
												10, 2);
					else
					...
					this.importe = Double.valueOf(Common.getNumeroFormateado(
							Double.parseDouble(this.netoGravado)
									...
									+ Double.parseDouble(this.percepcionIva), 10,
							2));
