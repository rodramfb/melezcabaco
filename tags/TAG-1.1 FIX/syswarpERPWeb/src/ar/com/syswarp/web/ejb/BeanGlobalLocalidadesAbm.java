...
public class BeanGlobalLocalidadesAbm implements SessionBean, Serializable {
	static Logger log = Logger.getLogger(BeanGlobalLocalidadesAbm.class);
	...
	public BeanGlobalLocalidadesAbm() {
		super();
	}
	...
	public boolean ejecutarValidacion() {
		...
		Clientes globalLocalidades = Common.getClientes();
		try {
			if (this.accion.equalsIgnoreCase("baja")) {
				if (idlocalidad == null
						|| idlocalidad.compareTo(new BigDecimal(0)) == 0) {
					this.mensaje = "Debe seleccionar un registro a eliminar.";
				} else {
					this.mensaje = globalLocalidades
							.globalLocalidadesDelete(idlocalidad);
				}
			}
			...
			if (buscar) {
				String[] campos = { "cpostal", "localidad" };
				this.totalRegistros = globalLocalidades
						.getTotalEntidadGlobalOcu("globalLocalidades", campos,
								this.ocurrencia);
				...
				this.globalLocalidadesList = globalLocalidades
						.getGlobalLocalidadesObseqOcu(this.limit, this.offset,
								this.ocurrencia, this.idempresa);
			} else {
				this.totalRegistros = globalLocalidades
						.getTotalEntidadGlobal("globalLocalidades");
				this.totalPaginas = (this.totalRegistros / this.limit) + 1;
				if (this.totalPaginas < this.paginaSeleccion)
					this.paginaSeleccion = this.totalPaginas;
				this.offset = (this.paginaSeleccion - 1) * this.limit;
				...
				this.globalLocalidadesList = globalLocalidades
						.getGlobalLocalidadesObseqAll(this.limit, this.offset,
								this.idempresa);
			}
