...
public class BeanProveedoProveedAbm implements SessionBean, Serializable {
	static Logger log = Logger.getLogger(BeanProveedoProveedAbm.class);
		...
		try {
			if (this.accion.equalsIgnoreCase("baja")) {
				if (idproveedor == null || idproveedor.longValue() < 0) {
					...
					this.mensaje = proveedoProveed.proveedoProveedDelete(
							idproveedor, this.idempresa);
				}
			}
			if (this.accion.equalsIgnoreCase("modificacion")) {
				if (idproveedor == null || idproveedor.longValue() < 0) {
					...
					dispatcher = request
							.getRequestDispatcher("proveedoProveedFrm.jsp");
			...
			if (buscar) {
				String[] campos = { "idproveedor", "razon_social", "cuit",
						"telefono" };
				this.totalRegistros = proveedoProveed.getTotalEntidadOcu(
						"proveedoProveed", campos, this.ocurrencia,
						this.idempresa);
				...
				this.proveedoProveedList = proveedoProveed
						.getProveedoProveedOcu(this.limit, this.offset,
								this.ocurrencia, this.sort, this.idempresa);
			} else {
				this.totalRegistros = proveedoProveed.getTotalEntidad(
						"proveedoProveed", this.idempresa);
				this.totalPaginas = (this.totalRegistros / this.limit) + 1;
				if (this.totalPaginas < this.paginaSeleccion)
					this.paginaSeleccion = this.totalPaginas;
				this.offset = (this.paginaSeleccion - 1) * this.limit;
				...
				this.proveedoProveedList = proveedoProveed
						.getProveedoProveedAll(this.limit, this.offset,
