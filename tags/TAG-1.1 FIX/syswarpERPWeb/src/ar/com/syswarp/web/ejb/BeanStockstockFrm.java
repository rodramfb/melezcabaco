...
public class BeanStockstockFrm implements SessionBean, Serializable {
	private SessionContext context;

	private BigDecimal idempresa = new BigDecimal(-1);

	static Logger log = Logger.getLogger(BeanStockstockFrm.class);
	...
	private Double kilaje = Double.valueOf("0"); 
	...
	public void ejecutarSentenciaDML() {
		try {
			...
			if (this.accion.equalsIgnoreCase("alta"))
				this.mensaje = Stockstock.StockstockCreate(this.codigo_st,
						this.alias_st, this.descrip_st, this.descri2_st,
						this.cost_pp_st, this.precipp_st, this.cost_uc_st,
						this.ultcomp_st, this.cost_re_st, this.reposic_st,
						this.nom_com_st, this.grupo_st, this.cantmin_st,
						this.unimed_st, this.bonific_st, this.impint_st,
						this.impcant_st, this.cuencom_st, this.cuenven_st,
						this.cuenve2_st, this.cuencos_st, this.cuenaju_st,
						this.inventa_st, this.proveed_st, this.provart_st,
						this.id_indi_st, this.despa_st, this.marca_st,
						this.cafecga_st, this.unialt1_st, this.unialt2_st,
						this.unialt3_st, this.unialt4_st, this.factor1_st,
						this.factor2_st, this.factor3_st, this.factor4_st,
						this.tipoiva_st, this.venta_st, this.compra_st,
						this.esquema_st, this.usuarioalt, this.idempresa ,
						this.kilaje);
			else if (this.accion.equalsIgnoreCase("modificacion"))
				
				this.mensaje = Stockstock.StockstockUpdate(this.codigo_st,
						this.alias_st, this.descrip_st, this.descri2_st,
						this.cost_pp_st, this.precipp_st, this.cost_uc_st,
						this.ultcomp_st, this.cost_re_st, this.reposic_st,
						this.nom_com_st, this.grupo_st, this.cantmin_st,
						...
						this.tipoiva_st, this.venta_st, this.compra_st,
						this.esquema_st, 
						this.usuarioact, this.idempresa, this.kilaje);
			else if (this.accion.equalsIgnoreCase("baja"))
				this.mensaje = Stockstock.StockstockDelete(this.codigo_st,
						this.idempresa);
		} catch (Exception ex) {
	...
	public void getDatosStockstock() {
		try {
			Stock Stockstock = Common.getStock();
			List listStockstock = Stockstock.getStockstockPK(this.codigo_st,
					this.idempresa);
			...
			if (iterStockstock.hasNext()) {
				...
				this.cost_pp_st = Double.valueOf(uCampos[4]);
				this.precipp_st = Double.valueOf(uCampos[5]);
				this.cost_uc_st = Double.valueOf(uCampos[6]);
				this.ultcomp_st = Double.valueOf(uCampos[7]);
				this.cost_re_st = Double.valueOf(uCampos[8]);
				this.reposic_st = Double.valueOf(uCampos[9]);
				this.nom_com_st = uCampos[10];
				this.d_nom_com_st = uCampos[11];
				this.grupo_st = uCampos[12];
				this.d_grupo_st = uCampos[13];
				this.cantmin_st = Double.valueOf(uCampos[14]);
				this.unimed_st = uCampos[15];
				this.d_unimed_st = uCampos[16];
				this.bonific_st = Double.valueOf(uCampos[17]);
				this.impint_st = Double.valueOf(uCampos[18]);
				this.impcant_st = Double.valueOf(uCampos[19]);
				...
				this.cafecga_st = Double.valueOf(uCampos[32]);
				...
				this.factor1_st = Double.valueOf(uCampos[41]);
				this.factor2_st = Double.valueOf(uCampos[42]);
				this.factor3_st = Double.valueOf(uCampos[43]);
				this.factor4_st = Double.valueOf(uCampos[44]);
				this.tipoiva_st = uCampos[45];
				this.d_tipoiva_st = uCampos[46];
				this.venta_st = uCampos[47];
				this.compra_st = uCampos[48];
				this.esquema_st = BigDecimal.valueOf(Long.parseLong(uCampos[49]));
				this.kilaje=Double.valueOf(uCampos[54]);
				this.tupla = uCampos[55];
			} else {
	...
	public boolean ejecutarValidacion() {
		try {
			if (!this.volver.equalsIgnoreCase("")) {
				response.sendRedirect("StockstockAbm.jsp");
				return true;
			}
			if (!this.validar.equalsIgnoreCase("")) {
				if (!this.accion.equalsIgnoreCase("baja")) {
					// 1. nulidad de campos
					if (descrip_st == null) {
						this.mensaje = "No se puede dejar vacio el campo descrip_st ";
						return false;
					}
					// 2. len 0 para campos nulos
					if (descrip_st.trim().length() == 0) {
						this.mensaje = "No se puede dejar vacio el campo Descripcion  ";
						return false;
					}

					if (codigo_st.trim().length() == 0) {
						this.mensaje = "No se puede dejar vacio el campo Codigo  ";
						return false;
					}

					if (alias_st.trim().length() == 0) {
						this.mensaje = "No se puede dejar vacio el campo Alias  ";
						return false;
					}
					if (descri2_st.trim().length() == 0) {
						this.mensaje = "No se puede dejar vacio el campo Descripcion Corta  ";
						return false;
					}

					if (d_grupo_st.trim().length() == 0) {
						this.mensaje = "No se puede dejar vacio el campo Grupo  ";
						return false;
					}
					
					if(!Common.esEntero(this.tipoiva_st)){
						this.mensaje = "Es necesario seleccionar Tipo de Iva.";
					    return false;						
					}
					
					if(!Common.esEntero(this.unimed_st)){
						this.mensaje = "Es necesario seleccionar Unidad de Medida.";
					    return false;						
					}
					
					if(Common.setNotNull(this.id_indi_st).equals("")){
						this.mensaje = "Es necesario seleccionar si es serializable o no.";
						return false;
					}
					
					if(Common.setNotNull(this.despa_st).equals("")){
						this.mensaje = "Es necesario seleccionar si requiere nro. de despacho.";
						return false;
					}
	...
	public Double getKilaje() {
		return kilaje;
	}

	public void setKilaje(Double kilaje) {
		this.kilaje = kilaje;
	}
