...
public class BeanCrmllamadosFrm implements SessionBean, Serializable {
	private SessionContext context;

	static Logger log = Logger.getLogger(BeanCrmllamadosFrm.class);
	...
	public boolean ejecutarValidacion() {
		try {
			...
			if (!this.volver.equalsIgnoreCase("")) {
				response.sendRedirect("crmllamadosAbm.jsp");
				return true;
			}
			if (!this.validar.equalsIgnoreCase("")) {
				if (!this.accion.equalsIgnoreCase("baja")) {
					...
					if (idtipollamada == null || idtipollamada.longValue() < 0) {
						this.mensaje = "No se puede dejar vacio el campo tipollamada ";
						return false;
					}
					if (idusuario == null) {
