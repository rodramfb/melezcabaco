...
public class BeanClienteslistasdepreciosAbm implements SessionBean,
		Serializable {
	static Logger log = Logger.getLogger(BeanClienteslistasdepreciosAbm.class);
	...
	public BeanClienteslistasdepreciosAbm() {
		super();
	}
		...
		try {
			...
			if (this.accion.equalsIgnoreCase("modificacion")) {
				if (idlista == null
					...
					dispatcher = request
							.getRequestDispatcher("clienteslistasdepreciosFrm.jsp");
			...
			if (buscar) {
				String[] campos = { "descri_lis", "codigo_st" };
				this.totalRegistros = clienteslistasdeprecios
						.getTotalEntidadOcu("vlistasprecios", campos,
				...
				this.clienteslistasdepreciosList = clienteslistasdeprecios
						.getClienteslistasdepreciosOcu(this.limit, this.offset,
								this.ocurrencia, this.idempresa);
			} else {
				this.totalRegistros = clienteslistasdeprecios.getTotalEntidad(
						"vlistasprecios", this.idempresa);
				this.totalPaginas = (this.totalRegistros / this.limit) + 1;
				if (this.totalPaginas < this.paginaSeleccion)
					this.paginaSeleccion = this.totalPaginas;
				this.offset = (this.paginaSeleccion - 1) * this.limit;
				...
				this.clienteslistasdepreciosList = clienteslistasdeprecios
						.getClienteslistasdepreciosAll(this.limit, this.offset,
								this.idempresa);
			}
