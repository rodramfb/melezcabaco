...
public class BeanPedidos_cabeFrmBORRAR_20090825 implements SessionBean,
		Serializable {
	private SessionContext context;

	static Logger log = Logger
			.getLogger(BeanPedidos_cabeFrmBORRAR_20090825.class);
	...
	// 20090825 - EJV
	private BigDecimal totalGeneralIva_NEW = new BigDecimal(0);
	...
	public BeanPedidos_cabeFrmBORRAR_20090825() {
		super();
	}
	...
	public void ejecutarSentenciaDML() {
		try {
			...
			resultado = pedidos_cabe.pedidosNormalesCreate(null, this.idestado,
					this.idcliente, this.idsucursal, this.iddomicilio,
					...
					this.obsarmado, this.obsentrega, new BigDecimal(
							this.recargo1), new BigDecimal(this.recargo2),
					...
					new BigDecimal(-10), new BigDecimal(-1), null, null, null, this.usuarioalt,
					this.idempresa, null);
		...
		try {
			...
			if (this.idcondicion.longValue() == 5) {
				...
				if (this.listaTarjetasCredito != null
						...
						&& this.listaTarjetasCredito.size() == 1) {

					java.sql.Date hoy = new java.sql.Date(Calendar
							.getInstance().getTimeInMillis());
					String datos[] = (String[]) this.listaTarjetasCredito
							.get(0);
	...
	private void llenarListas(Clientes clientes) {
		...
		try {
			...
			this.listaTarjetasCredito = clientes.getClienteTarjetasCliente(
					cuantos, desde, this.idcliente, this.idempresa);
	...
	private void getDatosCliente(Clientes clientes) {
		try {
			 ...
			 * 27-cl.idempresa
			 * ,....................................................
			 * 28-cl.usuarioalt,cl.usuarioact,cl.fechaalt,cl.fechaact
			 */
	...
	private void getDomiciliosCliente(Clientes clientes) {
		try {
			...
			this.listaDomicilios = clientes.getClientesDomiciliosCliente(50, 0,
		...
		try {
			...
			if (ht != null) {
				...
				while (en.hasMoreElements()) {
					...
					// 20090825 - EJV
					BigDecimal ivaArticulo = new BigDecimal(datos[23]);
					BigDecimal iva = ivaArticulo.floatValue() != 0f ? ivaArticulo
							: ivaArticulo;
					this.totalGeneralIva_NEW = totalGeneralIva_NEW
							.add(new BigDecimal(datos[indice]));
				...
				for (int i = 0; i < this.keyHashArticulo.length; i++) {
					if (ht.containsKey(this.keyHashArticulo[i])) {
						...
						if (this.iddescuento_apli[i].equals("-1")) {
							...
							if (Float.parseFloat(this.porcdesc_apli[i]) != 0) {
