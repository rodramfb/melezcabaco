...
public class BeanClientesPromocionesAbm implements SessionBean, Serializable {

	static Logger log = Logger.getLogger(BeanClientesPromocionesAbm.class);
	...
	public BeanClientesPromocionesAbm() {
		super();
	}
		...
		try {
			...
			if (this.accion.equalsIgnoreCase("modificacion")) {
				if (idpromocion.longValue() < 0) {
					...
					dispatcher = request
							.getRequestDispatcher("clientesPromocionesFrm.jsp");
			...
			if (buscar) {
				String[] campos = { "idpromocion", "promocion" };
				this.totalRegistros = clientes.getTotalEntidadOcu(
						"clientesPromociones", campos, this.ocurrencia,
						this.idempresa);
				...
				this.clientesPromocionesList = clientes
						.getClientesPromocionesOcu(this.limit, this.offset,
								this.ocurrencia, this.idempresa);
			} else {
				this.totalRegistros = clientes.getTotalEntidad(
						"clientesPromociones", this.idempresa);
				this.totalPaginas = (this.totalRegistros / this.limit) + 1;
				if (this.totalPaginas < this.paginaSeleccion)
					this.paginaSeleccion = this.totalPaginas;
				this.offset = (this.paginaSeleccion - 1) * this.limit;
				...
				this.clientesPromocionesList = clientes
						.getClientesPromocionesAll(this.limit, this.offset,
								this.idempresa);
			}
