...
public class BeanVPedidosEstadoAbm implements SessionBean, Serializable {

	static Logger log = Logger.getLogger(BeanVPedidosEstadoAbm.class);
	...
	public boolean ejecutarValidacion() {
		...
		try {
			...
			if (this.accion.equalsIgnoreCase("cambiarestado")) {
				dispatcher = request
						.getRequestDispatcher("pedidosCambioEstadoFrm.jsp");
				dispatcher.forward(request, response);

				return true;
			}

			this.estadosList = clie.getPedidosestadosAll(250, 0);
			...
			String filtro = " WHERE tipopedido =  '"
					+ this.tipopedido.toUpperCase()
					+ "'"
			...
			if (!this.filtroIdpedido.equals("")) {
				...
				if (!this.filtroSucursal.equals("") || !this.filtroComprob.equals("")) {
					...
					if (!Common.esEntero(this.filtroSucursal)) {
						...
						this.mensaje = "Comprobante debe ser un valor numerico.";					
						   ...
						   this.filtroSucursal = Common.strZero(this.filtroSucursal, 4);
						   this.filtroComprob = Common.strZero(this.filtroComprob, 8);
						
							auxFiltro += " AND idpedido_cabe IN  ( "

									+ " SELECT d.idpedido_cabe  "
									+ "   FROM pedidos_deta d  "
									+ "        INNER JOIN clientesremitos r ON d.idremitocliente = r.idremitocliente AND d.idempresa = r.idempresa "
									+ "  WHERE r.nrosucursal =  " + Integer.parseInt(this.filtroSucursal) 
			...
			if (fechadesde != null && fechahasta != null) {
				...
				// filtro += " AND fechapedido::DATE BETWEEN '" + fechadesde
			...
			if (buscar) {
				...
				this.totalRegistros = clie.getTotalEntidadFiltro(
						"vPedidosEstado", filtro, this.idempresa);
				...
				if (fechadesde == null || fechahasta == null) {
					this.vPedidosEstadoList = clie.getVPedidosEstadoOcu(
							this.limit, this.offset, this.idestado,
							this.tipopedido, this.ocurrencia, this.idempresa);
				} else {
					this.vPedidosEstadoList = clie
							.getVPedidosEstadoconFechaOcu(this.limit,
									this.offset, this.idestado,
									this.tipopedido, this.ocurrencia,
									this.idempresa, fechadesde.toString(),
									fechahasta.toString());
				...
				// if (fechadesde == null || fechahasta == null) {
				// this.vPedidosEstadoList = clie.getVPedidosEstadoAll(
				// this.limit, this.offset, this.idestado,
				// this.tipopedido, this.idempresa);
				// } else {
				// this.vPedidosEstadoList = clie
				// .getVPedidosEstadoconFechaAll(this.limit,
				// this.offset, this.idestado,
				// this.tipopedido, this.idempresa, fechadesde
				// .toString(), fechahasta.toString());
				//
				// }

				this.vPedidosEstadoList = clie.getVPedidosEstadoAll(this.limit,
