...
String color_fondo ="";
String titulo = "Familiares";
// variables de entorno
String pathskin = session.getAttribute("pathskin").toString();
String pathscript = session.getAttribute("pathscript").toString();
// variables de paginacion
int i = 0;
Iterator iterFamiliares   = null;
...
%>
<html>
<jsp:useBean id="BFFF"  class="ar.com.syswarp.web.ejb.BeanCrmfamiliaresLov"   scope="page"/>
...
<title><%=titulo%></title>
 ...
 <script >
function bajarDatos(idfamiliar, nombre){
	for(var i = 0; i<10;i++)
		if(opener.document.forms[i].idfamiliar)
			break;
	opener.document.forms[i].idfamiliar.value = idfamiliar;
	opener.document.forms[i].nombre.value = nombre; 
	window.close();
}
 </script>
</head>
...
tituCol[0] = "Codigo";
tituCol[1] = "Familiares";

java.util.List familiares = new java.util.ArrayList();
familiares= BFFF.getCrmfamiliaresList();
iterFamiliares = familiares.iterator();
%>
<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" >
<div id="overDiv" style="position:absolute; visibility:hidden; z-index:1000;"></div>
<form action="lov_familiar.jsp" method="POST" name="frm">
...
<table width="100%" border="0" cellspacing="1" cellpadding="1" name="rsTable" id=rsTable cols=<%=totCol+2-4%>  >
  <tr class="fila-encabezado">
   ...
   <%int r = 0;
   while(iterFamiliares.hasNext()){
      ++r;
      String[] sCampos = (String[]) iterFamiliares.next(); 
      // estos campos hay que setearlos segun la grilla 
      if (color_fondo.equals("fila-det-verde")) color_fondo = "fila-det";
      else color_fondo = "fila-det-verde";%>
   <tr onMouseOver="mOvr(this,this.className='fila-det-rojo');" onMouseOut="mOut(this,this.className='<%=color_fondo%>');" class="<%=color_fondo%>" scope="col" > 
      <td class="fila-det-border" ><img src="../imagenes/default/audit.gif" width="21" height="17" onClick="bajarDatos('<%=sCampos[0]%>', '<%=sCampos[3]%>' )"></td>
...
<%
   ...
   <input name="idindividuos" value="<%= BFFF.getIdindividuos() %>" type="hidden">
   </form>
  </body>
</html>
<% 
