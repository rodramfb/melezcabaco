...
function validarCampos() {
	...
	if (document.forms[0].ctaactivo42.value == "") {
...
function passBack(ctaactivo4, d_activo4) {  
  var opcion = "<%= opcion %>";
  var id     =  "<%= id %>";
  if(opcion == "byId"){
    opener.document.getElementById(id).value = ctaactivo4 ;
		id = (parseInt(id)+1) + "" ;
    opener.document.getElementById(id).value = d_activo4 ;		
  }
	else{
		var i = 0;
		 for(i=0;i<10;i++)
			 if(opener.document.forms[i].ctaactivo4)
				 break;    
		 opener.document.forms[i].ctaactivo4.value = ctaactivo4; 
	}
   close();
}
...
<hr>
<form action="lov_cuentascontables_ctaactivo4.jsp">
  <table width="59%" border="0" cellspacing="0" cellpadding="0" height="23">
    <tr> 
      ...
      <td width="54%" height="39"> 
        <input name="ctaactivo42" type="text" class="campo" id="ctaactivo42" value="<%=ctaactivo4%>" size="25">
      </td>
...
if (!ctaactivo42.equals("*")) {
  ...
  <table width="100%"  cellPadding=0 cellSpacing=0  border="0">
	...
	<input type="hidden" name="ctaactivo4" value="<%=ctaactivo4%>">
</form>
