...
function validarCampos() {
	...
	if (document.forms[0].ctaretiva2.value == "") {
...
function passBack(ctaretiva, d_ctaretiva) {  
  var opcion = "<%= opcion %>";
  var id     =  "<%= id %>";
  if(opcion == "byId"){
    opener.document.getElementById(id).value = ctaretiva ;
		id = (parseInt(id)+1) + "" ;
    opener.document.getElementById(id).value = d_ctaretiva ;		
  }
	else{
		var i = 0;
		 for(i=0;i<10;i++)
			 if(opener.document.forms[i].ctaretiva)
				 break;    
		 opener.document.forms[i].ctaretiva.value = ctaretiva; 
	}
   close();
}
...
<hr>
<form action="lov_cuentascontables_ctaretiva.jsp">
  <table width="59%" border="0" cellspacing="0" cellpadding="0" height="23">
    <tr> 
      ...
      <td width="54%" height="39"> 
        <input name="ctaretiva2" type="text" class="campo" id="ctaretiva2" value="<%=ctaretiva%>" size="25">
      </td>
...
if (!ctaretiva2.equals("*")) {
  ...
  <table width="100%"  cellPadding=0 cellSpacing=0  border="0">
	...
	<input type="hidden" name="ctaretiva" value="<%=ctaretiva%>">
</form>
