...
function validarCampos() {
	...
	if (document.forms[0].ctaactivo22.value == "") {
...
function passBack(ctaactivo2, d_activo2) {  
  var opcion = "<%= opcion %>";
  var id     =  "<%= id %>";
  if(opcion == "byId"){
    opener.document.getElementById(id).value = ctaactivo2 ;
		id = (parseInt(id)+1) + "" ;
    opener.document.getElementById(id).value = d_activo2 ;		
  }
	else{
		var i = 0;
		 for(i=0;i<10;i++)
			 if(opener.document.forms[i].ctaactivo2)
				 break;    
		 opener.document.forms[i].ctaactivo2.value = ctaactivo2; 
	}
   close();
}
...
<hr>
<form action="lov_cuentascontables_ctaactivo2.jsp">
  <table width="59%" border="0" cellspacing="0" cellpadding="0" height="23">
    <tr> 
      ...
      <td width="54%" height="39"> 
        <input name="ctaactivo22" type="text" class="campo" id="ctaactivo22" value="<%=ctaactivo2%>" size="25">
      </td>
...
if (!ctaactivo22.equals("*")) {
  ...
  <table width="100%"  cellPadding=0 cellSpacing=0  border="0">
	...
	<input type="hidden" name="ctaactivo2" value="<%=ctaactivo2%>">
</form>
