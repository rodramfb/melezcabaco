...
%>
<html>
<jsp:useBean id="BFFF"  class="ar.com.syswarp.web.ejb.BeanStockFamiliasgruposLov" scope="page"/>
...
<title><%=titulo%></title>
 ...
 <script >
function bajarDatos(grupo_st, d_grupo_st){
	for(var i = 0; i<10;i++)
		if(opener.document.forms[i].grupo_st)
			break;
	opener.document.forms[i].grupo_st.value = grupo_st;
	opener.document.forms[i].d_grupo_st.value = d_grupo_st; 
	window.close();
}
 </script>
</head>
...
<form action="lov_FamiliaGrupo.jsp" method="POST" name="frm">
...
<%
   ...
   <input name="codigo_fm" type="hidden" id="codigo_fm" value="<%=BFFF.getCodigo_fm()%>">
</form> 
  </body>
</html>
<% 
