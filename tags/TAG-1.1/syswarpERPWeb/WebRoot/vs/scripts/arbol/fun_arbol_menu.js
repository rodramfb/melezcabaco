 ...
 * DATE :20080722
 * OBSERVACIONES: 
 * Modificar funcion original, para manejar dinamicamente las imagenes por cada nodo 'FOLDER'
 * function Folder(folderDescription, hreference) //constructor 
 */
function Folder(folderDescription, hreference, imgFOpen, imgFClosed) //constructor 
...
{ 
  ...
  // EJV:20080722
  if(!imgFOpen || imgFOpen == '')
	imgFOpen='ftv2folderopen.gif';
  // EJV:20080722
  if(!imgFClosed  || imgFClosed == '')
    imgFClosed='ftv2folderclosed.gif';
  /*
  // EJV:20080722
  this.iconSrc = ICONPATH + "ftv2folderopen.gif";
  this.iconSrcClosed = ICONPATH + "ftv2folderclosed.gif";
  */  
  this.iconSrc = ICONPATH + imgFOpen;
  this.iconSrcClosed = ICONPATH + imgFClosed;
...
{ 
  ...
  { 
    ...
    docW = docW + "<font size=\"1\" face=\"verdana, Helvetica\">  " + this.desc + "</font></a>"
  } 
  else 
    docW = docW + "<font size=\"1\" face=\"verdana, Helvetica\">" + this.desc + "</font>" 
  docW = docW + "</td>"
...
function finalizeCreationOfItem(itemArray)
{
  ...
  // EJV:20080722
  // Ahora estos valores son seteados dinamicamente
  //itemArray.iconSrc = ICONPATH + "ftv2doc.gif" 
  //itemArray.iconSrc = ICONPATH + "ftv2doc_old.gif" 
...
{ 
  ...
  if (USETEXTLINKS) 
    docW = docW + "<a    " + fullLink + " id=\"itemTextLink"+this.id+"\">  <font size=\"1\" face=\"verdana, Helvetica\">" + this.desc + "</font></a>"
  else 
    docW = docW +  "<font size=\"1\" face=\"verdana, Helvetica\">" +  this.desc + "</font>"
...
//    alert("Treeview was not able to find the node object corresponding to ID=" + folderId + ". If the configuration file sets a.xID values, it must set them for ALL nodes, including the foldersTree root.")
    alert("No fue posible encontrar el nodo correspondiente al ID=" + folderId + ".")
	return;
  }
...
function finalizeCreationOfChildDocs(folderObj) {
  for(i=0; i < folderObj.nChildren; i++)  {
    ...
    if (typeof child[0] != 'undefined')
    {
      ...
      child.desc = child[0];
	  // EJV:20080722
	  // Seteo dinamico de imagenes, child solo manejaba dos elementos, ahora son tres .
	  child.iconSrc = ICONPATH + child[2];
	  /* for (var j=0;i==0&&j<child.length;j++) alert("child[" +j+ "]: " + child[j] + " | ***** | child.iconSrc: " + child.iconSrc  );*/
  ...
  if (browserVersion == 1 || browserVersion == 3) {
    ...
    if (clickedDOMObj != null) {
        if (lastClicked != null) {
			...
			//clickedDOMObj.style.fontSize=1;
            prevClickedDOMObj.style.color=lastClickedColor;
            prevClickedDOMObj.style.backgroundColor=lastClickedBgColor;
			//prevClickedDOMObj.style.fontWeight = 'bold';
        }
...
function gFld(description, hreference, imgFopen, imgFclosed) 
{ 
  folder = new Folder(description, hreference, imgFopen, imgFclosed);
  return folder;
} 
...
{ 
  if (optionFlags>=0) { //is numeric (old style) or empty (error)
    ...
    alert("Cambie el archivo de configuracion del arbol para usar un estilo nuevo de target.");
    return;
  }
...
function getElById(idVal) {
  ...
  alert("Error recuperando elemento por ID.")
  return null
}
      ...
      if (fldStr != "") {
        ...
        if (nodeObj!=null) //may have been deleted
          if (nodeObj.setState) {
            ...
            alert("Id interno no apunta a carpeta alguna.")			
      }
    }
  }
}
...
var PRESERVESTATE = 0;
var ICONPATH = 'vs/scripts/arbol/';
var HIGHLIGHT = 0;
var HIGHLIGHT_COLOR = 'black';
var HIGHLIGHT_BG    = '#FF9966';
var BUILDALL = 0;
var GLOBALTARGET = "basefrm";  // variable only applicable for addChildren uses
