...
%>
<html>
<jsp:useBean id="BFFF"  class="ar.com.syswarp.web.ejb.BeanSuborigenLov"   scope="page"/>
...
<title><%=titulo%></title>
 ...
 <script >
function bajarDatos(idsuborigen, suborigen){
	for(var i = 0; i<10;i++)
		if(opener.document.forms[i].idsuborigen)
			break;
	opener.document.forms[i].idsuborigen.value = idsuborigen;
	opener.document.forms[i].suborigen.value = suborigen; 
	window.close();
}
 </script>
</head>
...
<form action="lov_suborigen.jsp" method="POST" name="frm">
...
<%
   ...
   <input name="idorigen" value="<%=  BFFF.getIdorigen() %>" type="hidden">
   </form>
  </body>
</html>
<% 
