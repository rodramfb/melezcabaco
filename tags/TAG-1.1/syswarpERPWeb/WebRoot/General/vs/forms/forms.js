...
var varAlfa = "qwertyuiop?lkjhgfdsazxcvbnm ";
var varChar = "!|\"?#$%&/()=??\\?`+-*:;_";
...
function validarDV(objNro) {
   ...
   if (dv.length != 1) {
      alert("El digito verificador no tiene una 1 posici?n");
		objNro.select();
		objNro.focus();
      return false;
   }
...
//Modificaci?n del original para mantener el estilo de la tabla
	for (i=1;i<totalRows;i++)
...
function validarNumerico(obj,iTotal,iDecim) {
	...
	if (iDecim>0) 	{
		...
		if ( str.indexOf(".")==0 && len == 1 ) mensaje = "Formato inv?lido.";
		//alert(  str.indexOf(".") + " - " + ( len-str.indexOf(".")-1  ) + " - ");
		if ((str.indexOf(".") ==-1) && (len>iTotal)) mensaje="Desbordamiento de parte entera";
	}					
	else {
		cntP=1;
		if (len>iTotal) mensaje="Desbordamiento de tama?o num?rico";
	}		
	if ((iTotal+iDecim+1-cntP)<len) mensaje="Desbordamiento de tama?o num?rico";
	...
	if (cntP>1)	mensaje="Campo num?rico";
	if (mensaje.length>0) {
...
/* PFS 16-12-2003 Funci?n getYear devuelve 2 caracteres para a?os previos a 1999*/
...
function abreVentanaNombre( pagina, ventana, ancho, alto) {
	frmLOV = open(pagina ,ventana ,'scrollbars=yes,resizable=yes,width='+ancho+',height='+alto+',status=yes');
	if (frmLOV.opener == null) 
		frmLOV.opener = self;
}	
...
DATE: 20090611
AUTOR: EJV
OBJETIVO: Funcion generica para setear con un determinado valor, 
          en general se utiliza para limpiar objetos del tipo HIDDEN, TEXT o TEXTAREA que se cargan desde LOV's.
          Recibe como parametro el par campo - valor en una misma variable, separados por el caracter | "PIPE".
          Puede recibir tantos parametros como objetos se desee setear, ya que itera sobre el objeto ARGUMENTS de 
          si misma. 
...
function setObjText(){
	...
	var parObjValor = new Array(); 
		for (var i = 0; i < arguments.length; i++){
			parObjValor = arguments[i].split("|");
			eval (strFrm + parObjValor[0] + ".value = " + parObjValor[1] );
		}
}
//  End -->
