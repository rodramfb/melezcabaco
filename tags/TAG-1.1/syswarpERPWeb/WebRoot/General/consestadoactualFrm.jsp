...
<html>
  ...
  INSTANCIAR BEAN
	--%>  
  <jsp:useBean id="BUF"  class="ar.com.syswarp.web.ejb.BeanConsultaEstadosActualFrm" scope="page"/>
	<head>
  ...
  <%
	...
	<form action="consestadoactualFrm.jsp" method="post" name="frm">
	<input name="accion" type="hidden" value="<%=BUF.getAccion()%>" > 
	<input name="idcliente" type="hidden" value="<%=BUF.getIdcliente()%>" >	
	<input name="idtipoclie" type="hidden" value="<%=BUF.getIdtipoclie()%>" >	
<table width="90%"  border="0" cellspacing="0" cellpadding="0" align="center">
  <tr>
    <td>
			<table width="100%"  border="0" cellspacing="0" cellpadding="0">
				<tr class="text-globales">
					<td>Cliente Nº:<%=BUF.getIdcliente()  +  " - " + BUF.getRazon()%> </td>
				</tr>
			</table>
			<table width="100%"  border="0" cellspacing="0" cellpadding="0">
			...
			<tr  class="fila-det">
			  ...
			  <td class="fila-det-border"><select name="idpromocion" id="idpromocion" class="campo" style="width:200px" >
                <option value="-1">Seleccionar</option>
                <%
                   Iterator iter = BUF.getListPromoActiva().iterator();   
                    while(iter.hasNext()){
                     String[] datos = (String[]) iter.next();
                     %>
                <option value="<%= datos[0] %>" <%=  BUF.getIdpromocion().toString().equals(datos[0]) ? "selected" : "" %>><%= datos[1] %></option>
                <% 
                   } %>
              </select></td>
			  </tr>
