...
String color_fondo ="";
String titulo = "Movimientos de Valores";
...
%>
<html>
<jsp:useBean id="BCOM"  class="ar.com.syswarp.web.ejb.BeanCajaOtrosMovimientos"   scope="page"/>
<%-- EJECUTAR SETEO DE PROPIEDADES --%>
<jsp:setProperty name="BCOM" property="*" />
<%
 BCOM.setResponse(response);
 BCOM.setRequest(request);
 BCOM.setSession(session);
 BCOM.setUsuarioalt(usuario);
 BCOM.setIdempresa( new BigDecimal( session.getAttribute("empresa").toString() ));
 BCOM.ejecutarValidacion();
...
<title><%=titulo%></title>
 ...
 function getContrapartida(url, nombre, ancho, alto){ 
   var permiteContrapartida = <%= (htOMDocTercerosCancelarINOK != null && htOMDocTercerosCancelarINOK.isEmpty()) %>;
   if(permiteContrapartida){
     abrirVentana(url, nombre, ancho, alto);
   }else{
     alert("Operación no permitida, cargo documentos de terceros, \nel detalle de salida se genera automáticamente.\nPara poder cargar contrapartida, primero elimine \nlos documentos de terceros de la selección de Entrada.");
   }
 }
...
<form action="cajaOtrosMovimientos.jsp" method="POST" name="frm"> 
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  <tr >
   <td class="fila-det-bold-rojo"><jsp:getProperty name="BCOM" property="mensaje"/>
     <% if(!BCOM.getNrointdeb().equals("")) {%>
     <img src="../imagenes/default/gnome_tango/apps/pdf.jpg" width="20" height="20" onClick="abrirVentana('../reportes/jasper/generaPDF.jsp?comprob_mt=<%= BCOM.getNrointdeb() %>&plantillaImpresionJRXML=caja_otr_movs_frame', 'otros', '800', '450');" style="cursor:pointer">
     <% } %></td>
  </tr>
</table>
&nbsp;
<table width="98%"  border="1" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0"   >
        ...
        <tr class="text-globales">
          <td width="22%" class="fila-titulo-dos">Detalle de Entrada </td>
          <td width="78%" class="fila-titulo-dos">
            ...
            <input name="btndescuentos2" type="button" class="boton" id="btndescuentos2" style="cursor:pointer" onClick="abrirVentana('lov_cajaIdentificadoresEntradaOtrosMov.jsp?propio=N', 'descuentos', 800, 600)" value=">>"></td>
        </tr>
        <tr >
		...
		while (en.hasMoreElements()) {
			...
			//  System.out.println(" ~^~( IDENTIFICADORES[" + m + "]: " + identificadores[m]);		
              ...
              <tr class="fila-det" >
                ...
                <td width="30%" class="fila-det-border">&nbsp;<%= identificadores[29] %></td>
                <td width="32%" class="fila-det-border">&nbsp;<%= nro_cuenta.length() > 50 ? nro_cuenta.substring(0, 50  ) : nro_cuenta  %></td>
                <td width="10%" class="fila-det-border">&nbsp;<%= identificadores[31] %></td>
...
<!-- DESDE AQUI -->
	...
	if(htOMDocTercerosCancelarINOK != null && !htOMDocTercerosCancelarINOK.isEmpty()){
		Enumeration en = htOMDocTercerosCancelarINOK.keys();
		while (en.hasMoreElements()) {
			String key = (String) en.nextElement();
			String [] movimientos = (String []) htOMDocTercerosCancelarINOK.get(key);
      String nro_cuenta = (movimientos[4] + "-" + movimientos[34]);	
			//for(int m=0;m<movimientos.length;m++)
			//  System.out.println(" ~~>( movimientos[" + m + "]: " + movimientos[m]);	
						  %>
              <tr class="fila-det" >
                <td height="32" class="fila-det-border"> <%= movimientos[2]  %> - <%= movimientos[1] %> &nbsp;</td>
                <td class="fila-det-border"><%= movimientos[30] %>&nbsp;</td>
                <td class="fila-det-border"><%= movimientos[29] %></td>
                <td class="fila-det-border"><%= nro_cuenta.length() > 50 ? nro_cuenta.substring(0, 50  ) : nro_cuenta  %></td>
...
<!-- HASTA AQUI-->
  ...
  <tr>
    <td>
      <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0"   >
        ...
        <tr class="text-globales">
          <td width="22%" class="fila-titulo-dos">Detalle de Salida </td>
          <td width="78%" class="fila-titulo-dos">
            <!--img src="../imagenes/default/gnome_tango/actions/filefind.png" width="22" height="22" style="cursor:pointer" onClick="abrirVentana('lov_clientesMovCliAbm.jsp?cliente=' + document.frm.idcliente.value + '&razon=' + document.frm.razon.value, 'comprobantes', 700, 400)"-->
            <input name="btncontrapartida" type="button" class="boton" id="btncontrapartida" style="cursor:pointer" onClick="getContrapartida('lov_cajaIdentificadoresSalidaOtrosMov.jsp?propio=N', 'descuentos', 800, 400)" value=">>">
          </td>
        </tr>
        <tr >
          <td colspan="2" class="fila-titulo-dos">
		...
		while (en.hasMoreElements()) {
			...
			String nro_cuenta = (identificadores[4] + "-" + identificadores[37]);	
              ...
              <tr class="fila-det" >
                ...
                <td class="fila-det-border"><%= identificadores[29] %>&nbsp;</td>
  ...
  <tr>
    <td>
      ...
      <tr class="fila-det" >
        <td width="54%" height="29" class="fila-det-border">
				<textarea name="observaciones" cols="80" rows="2" class="campo" ><%= BCOM.getObservaciones() %></textarea> </td>
      </tr>
    </table></td>
  </tr>
	  <tr>
    <td><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" >
      ...
      <tr class="fila-det" >
        ...
        <td class="fila-det-border"><div align="right">  <%= BCOM.getTotalIngresos() %></div></td>
      </tr>

      <tr class="fila-det" >
        <td class="fila-det-border">Total Contrapartida </td>
        <td class="fila-det-border"><div align="right"> <%= BCOM.getTotalContrapartida() %></div></td>
      </tr>

      <tr class="fila-det" >
        <td class="fila-det-border">Total Movimiento </td>
        <td class="fila-det-border"><div align="right"> <%= BCOM.getTotalCobranza() %></div></td>
      </tr>
    </table></td>
  </tr>
  ...
  <tr>
    <td height="33"><span class="fila-titulo-dos">
      <input name="confirmar" type="submit" class="boton" id="confirmar" style="cursor:pointer" value="Confirmar Movimiento" >
    </span></td>
  </tr>
</table>
<p>&nbsp; </p>
