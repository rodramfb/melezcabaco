...
String color_fondo ="";
String titulo = "Condicion de Venta";
// variables de entorno
String pathskin = session.getAttribute("pathskin").toString();
String pathscript = session.getAttribute("pathscript").toString();
// variables de paginacion
int i = 0;
Iterator iterClientescondicion   = null;
...
%>
<html>
<jsp:useBean id="BFFF"  class="ar.com.syswarp.web.ejb.BeanClientesCondicionLov"   scope="page"/>
...
<title><%=titulo%></title>
 ...
 <script >
function bajarDatos(idcondicion, condicion){
	for(var i = 0; i<10;i++){
		...
		if(idcondicion != 5){
          if(opener.document.forms[i].idtarjeta  
		     && opener.document.forms[i].idtarjeta.type == 'hidden') {
		   
		    opener.document.forms[i].idtarjeta.value= -1;
            if(opener.document.forms[i].nrotarjeta) 
		      opener.document.forms[i].nrotarjeta.value= '';
            if(opener.document.forms[i].tarjetacredito) 
		      opener.document.forms[i].tarjetacredito.value= '';
...
java.util.List clientescondicion = new java.util.ArrayList();
clientescondicion= BFFF.getClientescondicionList();
iterClientescondicion = clientescondicion.iterator();
%>
<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" >
<div id="overDiv" style="position:absolute; visibility:hidden; z-index:1000;"></div>
<form action="lov_condventa.jsp" method="POST" name="frm">
...
<table width="100%" border="0" cellspacing="1" cellpadding="1" name="rsTable" id=rsTable cols=<%=totCol+2-4%>  >
  <tr class="fila-encabezado">
     ...
     <td width="92%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><%=tituCol[1]%></td>
   </tr>
   <%int r = 0;
   while(iterClientescondicion.hasNext()){
      ++r;
      String[] sCampos = (String[]) iterClientescondicion.next(); 
