...
String color_fondo ="";
String titulo = "Cobrador";
// variables de entorno
String pathskin = session.getAttribute("pathskin").toString();
String pathscript = session.getAttribute("pathscript").toString();
// variables de paginacion
int i = 0;
Iterator iterClientescobrador   = null;
...
%>
<html>
<jsp:useBean id="BFFF"  class="ar.com.syswarp.web.ejb.BeanClientesCobradorLov"   scope="page"/>
...
function bajarDatos(idcobrador, cobrador){
	for(var i = 0; i<10;i++)
		if(opener.document.forms[i].idcobrador)
			break;
	opener.document.forms[i].idcobrador.value = idcobrador;
	opener.document.forms[i].cobrador.value = cobrador; 
	window.close();
}
 </script>
</head>
...
java.util.List clientescobrador = new java.util.ArrayList();
clientescobrador= BFFF.getClientescobradorList();
iterClientescobrador = clientescobrador.iterator();
%>
<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" >
<div id="overDiv" style="position:absolute; visibility:hidden; z-index:1000;"></div>
<form action="lov_cobrador.jsp" method="POST" name="frm">
...
<table width="100%" border="0" cellspacing="1" cellpadding="1" name="rsTable" id=rsTable cols=<%=totCol+2-4%>  >
  <tr class="fila-encabezado">
   ...
   <%int r = 0;
   while(iterClientescobrador.hasNext()){
      ++r;
      String[] sCampos = (String[]) iterClientescobrador.next(); 
      // estos campos hay que setearlos segun la grilla 
      if (color_fondo.equals("fila-det-verde")) color_fondo = "fila-det";
      else color_fondo = "fila-det-verde";%>
   <tr onMouseOver="mOvr(this,this.className='fila-det-rojo');" onMouseOut="mOut(this,this.className='<%=color_fondo%>');" class="<%=color_fondo%>" scope="col" > 
      <td class="fila-det-border" ><img src="../imagenes/default/audit.gif" width="21" height="17" onClick="bajarDatos('<%=sCampos[0]%>', '<%= Common.setNotNull(sCampos[1]).trim()%>' )"></td>
