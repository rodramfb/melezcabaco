...
String color_fondo ="";
//String titulo = "Expreso - Distribuidor";
String titulo = "Zona - Distribuidor";
// variables de entorno
String pathskin = session.getAttribute("pathskin").toString();
String pathscript = session.getAttribute("pathscript").toString();
// variables de paginacion
int i = 0;
Iterator iterClienteszona   = null;
...
%>
<html>
<jsp:useBean id="BFFF"  class="ar.com.syswarp.web.ejb.BeanClientesExpresoZonaLov"   scope="page"/>
...
<title><%=titulo%></title>
 ...
 <script >
function bajarDatos(idexpresozona, idexpreso, expreso, idzona, zona){
	
	for(var i = 0; i<10;i++){
		if(opener.document.forms[i].idexpresozona){
      opener.document.forms[i].idexpresozona.value = idexpresozona;		
      opener.document.forms[i].expresozona.value = expreso + " / " + zona;		
...
tituCol[1] = "Zona";
//tituCol[2] = "Zona";
tituCol[2] = "Distribuidor";

java.util.List clienteszona = new java.util.ArrayList();
clienteszona= BFFF.getClienteszonaList();
iterClienteszona = clienteszona.iterator();
%>
<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" >
<div id="overDiv" style="position:absolute; visibility:hidden; z-index:1000;"></div>
<form action="lov_expreso_zona.jsp" method="POST" name="frm">
...
<table width="100%" border="0" cellspacing="1" cellpadding="1" name="rsTable" id=rsTable cols=<%=totCol+2-4%>  >
  <tr class="fila-encabezado">
   ...
   <%int r = 0;
   while(iterClienteszona.hasNext()){
      ++r;
      String[] sCampos = (String[]) iterClienteszona.next(); 
      // estos campos hay que setearlos segun la grilla 
      if (color_fondo.equals("fila-det-verde")) color_fondo = "fila-det";
      else color_fondo = "fila-det-verde";%>
   <tr onMouseOver="mOvr(this,this.className='fila-det-rojo');" onMouseOut="mOut(this,this.className='<%=color_fondo%>');" class="<%=color_fondo%>" scope="col" > 
      <td class="fila-det-border" >
	    <img src="../imagenes/default/gnome_tango/actions/down.png" width="18" height="18" onClick="bajarDatos('<%=sCampos[0]%>', '<%=sCampos[1]%>', '<%=sCampos[2]%>', '<%=sCampos[3]%>', '<%=sCampos[4]%>' )">
	  </td>
