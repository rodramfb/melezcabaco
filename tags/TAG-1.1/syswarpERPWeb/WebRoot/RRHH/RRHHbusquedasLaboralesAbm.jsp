...
String color_fondo ="";
String titulo = "Busqueda Laboral";
...
%>
<html>
<jsp:useBean id="BRRHHLA"  class="ar.com.syswarp.web.ejb.BeanRRHHbusquedasLaboralesAbm"   scope="page"/>
<%-- EJECUTAR SETEO DE PROPIEDADES --%>
<jsp:setProperty name="BRRHHLA" property="*" />
<%
 BRRHHLA.setResponse(response);
 BRRHHLA.setRequest(request); 
 BRRHHLA.setIdempresa( new BigDecimal( session.getAttribute("empresa").toString() ));
...
tituCol[2] = "Fecha busqueda desde";
tituCol[3] = "Fecha busqueda hasta";
tituCol[4] = "Seniority";
...
<form action="RRHHbusquedasLaboralesAbm.jsp" method="POST" name="frm">
                                   ...
                                   <td>
                                     <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                        <tr class="text-globales">
                                          ...
                                          <td width="23%">&nbsp;Total de registros:&nbsp;<%=BRRHHLA.getTotalRegistros()%></td>
                                          <td width="11%" >Visualizar:</td>
                                          <td width="11%">
                                             <select name="limit" >
                                                <%for(i=15; i<= 150 ; i+=15){%>
                                                    <%if(i==BRRHHLA.getLimit()){%>
                                                        <option value="<%=i%>" selected><%=i%></option>
                                                    <%}else{%>
                                                        <option value="<%=i%>"><%=i%></option>
                                                    <%}
                                                      if( i >= BRRHHLA.getTotalRegistros() ) break;
                                                    %>
                                                <%}%>
                                                <option value="<%= BRRHHLA.getTotalRegistros()%>">Todos</option>
                                             </select>
                                          </td>
                                          <td width="7%">&nbsp;P&aacute;gina:</td>
                                          <td width="12%">
                                             <select name="paginaSeleccion" >
                                                <%for(i=1; i<= BRRHHLA.getTotalPaginas(); i++){%>
                                                    <%if ( i==BRRHHLA.getPaginaSeleccion() ){%>
                                                       <option value="<%=i%>" selected><%=i%></option> 
                                                    <%}else{%>
                                                       <option value="<%=i%>"><%=i%></option>
                                                    <%}%>
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  <tr >
   <td class="fila-det-bold-rojo"><jsp:getProperty name="BRRHHLA" property="mensaje"/></td>
  </tr>
</table>
<table width="100%" border="0" cellspacing="1" cellpadding="1" name="rsTable" id=rsTable cols=<%=totCol+2-4%>  >
  <tr class="fila-encabezado">
   ...
   <tr onMouseOver="mOvr(this,this.className='fila-det-rojo');" onMouseOut="mOut(this,this.className='<%=color_fondo%>');" class="<%=color_fondo%>" scope="col" > 
      <td class="fila-det-border" ><input type="radio" name="idbusquedalaboral" value="<%= sCampos[0]%>"></td>
