...
function validarCampos() {
	...
	if (document.forms[0].idctacont2.value == "") {
...
function passBack(idctacont, d_cContable) {  
  var opcion = "<%= opcion %>";
  var id     =  "<%= id %>";
  if(opcion == "byId"){
    opener.document.getElementById(id).value = idctacont ;
		id = (parseInt(id)+1) + "" ;
    opener.document.getElementById(id).value = d_cContable ;		
		...
		var i = 0;
		 for(i=0;i<10;i++)
			 if(opener.document.forms[i].idctacont)
				 break;    
		 opener.document.forms[i].idctacont.value = idctacont; 
	}
   close();
}
  ...
  <table width="59%" border="0" cellspacing="0" cellpadding="0" height="23">
    <tr> 
      ...
      <td width="54%" height="39"> 
        <input name="idctacont2" type="text" class="campo" value="<%=idctacont2%>" size="25">
      </td>
...
<%  
if (!idctacont2.equals("*")) {
   java.util.List cuentasContables =  new ArrayList();
   Iterator iterCuentasContables=null;
   int totReg  = 0;
   try {
		Contable contable = Common.getContable();
   	cuentasContables =  contable.getCuentasOcu(ejercicioActivo, idctacont2, new BigDecimal( session.getAttribute("empresa").toString() ));
  ...
  <table width="100%"  cellPadding=0 cellSpacing=0  border="0">
	...
	<input type="hidden" name="idctacont" value="<%=idctacont%>">
</form>
