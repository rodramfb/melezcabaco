...
public class BeanClientesRemitosConformar implements SessionBean, Serializable {

	static Logger log = Logger.getLogger(BeanClientesRemitosConformar.class);
	...
	private List listRemitoStatus = new ArrayList();
	...
	public boolean ejecutarValidacion() {
		...
		try {
			...
			if (this.zonaUsuario.longValue() < 0) {
				...
				this.origentransaccion = "EXTRANET";
			...
			if (!this.accion.equalsIgnoreCase("")) {
				...
				} else if (this.idzona.longValue() < 0) {

					this.mensaje = "Seleccione zona a consultar.";

				} else {

					if (this.accion.equalsIgnoreCase("cambiarestado")) {

						if (!this.nroremitocliente
								.equals(this.nroremitoclienteAnt)
								|| !this.nrosucursal
										.equals(this.nrosucursalAnt)) {

							this.mensaje = "Cambiaron los parametros de búsqueda, para cambiar estado debe ejecutar nuevamente esta con los parametros que corresponda.";
							...
							if (this.mensaje.equalsIgnoreCase("OK")) {
								...
								this.mensaje = "Conformacion efectuada con éxito.";
						...
						if (this.listRemitoStatus.isEmpty()) {
							this.mensaje = "Registro inexistente o asignado a otra zona.";
						}
				...
				this.mensaje = "Ingrese parametros para la busqueda.";
	...
	private void getDatosRemito() {

		try {

			this.listRemitoStatus = Common.getClientes()
					.getVClientesRemitosConformacionStatusPK(
							new BigDecimal(this.nrosucursal),
							new BigDecimal(this.nroremitocliente), this.idzona,
							this.idempresa);

		} catch (Exception e) {
			log.error("getDatosRemito(): " + e);
		}
	...
	public List getListRemitoStatus() {
		return listRemitoStatus;
	}

	public void setListRemitoStatus(List listRemitoStatus) {
		this.listRemitoStatus = listRemitoStatus;
	}
