...
public class BeanProveedoMovProvAbm implements SessionBean, Serializable {
	static Logger log = Logger.getLogger(BeanProveedoMovProvAbm.class);
		...
		try {
			if (this.accion.equalsIgnoreCase("baja")) {
				if (nrointerno.longValue() < 0) {
					...
					this.mensaje = proveedoMovProv.proveedoMovProvDelete(
							nrointerno, this.idempresa);
				}
			}
			...
			if (buscar) {
				String[] campos = { "nrointerno", "idproveedor" };
				this.totalRegistros = proveedoMovProv.getTotalEntidadOcu(
						"proveedoMovProv", campos, this.ocurrencia,
						this.idempresa);
				...
				this.proveedoMovProvList = proveedoMovProv
						.getProveedoMovProvOcu(this.limit, this.offset,
								this.ocurrencia, this.idempresa);
			} else {
				this.totalRegistros = proveedoMovProv.getTotalEntidad(
						"proveedoMovProv", this.idempresa);
				this.totalPaginas = (this.totalRegistros / this.limit) + 1;
				if (this.totalPaginas < this.paginaSeleccion)
					this.paginaSeleccion = this.totalPaginas;
				this.offset = (this.paginaSeleccion - 1) * this.limit;
				...
				this.proveedoMovProvList = proveedoMovProv
						.getProveedoMovProvAll(this.limit, this.offset,
								this.idempresa);
			}
