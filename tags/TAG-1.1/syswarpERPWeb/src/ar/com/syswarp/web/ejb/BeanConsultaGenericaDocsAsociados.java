...
public class BeanConsultaGenericaDocsAsociados implements SessionBean,
		Serializable {
	private SessionContext context;

	static Logger log = Logger
			.getLogger(BeanConsultaGenericaDocsAsociados.class);
	...
	private List listEntidadesAsociables = new ArrayList();

	public BeanConsultaGenericaDocsAsociados() {
		super();
	}
	...
	public boolean ejecutarValidacion() {
		try {
			...
			this.listEntidadesAsociables = gral
					.getGlobalentidadesasociablesAll(200, 0, this.idempresa);

			if (this.identidadesasociables.compareTo(new BigDecimal(0)) > 0) {
				...
				if (buscar) {
					...
					String filtro = " WHERE (UPPER(\""
							+ this.columnsMetaData[1][0] + "\"::VARCHAR) LIKE '%";
					...
					this.documentosGenricList = gral
							.getDocumentosGenericosOcu(this.limit,
									this.offset, entidad, filtro.replaceFirst("WHERE", "AND"), 
									this.idempresa);
				} else {

					// TODO: revisar si es necesario sobrecargar getTotalEntidad
					// con
					// idempresa
					...
					this.documentosGenricList = gral.getDocumentosGenericosAll(
				...
				this.totalPaginas = 1;
				this.mensaje = "Seleccionar tipo de registro a visualizar.";
			}
	...
	public List getListEntidadesAsociables() {
		return listEntidadesAsociables;
	}

	public void setListEntidadesAsociables(List listEntidadesAsociables) {
		this.listEntidadesAsociables = listEntidadesAsociables;
	}
