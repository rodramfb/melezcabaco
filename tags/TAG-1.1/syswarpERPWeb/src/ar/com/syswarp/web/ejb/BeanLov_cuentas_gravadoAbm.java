...
public class BeanLov_cuentas_gravadoAbm implements SessionBean, Serializable {
	static Logger log = Logger.getLogger(BeanLov_cuentas_gravadoAbm.class);
	...
	private BigDecimal tipomov = new BigDecimal(0);

	public BeanLov_cuentas_gravadoAbm() {
		super();
	}
	...
	public boolean ejecutarValidacion() {
		...
		try {
			...
			if (((Hashtable) session.getAttribute("htNetoGravadoConfirma")) != null
					&& !((Hashtable) session
							.getAttribute("htNetoGravadoConfirma")).isEmpty()) {

				this.htNetoGravadoConfirma = (Hashtable) session
						.getAttribute("htNetoGravadoConfirma");
			...
			session.setAttribute("htNetoGravadoConfirma",
				...
				if (this.asignarValidar(this.htNetoGravadoConfirma)) {
					...
					if (this.auxTNGravado == this.TNGravado) {
						...
						while (e.hasMoreElements()) {
							...
							System.out.println("elemento: " +this.htNetoGravadoConfirma.get(key));
						}
						session.removeAttribute("htNetoGravado");
						session.removeAttribute("htNetoGravadoConfirma");
						session.setAttribute("htNetoGravado", htAux);
					} else
			...
			if (buscar) {
				...
				this.totalRegistros = lov_cuentas_gravado
						.getTotalEntidadFiltro("contableinfiplan", filtro,
								this.idempresa);
				...
				this.totalRegistros = lov_cuentas_gravado.getTotalEntidad(
						"contableinfiplan", this.idempresa);
				this.totalPaginas = (this.totalRegistros / this.limit) + 1;
				if (this.totalPaginas < this.paginaSeleccion)
					this.paginaSeleccion = this.totalPaginas;
				this.offset = (this.paginaSeleccion - 1) * this.limit;
