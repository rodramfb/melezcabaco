...
public class BeanCajaPagosAbm implements SessionBean, Serializable {
	static Logger log = Logger.getLogger(BeanCajaPagosAbm.class);
	...
	private BigDecimal idproveedor = null;
	...
	public void ejecutarSentenciaDML() {
		try {
			...
			resultado = pagos.pagosMovProveedorCreate(idproveedor,
					fechamovimiento, totalPago, this.htComprobantesProv,
					this.htIdentificaSalidaPagos, this.htMovimientosCancelar,
					observaciones, usuarioalt, this.idempresa);

			if (Common.esNumerico(resultado[0])) {

				if (this.htComprobantesProv.containsKey("ADELANTO")) {
					String[] datosAdelanto = (String[]) this.htComprobantesProv
							.get("ADELANTO");
				...
				this.nrointerno = resultado[0];

				this.mensaje = "Pago generado Nro.: " + resultado[1];
	...
	public boolean ejecutarValidacion() {
		...
		try {
			...
			if (this.primerCarga) {
				...
				session.setAttribute("htComprobantesProvOK", new Hashtable());
			...
			// --->
			if (this.idproveedor != null && this.flagRetencion.equals("0")) {
				List listRetencionesProv = Common.getTesoreria()
						.getRetencionesProveedor(idproveedor, ejercicio,
								idempresa);
			...
			this.totalComprobantes = calculaTotalPosicion(htComprobantesProv, 1);
			...
			if (!this.confirmar.equals("")) {
				...
				if (this.htComprobantesProv == null
						|| this.htComprobantesProv.isEmpty()) {
	...
	private BigDecimal calculaTotalPosicion(Hashtable ht, int posicion) {
		...
		try {
			...
			while (enu.hasMoreElements()) {
				...
				total = total.add(new BigDecimal(Common
						.getNumeroFormateado(Double.parseDouble(datos[posicion]),
								10, 2)));
				// <--
