...
public class BeanClientesPrecargaAbm implements SessionBean, Serializable {
	static Logger log = Logger.getLogger(BeanClientesPrecargaAbm.class);
	...
	public BeanClientesPrecargaAbm() {
		super();
	}
		...
		try {
			if (this.accion.equalsIgnoreCase("baja")) {
				if (idprecarga.longValue() < 0) {
					...
					this.mensaje = clientesClientes.getClientesPrecargaDelete(
			...
			if (accion.equalsIgnoreCase("pasaje")) {
				...
				if (!Common.esEntero(this.ejercicioContable)) {
					...
					this.mensaje = clientesClientes.UpdateClientesPrecarga(
							this.idprecarga, ejercicioActivo, this.usuarioalt,
							this.idempresa);
			...
			if (buscar) {
				...
				this.totalRegistros = clientesClientes.getTotalEntidadOcu(
				...
				this.clientesClientesList = clientesClientes
						.getClientesPrecargaOcu(this.limit, this.offset,
								this.ocurrencia, this.idempresa);
			} else {
				this.totalRegistros = clientesClientes
						.getTotalEntidad("clientesPrecargaclientes");
				this.totalPaginas = (this.totalRegistros / this.limit) + 1;
				if (this.totalPaginas < this.paginaSeleccion)
					this.paginaSeleccion = this.totalPaginas;
				this.offset = (this.paginaSeleccion - 1) * this.limit;
				...
				this.clientesClientesList = clientesClientes
						.getClientesPrecargaAll(this.limit, this.offset,
								this.idempresa);
			}
