...
public class BeanCrmcotizacionesFrm implements SessionBean, Serializable {
	private SessionContext context;

	static Logger log = Logger.getLogger(BeanCrmcotizacionesFrm.class);
	...
	public void getDatosCrmcotizaciones() {
		try {
			...
			if (iterCrmcotizaciones.hasNext()) {
				...
				this.idindividuos = uCampos[15];
				this.razon_nombre = uCampos[16];
				this.idresultadocotizacion = new BigDecimal(
						uCampos[18] != null ? uCampos[18] : "-1");
			} else {
	...
	public boolean ejecutarValidacion() {
		try {
			...
			if (!this.volver.equalsIgnoreCase("")) {
				response.sendRedirect("crmcotizacionesAbm.jsp");
				return true;
			}
			...
			if (!this.validar.equalsIgnoreCase("")) {
				if (!this.accion.equalsIgnoreCase("baja")) {
					...
					if (idtipocotizacion == null) {
						this.mensaje = "No se puede dejar vacio el campo idtipocotizacion ";
						return false;
					}
					if (superficie == null) {
					...
					if (valor_unitario == null) {
						this.mensaje = "No se puede dejar vacio el campo valor_unitario ";
						return false;
					}
					if (valor_total == null) {
						this.mensaje = "No se puede dejar vacio el campo valor_total ";
						return false;
					}
