...
public class BeanLovCajaIdentificadoresSalidaPagos implements SessionBean,
		Serializable {
	static Logger log = Logger
			.getLogger(BeanLovCajaIdentificadoresSalidaPagos.class);
	...
	public BeanLovCajaIdentificadoresSalidaPagos() {
		super();
	}
	...
	public boolean ejecutarValidacion() {
		...
		Hashtable htIdentificaSalidaPagos = (Hashtable) session
				.getAttribute("htIdentificaSalidaPagos") != null ? (Hashtable) session
				.getAttribute("htIdentificaSalidaPagos")
				: (Hashtable) session.getAttribute("htIdentificaSalidaPagosOK");
		...
		try {
			...
			String filtro = " WHERE ididentificador NOT IN "
					...
					+ "         WHERE (UPPER(tipomov) = 'T' AND  propio = 'N') OR UPPER(tipomov) = 'X')";
			if (buscar) {
				...
				this.cajaIdentificadoresList = cajaIdentificadores
						.getLovCajaIdentificaSalidaPagosOcu(this.limit,
				...
				this.cajaIdentificadoresList = cajaIdentificadores
						.getLovCajaIdentificaSalidaPagosAll(this.limit,
			...
			if (this.accion.equalsIgnoreCase("agregar")) {
				...
				if (!this.identificador.equals("")) {
					...
					if (lista.size() > 0) {
						...
						// 20090609 - EJV: Cargaba el mismo identificador solo
						// UNA (1) vez.
						// htIdentificaSalidaPagos.put(this.identificador,
						// datos);

						// 20090609 - EJV: Permitir cargar el mismo
						// identificador N cantidad de veces.
						String key = Calendar.getInstance().getTimeInMillis()
								+ "";
						htIdentificaSalidaPagos.put(key, datos);
				...
				// session.setAttribute("htIdentificaSalidaPagos",
				// htIdentificaSalidaPagos);
			}
			...
			if (this.accion.equalsIgnoreCase("eliminar")) {
				if (this.delKey == null) {
					...
					for (int i = 0; i < this.delKey.length; i++) {
						if (htIdentificaSalidaPagos.containsKey(this.delKey[i])) {
							htIdentificaSalidaPagos.remove(this.delKey[i]);
						}
			...
			this.asignarValidar(htIdentificaSalidaPagos);
	...
	private boolean asignarValidar(Hashtable ht) {
		...
		try {
			...
			String fechaTesoCaja = Common.getGeneral()
					.getValorSetupVariablesNoStatic("tesoFechaCaja",
							this.idempresa);
			java.util.Date fechamovimiento = (java.util.Date) Common
					.setObjectToStrOrTime(fechaTesoCaja, "StrToJUDate");
				...
				for (int i = 0; i < this.keyHashDatosIdentificador.length; i++) {
					...
					if (ht.containsKey(this.keyHashDatosIdentificador[i])) {
						...
						if (!Common.esNumerico(this.importeingreso[i])) {
							...
							// valida - cheque
						} else if (Common.setNotNull(datos[6])
								.equalsIgnoreCase("C")) {
							...
							if (!this.chequenro[i].equals("")
								...
								if (!Common.isFormatoFecha(this.fecha[i])
									...
									mensaje = "Fecha invalida: "
											+ this.fecha[i];
								} else if (((java.util.Date) Common
										.setObjectToStrOrTime(Common
												.setNotNull(this.fecha[i]),
												"StrToJUDate"))
										.before(fechamovimiento)) {

									mensaje = "Fecha de cheque no puede ser menor a fecha de caja: "
											+ fechaTesoCaja;
									fOk = false;
							...
							// valida - tarjeta
						} else if (Common.setNotNull(datos[4])
								.equalsIgnoreCase("T")) {
						...
						if (fOk) {
							...
							log.info("this.importeingreso[i]: "
									+ this.importeingreso[i]);
							...
							datos[28] = Common.getNumeroFormateado(

							Double.parseDouble(this.importeingreso[i]), 10, 2);

							log.info("datos[28]: " + datos[28]);
			...
			if ((this.accion.equalsIgnoreCase("confirmar") || ((ht == null || ht
					.isEmpty()) && this.accion.equalsIgnoreCase("eliminar")))
					&& mensaje.equals("")) {

				session.setAttribute("htIdentificaSalidaPagosOK", ht);
				session.removeAttribute("htIdentificaSalidaPagos");

			} else {
				if (!mensaje.equals(""))
					this.mensaje = mensaje;
				session.setAttribute("htIdentificaSalidaPagos", ht);
			}
