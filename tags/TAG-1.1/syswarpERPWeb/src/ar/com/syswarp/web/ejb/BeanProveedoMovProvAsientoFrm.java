...
public class BeanProveedoMovProvAsientoFrm implements SessionBean, Serializable {
	private SessionContext context;

	private BigDecimal idempresa = new BigDecimal(-1);

	static Logger log = Logger.getLogger(BeanProveedoMovProvAsientoFrm.class);
...
//20110811 - CAMI - Condicion de pago -------->
	...
	public BeanProveedoMovProvAsientoFrm() {
		super();
	}
		...
		try {
			...
			if (this.accion.equalsIgnoreCase("alta")) {
				...
				if (session.getAttribute("flagMovAplicado") == null) {
					...
					resultado = proveedores.capturaComprobantesProvCreate(
								...
								this.ejercicioActivo, usuarioalt, htAsiento,
								htArticulos, this.idempresa,
					...
					if (this.stock_fact.equalsIgnoreCase("C")) { // si es
						...
						log.debug("--------------------------");
						log.debug("importe " + importe.toString());

						resultado = proveedores
								.capturaComprobantesProvContableCreate(
										idproveedor, fechamov, sucursal,
										comprob, tipomov, tipomovs,
										new BigDecimal(importe.toString()),
										new BigDecimal(saldo.toString()),
										idcondicionpago, fecha_subd, retoque,
										fechavto, this.ejercicioActivo,
										usuarioalt, this.conc, this.idcuenta,
										this.valor, this.tipo, this.idempresa,
										this.observacionesContables);
					}
			...
			if (Common.esNumerico(resultado)) {
				...
				dispatcher = request
						.getRequestDispatcher("proveedoMovProvImpresion.jsp?nrointerno="
								+ resultado);
