...
public class BeanProveedo_oc_grupos_cotizacionesAbm implements SessionBean,
		Serializable {
	static Logger log = Logger
			.getLogger(BeanProveedo_oc_grupos_cotizacionesAbm.class);
	...
	public BeanProveedo_oc_grupos_cotizacionesAbm() {
		super();
	}
	...
	public boolean ejecutarValidacion() {
		...
		try {
			if (this.accion.equalsIgnoreCase("baja")) {
				if (idgrupooc == null || idgrupooc.longValue() < 0) {
					this.mensaje = "Debe seleccionar un registro a eliminar.";
				} else {
					this.mensaje = proveedo_oc_grupos_cotizaciones
							.proveedo_oc_grupos_cotizacionesDelete(idgrupooc,
									this.idempresa);
				}
			}
			if (this.accion.equalsIgnoreCase("modificacion")) {
				if (idgrupooc == null || idgrupooc.longValue() < 0) {
					...
					dispatcher = request
							.getRequestDispatcher("proveedo_oc_grupos_cotizacionesFrm.jsp");
			...
			if (buscar) {
				String[] campos = { "idgrupooc", "grupooc" };
				this.totalRegistros = proveedo_oc_grupos_cotizaciones
						.getTotalEntidadOcu("proveedo_oc_grupos_cotizaciones",
				...
				this.proveedo_oc_grupos_cotizacionesList = proveedo_oc_grupos_cotizaciones
						.getProveedo_oc_grupos_cotizacionesOcu(this.limit,
								this.offset, this.ocurrencia, this.idempresa);
			} else {
				this.totalRegistros = proveedo_oc_grupos_cotizaciones
						.getTotalEntidad("proveedo_oc_grupos_cotizaciones",
								this.idempresa);
				this.totalPaginas = (this.totalRegistros / this.limit) + 1;
				if (this.totalPaginas < this.paginaSeleccion)
					this.paginaSeleccion = this.totalPaginas;
				...
				this.proveedo_oc_grupos_cotizacionesList = proveedo_oc_grupos_cotizaciones
						.getProveedo_oc_grupos_cotizacionesAll(this.limit,
