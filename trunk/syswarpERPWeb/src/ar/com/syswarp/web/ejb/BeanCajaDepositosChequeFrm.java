...
public class BeanCajaDepositosChequeFrm implements SessionBean, Serializable {
	private SessionContext context;

	private BigDecimal idempresa = new BigDecimal(-1);

	static Logger log = Logger.getLogger(BeanCajaDepositosChequeFrm.class);
	...
	private Hashtable htChqDepositarOK = null;

	private boolean primeraCarga = true;

	public BeanCajaDepositosChequeFrm() {
		super();
	}
	...
	public void ejecutarSentenciaDML() {
		try {
			...
			if (this.tipodeposito.equalsIgnoreCase("CARTERA")) {
				...
				resultado = caja.generarDepositosCHQCartera(
						identificadorDeposito, identificadorBanco,
						new BigDecimal(importeDeposito), this.fecha_teso,
						this.htChqDepositarOK, cc1Deposito, cc2Deposito,
						cc1Banco, cc2Banco, idempresa, usuarioalt);
				...
				if (this.mensaje.equalsIgnoreCase("OK")) {
					...
					response
							.sendRedirect("cajaDepositosCallImpresion.jsp?depositos="
									+ resultado[1] + "&tipoDeposito=cheque");
				}
				

			} else if (this.tipodeposito.equalsIgnoreCase("CAUCIONAR")) {

				resultado = caja.generarDepositosCHQCaucion(
						identificadorDeposito, identificadorBanco,
						new BigDecimal(importeDeposito), this.fecha_teso,
						this.htChqDepositarOK, cc1Deposito, cc2Deposito,
						cc1Banco, cc2Banco, idempresa, usuarioalt);
				...
				log.info("CAUCIONADOS ....");

				resultado = caja.generarDepositosCHQCaucionados(
						identificadorDeposito, identificadorBanco,
						new BigDecimal(importeDeposito), this.fecha_teso,
						this.htChqDepositarOK, cc1Deposito, cc2Deposito,
						cc1Banco, cc2Banco, idempresa, usuarioalt);
	...
	public boolean ejecutarValidacion() {
		try {
			...
			// TODO : FALTA CUANDO ES PRIMERA VEZ .... !!!
			this.htChqDepositarOK = new Hashtable();

			if (this.primeraCarga
					|| this.accion.equalsIgnoreCase("cambiarTipoDeposito")) {

				session.removeAttribute("htChqDepositarOK");
			...
			if (session.getAttribute("htChqDepositarOK") != null
					&& !((Hashtable) session.getAttribute("htChqDepositarOK"))
							.isEmpty()) {
				this.htChqDepositarOK = (Hashtable) session
						.getAttribute("htChqDepositarOK");
			}
			...
			if (!this.validar.equalsIgnoreCase("")) {
				if (!this.accion.equalsIgnoreCase("baja")) {
					...
					if (this.htChqDepositarOK == null
							|| this.htChqDepositarOK.isEmpty()) {
						this.mensaje = "Es necesario seleccionar cheques en cartera a depositar.";
						return false;
					}
	...
	public Hashtable getHtChqDepositarOK() {
		return htChqDepositarOK;
	}

	public void setHtChqDepositarOK(Hashtable htChqDepositarOK) {
		this.htChqDepositarOK = htChqDepositarOK;
	}
