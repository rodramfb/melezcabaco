/* 
 javabean para la entidad: pedidosestados
 Copyrigth(r) sysWarp S.R.L. 
 Fecha de creacion: Tue Mar 27 11:09:46 GMT-03:00 2007 
 
 Para manejar la pagina: pedidosestadosAbm.jsp
...
public class BeanConsultaPedidosStockNegativo implements SessionBean,
		Serializable {
	static Logger log = Logger
			.getLogger(BeanConsultaPedidosStockNegativo.class);
	...
	private List pedidosList = new ArrayList();
	...
	public BeanConsultaPedidosStockNegativo() {
		super();
	}
	...
	public boolean ejecutarValidacion() {
		...
		this.codigo_st = Common.setNotNull(this.codigo_st);
		try {
			...
			String entidad = "("
					...
					+ " ) st ";
			String filtro = " WHERE cantidad_sb < 1  "
					+ ( !this.codigo_st.equals("") ? " AND codigo_st = '"
							+ this.codigo_st + "'"
							: "");

			if (buscar) {

				filtro += " AND (idcliente::VARCHAR LIKE '%" + this.ocurrencia
						+ "%' OR " + " UPPER(razon) LIKE '%"
				...
				this.pedidosList = clie.getPedidoStockNegativoOcu(this.limit,
						this.offset, this.ocurrencia, this.codigo_st, this.idempresa);
			} else {
				this.totalRegistros = clie.getTotalEntidadFiltro(entidad,
						filtro, this.idempresa);
				this.totalPaginas = (this.totalRegistros / this.limit) + 1;
				...
				this.pedidosList = clie.getPedidoStockNegativoAll(this.limit,
						this.offset, this.codigo_st, this.idempresa);
			}
	...
	public List getPedidosList() {
		return pedidosList;
	}

	public void setPedidosList(List pedidosList) {
