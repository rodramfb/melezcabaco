...
public class BeanLocalidadesLov implements SessionBean, Serializable {
	static Logger log = Logger.getLogger(BeanLocalidadesLov.class);
	...
	public boolean ejecutarValidacion() {
		...
		Stock globalLocalidades = Common.getStock();
		try {
			...
			if (buscar) {
				String[] campos = { "idlocalidad", "localidad","cpostal" };
				this.totalRegistros = globalLocalidades.gettotalgloballocalidadesOcu(
				...
				this.globalLocalidadesList = globalLocalidades
						.getGlobalLocalidadesOcu(this.limit, this.offset,
								this.ocurrencia);
			} else {
				this.totalRegistros = globalLocalidades
						.gettotalgloballocalidadesAll();
				this.totalPaginas = (this.totalRegistros / this.limit) + 1;
				if (this.totalPaginas < this.paginaSeleccion)
					this.paginaSeleccion = this.totalPaginas;
				this.offset = (this.paginaSeleccion - 1) * this.limit;
				...
				this.globalLocalidadesList = globalLocalidades
						.getGlobalLocalidadesAll(this.limit, this.offset);
			}
