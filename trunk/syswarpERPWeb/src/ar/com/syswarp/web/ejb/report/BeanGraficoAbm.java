/* 
 javabean para la entidad: grafico
 Copyrigth(r) sysWarp S.R.L. 
 Fecha de creacion: Mon Jul 03 14:29:38 GMT-03:00 2006 
 
 Para manejar la pagina: graficoAbm.jsp
...
public class BeanGraficoAbm implements SessionBean, Serializable {
	static Logger log = Logger.getLogger(BeanGraficoAbm.class);
	...
	private List graficoList = new ArrayList();
	...
	public boolean ejecutarValidacion() {
		...
		try {
			if (this.accion.equalsIgnoreCase("baja")) {
				if (idgrafico.longValue() < 0) {
					this.mensaje = "Debe seleccionar un registro a eliminar.";
				} else {
					this.mensaje = grafico.graficoDelete(idgrafico);
				}
			}
			if (this.accion.equalsIgnoreCase("modificacion")) {
				if (idgrafico.longValue() < 0) {
					...
					dispatcher = request.getRequestDispatcher("graficoFrm.jsp");
			...
			this.datasourceList = grafico.getDatasourcesAll();
			...
			this.tipoGraficoList = grafico.getTipoGraficosAll();
			...
			if (buscar) {
				...
				this.totalRegistros = grafico.getTotalEntidadOcu("grafico",
				...
				this.graficoList = grafico.getGraficoOcu(this.limit,
						this.offset, this.ocurrencia);
			} else {
				this.totalRegistros = grafico.getTotalEntidad("grafico");
				this.totalPaginas = (this.totalRegistros / this.limit) + 1;
				if (this.totalPaginas < this.paginaSeleccion)
					this.paginaSeleccion = this.totalPaginas;
				this.offset = (this.paginaSeleccion - 1) * this.limit;
				...
				this.graficoList = grafico.getGraficoAll(this.limit,
						this.offset);
			}
	...
	public List getGraficoList() {
		return graficoList;
	}

	public void setGraficoList(List graficoList) {
	...
	public Object recuperarValueHtTG(Object obj) {
		try {
			return this.htTG.get(obj);
		} catch (Exception e) {
			log.error("Object recuperarValueTG( Object obj ): " + e);
			return obj;
		}
	}
