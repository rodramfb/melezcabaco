...
public class BeanClientesAplicacionMovimientos implements SessionBean, Serializable {
	static Logger log = Logger.getLogger(BeanClientesAplicacionMovimientos.class);
	...
	public BeanClientesAplicacionMovimientos() {
		super();
	}
	...
	public void ejecutarSentenciaDML() {
		try {
			...
			this.mensaje = cobranzas.cobranzasMovClienteCreate(idcliente,
					idcobrador, fechamovimiento, this.totalCobranza,
					this.htComprobantes, this.htIdentificadores,
					this.htIdentificadoresIngresos, observaciones, usuarioalt,
					this.idempresa);
			if (Common.esNumerico(this.mensaje)) {
				/*
				dispatcher = request
						.getRequestDispatcher("impresionMovimientoTesoreria.jsp?nrocobranza="
								+ mensaje + "&plantillaImpresionJRXML=modelo.recibo" );
				dispatcher.forward(request, response);
				
				*/
				
				response
				.sendRedirect("../Tesoreria/impresionMovimientoTesoreria.jsp?comprobante="
						+ mensaje + "&plantillaImpresionJRXML=modelo.recibo" );
			}
	...
	public boolean ejecutarValidacion() {
		...
		try {
			...
			if (this.primerCarga) {
				...
				session.setAttribute("htComprobantesOK", new Hashtable());
				session.setAttribute("htIdentificadoresOK", new Hashtable());
			...
			if (!this.confirmar.equals("")) {
				...
				if (this.htComprobantes == null
						|| this.htComprobantes.isEmpty()) {
				...
				if ((this.totalCobranza.subtract(this.totalIngresos)).longValue() != 0) {
					this.mensaje = "Total cobranza y total ingresos deben ser cifras iguales.";
					return false;
				}
