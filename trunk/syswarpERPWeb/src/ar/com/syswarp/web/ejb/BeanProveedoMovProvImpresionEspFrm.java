...
public class BeanProveedoMovProvImpresionEspFrm implements SessionBean,
		Serializable {
	private SessionContext context;

	private BigDecimal idempresa = new BigDecimal(-1);

	static Logger log = Logger.getLogger(BeanProveedoMovProvImpresionFrm.class);
	...
	//20111122 - CAMI - Captura especial bug
	...
	public BeanProveedoMovProvImpresionEspFrm() {
		super();
	}
		...
		try {
			...
			if (this.idproveedor != null && this.idproveedor.longValue() > 0) {
				...
				if (listProveedor.iterator().hasNext()) {
					...
					this.idcondicionpago = new BigDecimal(request.getSession().getAttribute("idcondPago").toString());
					//<-------------------------------------------
				...
				// 20120514 - CAMI - C. C. Especiales
				// esto esta comentado porque no se aplica la discriminacion de iva, se trabaja directamente con el total.
	...
	public boolean calcularImportes() {
		...
		try {
			...
			if (!Common.esNumerico(this.porcentajeBonificacion)) {
				...
				if (!esPorcentaje(this.porcentajeBonificacion)) {
					mensajeCI = " % (2) Bonificación Incorrecto.";
				} else if (!esPorcentaje(this.provPorcNormalIva)) {
					mensajeCI = " % (2) Iva Incorrecto.";
				} else if (!esPorcentaje(this.provPorcEspecialIvaPercep)) {
					...
					// 20120514 - CAMI - C. C. Especiales
					// Se trabajada directamente con el total calculado sin aplicar otro IVA
	...
	public boolean ejecutarValidacion() {
		...
		try {
			...
			if (!this.volver.equalsIgnoreCase("")) {
				response.sendRedirect("proveedoMovProvAbm.jsp");
				return true;
			}
	...
	public String[] getDatosCuentas(String cuenta) throws RemoteException {
		...
		try {
			Contable contable = Common.getContable();
			int ejercicioActivo = Integer.parseInt(this.session.getAttribute(
					"ejercicioActivo").toString());
			List listaCuenta = contable.getCuentasPK(ejercicioActivo, new Long(
					cuenta), this.idempresa);
			if (listaCuenta != null) {
