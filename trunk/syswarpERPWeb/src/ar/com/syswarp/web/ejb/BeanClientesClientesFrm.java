...
public class BeanClientesClientesFrm implements SessionBean, Serializable {
	private SessionContext context;

	static Logger log = Logger.getLogger(BeanClientesClientesFrm.class);
	...
	private BigDecimal idcategoria = new BigDecimal(4);
	...
	/*
	 * nota: se crea la variable resumido la inicializo en 1 esto es para poder
	 * capturarla en el metodo y ejecutar lo que corresponda para el valor 1
	 */
	private BigDecimal resumido = new BigDecimal(1);
	...
	public BeanClientesClientesFrm() {
		super();
	}
		...
		try {
			...
			if (this.accion.equalsIgnoreCase("alta")) {
				this.mensaje = clientesClientes.clientesClientesCreate(
						...
						this.resumido, new BigDecimal(this.sucursalFactura));
				...
				this.mensaje = clientesClientes.clientesClientesUpdate(
						...
						this.usuarioact, this.resumido, new BigDecimal(
								this.sucursalFactura));
		...
		try {
			...
			if (this.isPrimeraCarga()) {
				...
				if (iterClientesClientes.hasNext()) {
					...
					// 20110624 - EJV - Factuaracion FE-CF-MA -->
					this.sucursalFactura = Common.setNotNull(uCampos[34]);
					// <--

					// 20110907 - EJV - Mantis 777 -->
					this.promocion = Common.setNotNull(uCampos[36]);
					// <--
					...
					if (iter.hasNext()) {
						...
						this.idcategoria = new BigDecimal(0);
						this.categoria = "";
						this.mensaje = "Imposible recuperar categoria actual del cliente. / Cliente no categorizado.";
					}
	...
	public boolean ejecutarValidacion() {
		...
		log.info("SUB-ACCION: " + this.subAccion);
		log.info("ACCION: " + this.accion);
		...
		try {
			...
			if (!this.volver.equalsIgnoreCase("")) {
				response.sendRedirect("clientesClientesAbm.jsp");
				return true;
			}
			...
			this.nrotarjeta = Common.setNotNull(this.nrotarjeta).trim();
			...
			if (this.subAccion.equalsIgnoreCase("agregarTarjeta")) {
				...
				if (this.calle.trim().equals("")) {
					...
					domicilioCliente = new Object[] {
							...
							this.statusItem, "0",
							this.idanexolocalidad.toString(), this.obsentrega };
