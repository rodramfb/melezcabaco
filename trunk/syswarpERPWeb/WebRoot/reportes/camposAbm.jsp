...
String color_fondo ="";
String titulo = "CAMPOS";
// variables de entorno
String pathskin = session.getAttribute("pathskin").toString();
String pathscript = session.getAttribute("pathscript").toString();
// variables de paginacion
int i = 0;
Iterator iterCampos   = null;
...
%>
<html>
<jsp:useBean id="BCA"  class="ar.com.syswarp.web.ejb.report.BeanCamposAbm"   scope="page"/>
...
tituCol[5] = "Clase";
tituCol[6] = "Longitud";
tituCol[7] = "Comentario";
java.util.List Campos = new java.util.ArrayList();
Campos= BCA.getCamposList();
iterCampos = Campos.iterator();
%>
<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" >
<div id="overDiv" style="position:absolute; visibility:hidden; z-index:1000;"></div>
<form action="camposAbm.jsp" method="POST" name="frm">
...
<table width="100%" border="0" cellspacing="1" cellpadding="1" name="rsTable" id=rsTable cols=<%=totCol+2-4%>  >
  <tr class="fila-encabezado">
     ...
     <td width="3%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><%=tituCol[6]%></td>
     <td width="28%" onClick="javascript:sortTable(<%=totCol-4%>, rsTable);"><%=tituCol[7]%></td>
   </tr>
   <%int r = 0;
   while(iterCampos.hasNext()){
      ++r;
      String[] sCampos = (String[]) iterCampos.next(); 
      // estos campos hay que setearlos segun la grilla 
      if (color_fondo.equals("fila-det-verde")) color_fondo = "fila-det";
      else color_fondo = "fila-det-verde";%>
   <tr onMouseOver="mOvr(this,this.className='fila-det-rojo');" onMouseOut="mOut(this,this.className='<%=color_fondo%>');" class="<%=color_fondo%>" scope="col" > 
      <td class="fila-det-border" ><input type="radio" name="idcampo" value="<%= sCampos[0]%>"></td>
      <td class="fila-det-border" ><%=sCampos[0]%>&nbsp;</td>
      <td class="fila-det-border" ><%=str.parteStr(sCampos[1], 30, " ...")%>&nbsp;</td>
      <td class="fila-det-border" ><%=str.parteStr(BCA.recuperarValueHtTablas( sCampos[2]  ).toString(), 20, " ...")%>&nbsp;</td>
      <td class="fila-det-border" ><%=sCampos[3]%>&nbsp;</td>
      <td class="fila-det-border" ><%=sCampos[4]%>&nbsp;</td>
      <td class="fila-det-border" ><%=sCampos[5]%>&nbsp;</td>
      <td class="fila-det-border" ><%=sCampos[6]%>&nbsp;</td>
      <td class="fila-det-border" ><%=str.parteStr(sCampos[7], 30, " ...")%>&nbsp;</td>
   </tr>
<%
   }%>
  </table>
