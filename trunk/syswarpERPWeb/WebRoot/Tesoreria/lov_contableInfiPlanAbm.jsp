...
<title><%=titulo%></title>
 ...
 <script >
	function bajarDatos( cuenta, descripcion ){
		<%
		//20081008 Cambios para visualizar descripcion de cuentas.
		if(Common.esEntero(BCIPA.getElemento())){  %>
			var objCuenta = eval("opener.document.getElementById('cuenta<%= BCIPA.getElemento() %>' )");
			var objDescripcionCuenta = eval("opener.document.getElementById('descripcion_cuenta<%= BCIPA.getElemento() %>')");
			objCuenta.value = cuenta;
			objDescripcionCuenta.value = descripcion ;
		<%
		}
		else{  %>
			var objCuenta = eval("opener.document.getElementById('<%= BCIPA.getElemento() %>' )");
			objCuenta.value = cuenta;
		<%
		}%>
		if(!opener.document.frm.nosubmit)
...
<form action="lov_contableInfiPlanAbm.jsp" method="POST" name="frm">
   ...
   <tr onMouseOver="mOvr(this,this.className='fila-det-rojo');" onMouseOut="mOut(this,this.className='<%=color_fondo%>');" class="<%=color_fondo%>" scope="col" > 
      <td class="fila-det-border" >
		<% if(str.esNulo(sCampos[2]).equalsIgnoreCase("S")){ %>
			  <img src="../imagenes/default/gnome_tango/actions/down.png" width="18" height="18" style="cursor:pointer" onClick="bajarDatos(<%=sCampos[0]%>, '<%=sCampos[1]%>')" title="Asignar cuenta <%=sCampos[0]%>.">
		<% } else out.write("&nbsp;");%>			</td>
