...
%>
<html>
<jsp:useBean id="BCIA"  class="ar.com.syswarp.web.ejb.BeanLovCajaIdentificadoresEntradaOtrosMov"   scope="page"/>
...
//-->
  ...
  function addIdentificador( identificador, permitido ){
    if(permitido){
      ...
      alert("Ya ingreso documentos de terceros, no está permtido.\nPara ingresar identificadores, primero elimine de la selección, los documentos de terceros.");
  }

  function getDocumentos(url, nombre, ancho, alto, permitido){
    if(permitido)
      ventana = abrirVentana(url, nombre, ancho, alto);
    else 
      alert("Ya ingreso identificadores, no está permitido.\nPara ingresar documentos de terceros, primero elimine de la selección, los identificadores.");
...
Hashtable htIdentificadoresIngresos = (Hashtable) session.getAttribute("htIdentificadoresIngresos");
Hashtable htOMDocTercerosCancelarINOK  = (Hashtable) session.getAttribute("htOMDocTercerosCancelarINOK"); 
%>
<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" >
<div id="overDiv" style="position:absolute; visibility:hidden; z-index:1000;"></div>
<form action="lov_cajaIdentificadoresEntradaOtrosMov.jsp" method="POST" name="frm">
  ...
  <tr class="fila-encabezado">
   ...
   <tr onMouseOver="mOvr(this,this.className='fila-det-rojo');" onMouseOut="mOut(this,this.className='<%=color_fondo%>');" class="<%=color_fondo%>" scope="col" > 
      <td class="fila-det-border" >
        <div align="center">
			<%if( !sCampos[6].equalsIgnoreCase("D") || 
					   sCampos[7].equalsIgnoreCase("S") ){  %>        
        <img src="../imagenes/default/nuevo.gif" width="16" height="16" onClick="addIdentificador('<%= sCampos[2] %>', <%= (htOMDocTercerosCancelarINOK == null || htOMDocTercerosCancelarINOK.isEmpty()) %>);">
      <%
			  }
			  else{  
      %>
				 <img src="../imagenes/default/gnome_tango/actions/edit-find.png" width="18" height="18" title="<%=sCampos[1].replace ('.', ' ')%> a cancelar." onClick="getDocumentos('lovCajaOtrMovDocTercerosCanc.jsp?tipcart_mt=<%= sCampos[6] %>&cartera_mt=<%= sCampos[2] %>&cartera=<%= sCampos[1] %>', 'buscarmov', 700, 350,  <%= (htIdentificadoresIngresos == null || htIdentificadoresIngresos.isEmpty()) %>)"> 
...
<%
   ...
   &nbsp;
     <% 
  Enumeration en;
	if(  (htIdentificadoresIngresos != null && !htIdentificadoresIngresos.isEmpty()) 
     ||(htOMDocTercerosCancelarINOK != null && !htOMDocTercerosCancelarINOK.isEmpty()) 
    ){
	   %>
   ...
   <table width="100%"  border="0" align="center" cellpadding="0" cellspacing="1">
     <tr class="text-globales">
       ...
       <td width="2%">Sel</td>
       <td width="2%"><div align="center">Ide</div></td>
       <td width="8%"><div align="center">N&uacute;mero</div></td>
       <td width="26%"><div align="center">Detalle</div></td>
       <td width="39%">Cuentas</td>
	...
	if(  (htIdentificadoresIngresos != null && !htIdentificadoresIngresos.isEmpty()) 
	   			...
	   			en = htIdentificadoresIngresos.keys();
					...
					while (en.hasMoreElements()) {
						...
						String [] identificador = (String []) htIdentificadoresIngresos.get(key);
						String nro_cuenta = (identificador[4] + "-" + identificador[34]);
     ...
     <tr class="fila-det"> 
       ...
       <td class="fila-det-border">         <input name="cuotas" type="text" id="cuotas" value="<%= identificador[20] %>" size="3" maxlength="2" class="campo" <%= str.esNulo( identificador[6] ).equalsIgnoreCase("T") ? "": "readonly"%>></td>
       <td class="fila-det-border">         <input name="importeingreso" type="text" value="<%= identificador[28] %>" class="campo" size="10" maxlength="15" style="text-align:right">       </td>
      <td class="fila-det-border">
         ...
         <img src="../imagenes/default/gnome_tango/apps/date.png" width="18" height="18" onClick="abrirVentana('lov_cajaClearingAbm.jsp?elemento=clearing<%=element %>' , 'clearing',700, 200)" style="cursor:pointer" onMouseOver="this.title='Clearing seleccionado: ' + document.getElementById ( 'clearing<%=element %>' ).value">
          <% } else out.write("&nbsp;");%>       </td>
     </tr>
     <%
		 				element++;
...
<%
	if(htOMDocTercerosCancelarINOK != null && !htOMDocTercerosCancelarINOK.isEmpty()){
	    %>
     <tr class="permiso-cuatro" >
       <td height="3" class="fila-det-border" colspan="20">
         
       </td>
     </tr>
			
			<%
		en = htOMDocTercerosCancelarINOK.keys();
		while (en.hasMoreElements()) {
			String key = (String) en.nextElement();
			String [] movimientos = (String []) htOMDocTercerosCancelarINOK.get(key);
						  %>
     <tr class="fila-det">
       <td height="22" class="fila-det-border">
       ...
       <td class="fila-det-border">
				 <input name="chequenro" type="text" value="<%= movimientos[37] %>" class="campo" id="chequenro" size="10" maxlength="10" readonly></td>
       <td class="fila-det-border">
				 <input name="detalle" type="text" value="<%= movimientos[29] %>" class="campo" id="detalle" size="30" maxlength="50"  readonly></td>
       <td class="fila-det-border"><div align="center">
         <% if( str.esNulo(movimientos[6]).equalsIgnoreCase("C")){%>
         </div>
         <% } else out.write("&nbsp;");%></td>
       <td class="fila-det-border">
				 <input name="fecha" type="text" value="<%= movimientos[31] %>" class="campo" id="fecha" size="10" maxlength="10" readonly title="Formato: dd/mm/yyyy "></td>
       <td class="fila-det-border"><div align="center">
       ...
       <td class="fila-det-border"><div align="justify">
         <input name="importeingreso" type="text" value="<%= movimientos[28] %>" class="campo" size="10" maxlength="15" readonly>
       </div></td>
       <td align="center" valign="middle" class="fila-det-border"><div align="center">
			 <% if( str.esNulo(movimientos[8]).equalsIgnoreCase("S")){%>
         </div>
			 <% } else out.write("&nbsp;");%>
			 </td>
     </tr>
