...
function validarCampos() {
	...
	if (document.forms[0].ctapasivo2.value == "") {
...
function passBack(ctapasivo, d_pasivo) {  
  var opcion = "<%= opcion %>";
  var id     =  "<%= id %>";
  if(opcion == "byId"){
    opener.document.getElementById(id).value = ctapasivo ;
		id = (parseInt(id)+1) + "" ;
    opener.document.getElementById(id).value = d_pasivo ;		
  }
	else{
		var i = 0;
		 for(i=0;i<10;i++)
			 if(opener.document.forms[i].ctapasivo)
				 break;    
		 opener.document.forms[i].ctapasivo.value = ctapasivo; 
	}
   close();
}
...
<hr>
<form action="lov_cuentascontables_ctapasivo.jsp">
  <table width="59%" border="0" cellspacing="0" cellpadding="0" height="23">
    <tr> 
      ...
      <td width="54%" height="39"> 
        <input name="ctapasivo2" type="text" class="campo" id="ctapasivo2" value="<%=ctapasivo%>" size="25">
      </td>
...
if (!ctapasivo2.equals("*")) {
  ...
  <table width="100%"  cellPadding=0 cellSpacing=0  border="0">
	...
	<input type="hidden" name="ctapasivo" value="<%=ctapasivo%>">
</form>
