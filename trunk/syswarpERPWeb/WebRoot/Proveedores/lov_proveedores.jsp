...
<title><%=titulo%></title>
 ...
 <script>
 function passBack(idproveedor, dproveedor) {
  var openDesde = (opener.location + "").substring(( opener.location + "" ).lastIndexOf("/") + 1 ).toLowerCase();
  var i = 0;
   for(i=0;i<10;i++)
     if(opener.document.forms[i].idproveedor)
       break;    
   opener.document.forms[i].idproveedor.value = idproveedor;
   if(opener.document.forms[i].dproveedor)
     opener.document.forms[i].dproveedor.value = dproveedor;
   if(opener.document.forms[i].proveedor)
	   opener.document.forms[i].proveedor.value = dproveedor;

   if(openDesde == "cajapagosabm.jsp" || openDesde == "proveedomovprovespfrm.jsp") {
...
<form action="lov_proveedores.jsp" method="POST" name="frm">
...
<table width="100%" border="0" cellspacing="1" cellpadding="1" name="rsTable" id=rsTable cols=<%=totCol+2-4%>  >
  <tr class="fila-encabezado">
   ...
   <tr onMouseOver="mOvr(this,this.className='fila-det-rojo');" onMouseOut="mOut(this,this.className='<%=color_fondo%>');" class="<%=color_fondo%>" scope="col" > 
      <td class="fila-det-border" ><img src="../imagenes/default/gnome_tango/actions/down.png" width="18" height="18" onClick="passBack(<%=sCampos[0]%>, '<%=sCampos[1]%>')"></td>
