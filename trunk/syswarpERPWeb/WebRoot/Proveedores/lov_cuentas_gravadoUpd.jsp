...
%>
<html>
<jsp:useBean id="BLCGA"  class="ar.com.syswarp.web.ejb.BeanLov_cuentas_gravadoAbmUpd"   scope="page"/>
...
<form action="lov_cuentas_gravadoUpd.jsp" method="POST" name="frm">
...
<%
	 ...
	 <% 
   Hashtable htNetoGravadoConfirma = (Hashtable) session.getAttribute("htNetoGravadoConfirmaUpd");
	 if(htNetoGravadoConfirma != null && !htNetoGravadoConfirma.isEmpty() ){
	   %> 	 
		...
		<tr class="fila-det">
			...
			<td class="fila-det-border">&nbsp;
		  <input type="text" class="campo" name="totalAsiento" value="<%= asientoNE[2] %>" size="10" <%= BLCGA.getReadonlyTA() %>></td>
		  <td class="fila-det-border"><%= asientoNE[2] %></td>
		</tr>							
		<%
					}
	  %>
