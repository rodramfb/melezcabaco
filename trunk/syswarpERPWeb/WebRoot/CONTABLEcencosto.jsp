...
/*
Programa CONTABLEcencosto.jsp
Objetivo: Grilla de Centro de Costo
Sistema : Syswarp ERP
Copyrigth: Syswarp S.R.L.
Fecha de creacion: 29/03/2006
...
String action = "CONTABLEcencosto.jsp";
String formulario = "frmCONTABLEcencosto.jsp";
...
Iterator iterCentrodecosto  = null;
...
String fechaalt   = "";
String fechaact   = "";
String ayudalink  = "ayuda.jsp?idayuda=7";   // link a las ayudas en linea de cada punto
...
try{
   ...
   if (accion != null){
      ...
      if (accion.equalsIgnoreCase("baja")){
	        resultadoBaja = contable.delCenCosto(new java.lang.Integer(codigo), new BigDecimal(session.getAttribute("empresa").toString() )); 			   			
     }    
      if (resultadoBaja.equalsIgnoreCase("OK")){
         %><script>alert('Se Borro en forma correcta');</script><%
      }
      else{  
   ...
   java.util.List Centrodecosto = new java.util.ArrayList();	 
   if (buscar==null ) buscar = "";
   if ( buscar.trim().equals("")){	 
     Centrodecosto = contable.getCenCosto(new BigDecimal(session.getAttribute("empresa").toString() ));      
   }
   else
   {
	   Centrodecosto = contable.getCenCosto(buscar.trim(), new BigDecimal(session.getAttribute("empresa").toString() ));    
   }
   iterCentrodecosto = Centrodecosto.iterator();      
   totReg = Centrodecosto.size();   
                  ...
                  <tr> 
                    ...
                    <td width="11%"> <div align="left"> 
                        <input name="buscar" type="submit" id="buscar" value="Buscar" 
														   onmouseover="return overlib('Boton para realizar busquedas por ocurrencias. Si despues de hacer una busqueda desea eliminar el filtro debe quitar la ocurrencia y volver a presionar buscar', STICKY, CAPTION, 'Busquedas',TIMEOUT,3000,HAUTO,VAUTO);"
 ...
 <table width="100%" border="0" cellspacing="1" cellpadding="1" name="rsTable" id=rsTable cols=<%=totCol+2-4%> >
  <tr class="fila-encabezado">
    ...
    <td width="82%"  onClick="javascript:sortTable(<%=totCol+1-4%>, rsTable);"><%=tituCol[2]%></td>
		<td width="6%"  onClick="javascript:sortTable(<%=totCol+1-4%>, rsTable);"><%=tituCol[3]%></td>
  </tr>
 <%int r = 0;
    while(iterCentrodecosto.hasNext()){      
	 ...
	 if ( r >= posIni && r <= posFin ){
	    ...
	    String cmp_codigo = sCampos[0];
		         usuarioalt = sCampos[2] ;
	           usuarioact = sCampos[3] ;
	           fechaalt   = sCampos[4] ;
   	         fechaact   = sCampos[5] ;	 
