...
function validarCampos() {
	...
	if (document.forms[0].ctapresupuesto.value == "") {
...
function passBack(ctapresupuesto, d_ctapresupuesto) {  
  var opcion = "<%= opcion %>";
  var id     =  "<%= id %>";
  if(opcion == "byId"){
    opener.document.getElementById(id).value = ctapresupuesto ;
		id = (parseInt(id)+1) + "" ;
    opener.document.getElementById(id).value = d_ctapresupuesto ;		
  }
	else{
		var i = 0;
		 for(i=0;i<10;i++)
			 if(opener.document.forms[i].ctapresupuesto)
				 break;    
		 opener.document.forms[i].ctapresupuesto.value = ctapresupuesto; 
	}
   close();
}
  ...
  <table width="59%" border="0" cellspacing="0" cellpadding="0" height="23">
    <tr> 
      ...
      <td width="54%" height="39"> 
        <input name="ctapresupuesto" type="text" class="campo" id="ctapresupuesto" value="<%=ctapresupuesto%>" size="25">
      </td>
...
if (!ctapresupuesto.equals("*")) {
  ...
  <table width="100%"  cellPadding=0 cellSpacing=0  border="0">
	...
	<input type="hidden" name="ctapresupuesto" value="<%=ctapresupuesto%>">
</form>
